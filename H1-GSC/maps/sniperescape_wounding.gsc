 ...
 * Script: maps\sniperescape_wounding.gsc
******************************************/
...
bring_out_the_hounds() {
  ...
  for(;;) {
    ...
    for(var_3 = var_1.size - 1; var_3 >= 0; var_3--) {
      ...
      if(bullettracepassed(level.player.origin + (0, 0, 32), var_4.origin + (0, 0, 32), 0, level.player)) {
        continue;
      }
      var_2++;
      var_4.count = 1;
...
plant_price() {
  var_0 = physicstrace(level.price.origin + (0, 0, 2), level.price.origin + (0, 0, -100), level.price);
...
wall_explosion_rocket_acceleration(var_0) {
  ...
  while(isDefined(self) && self.units_per_second < var_0.finalspeed) {
    self.units_per_second = self.units_per_second + var_0.acceleration * 0.05;
...
linktoblade(var_0) {
  self linkto(var_0, self.tag, (0, 0, 0), (0, 0, 0));
  self.root = var_0;
}
...
script_animator() {
  ...
  for(;;) {
    var_0.origin = var_0.origin + (0, 0, -5000);
    var_0 hide();
...
helicopter_broadcast(var_0) {
  ...
  common_scripts\utility::play_sound_in_space(level.scr_sound["heli"][var_1], level.player.origin + (0, 0, 800));
}

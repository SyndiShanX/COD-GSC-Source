 ...
 * Script: maps\zombie_pentagon_magic_box.gsc
**********************************************/
...
magic_box_init() {
  ...
  level._box_locations = array("level1_chest", "level1_chest2", "level2_chest", "start_chest", "start_chest2", "start_chest3");
...
magic_box_update() {
  ...
  box_mode = "no_power";
  while(1) {
    if((!flag("power_on") || flag("moving_chest_now")) && level.zombie_vars["zombie_powerup_fire_sale_on"] == 0) {
      box_mode = "no_power";
    ...
    switch (box_mode) {
      case "no_power":
        setclientsysstate("box_indicator", level._pentagon_no_power);
        while(!flag("power_on") && level.zombie_vars["zombie_powerup_fire_sale_on"] == 0) {
          wait(0.1);
        }
        break;
      case "fire_sale":
      ...
      case "box_available":
        ...
        while(!flag("moving_chest_now") && level.zombie_vars["zombie_powerup_fire_sale_on"] == 0) {
          wait(0.1);
        }
        break;
      default:

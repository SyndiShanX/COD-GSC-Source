...
foreach(array, functor, unary_predicate, predicate_argument) {
  if(isDefined(functor)) {
    for(i = 0; i < array.size; i++) {
      if(isDefined(array[i])) {
        if(!isDefined(unary_predicate) || (isDefined(unary_predicate) &&
            ((isDefined(predicate_argument) && array[i][[unary_predicate]](predicate_argument)) || (!isDefined(predicate_argument) && array[i][[unary_predicate]]())))) {
          array[i][[functor]]();
        }
      }
    }

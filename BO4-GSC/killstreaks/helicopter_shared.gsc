...
function_bff5c062(helicopter, attackingplayer) {
...
getvalidprotectlocationstart(random_path, protectlocation, destination, var_aea79ccc = 0) {
  startnode = level.heli_paths[destination][random_path];
...
getvalidrandomleavenode(start, var_aea79ccc = 1) {
  if(self === level.vtol) {
...
getvalidrandomstartnode(dest, var_aea79ccc = 1) {
...
cantargetactor_turret(actor, hardpointtype) {
  helicopter = self;
  cantarget = 1;
...
heli_hacked_health_update(hacker) {
  helicopter = self;
...
heli_set_active_camo_state(state) {
  heli = self;
...
heli_active_camo_damage_update(damage) {
  self endon(#"death", # "crashing");
  heli = self;
...
heli_health(hardpointtype, playernotify) {
  self endon(#"death", # "crashing");
...
function_86012f82(goalpos, stop) {
  if(!isDefined(self)) {
    return;
  }
...
private function_8de67419(var_b4c35bb7) {
  self endon(#"death");
...
function_62eb6272(var_70031e7b) {
...
function_fabf8bc5(var_b4c35bb7) {
...
heli_leave(var_70031e7b = undefined, var_1caffd41 = 0) {
...
function_438e7b44(startnode, protectdest, hardpointtype, heli_team) {
  self endon(#"death");
  self notify(#"flying");
  self endon(#"flying", # "abandoned");
  self.reached_dest = 0;
...
function_d5de7cb1(protectdest, nodeheight) {

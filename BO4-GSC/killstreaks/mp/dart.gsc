 ...
 * Script: killstreaks\mp\dart.gsc
***********************************************/
...
function_6fe329bb(missileweapon) {
  player = self;
...
dart_hacked_health_update(hacker) {
  dart = self;
...
spawndart(grenade, killstreak_id, spawn_origin) {
  player = self;
  assert(isplayer(player));
  playerentnum = player.entnum;
...
enddartremotecontrol(dart, exitrequestedbyowner) {
...
stop_remote_weapon(attacker, weapon) {
  dart = self;
...
dartdamageoverride(einflictor, eattacker, idamage, idflags, smeansofdeath, weapon, vpoint, vdir, shitloc, vdamageorigin, psoffsettime, damagefromunderneath, modelindex, partname, vsurfacenormal) {
  dart = self;
...
watchownernondeathevents(endcondition1, endcondition2) {
  dart = self;
  player = dart.owner;
  player endon(#"dart_entered");
  dart endon(#"death");

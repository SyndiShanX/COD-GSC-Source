...
function_bff5c062(var_2f03ffd6, attackingplayer) {
...
activatemaingunner(killstreaktype) {
  player = self;
  attempts = 0;
...
waitforvtolshutdownthread(ac130) {
...
function_849819e9(missile, bundle, var_61bbac7a) {
...
function_6650cc9c(missile, bundle, beep_alias) {
  assert(isentity(missile));
  assert(isstruct(bundle));
...
private function_4d980695(isowner) {
  assert(isplayer(self));
  player = self;
...
function_dea7ec6a(einflictor, eattacker, idamage, idflags, smeansofdeath, weapon, vpoint, vdir, shitloc, vdamageorigin, psoffsettime, damagefromunderneath, modelindex, partname, vsurfacenormal) {
  ac130 = self;
...
updatedrivabletimeforalloccupants(duration_ms, end_time_ms) {
...
function_631f02c5(isprimaryweapon) {
  self endon(#"killed");
...
function_568f6426(isprimaryweapon) {
  if(isprimaryweapon) {

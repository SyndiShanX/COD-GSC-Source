 ...
 * Script: killstreaks\emp_shared.csc
***********************************************/
...
get_closest_enemy_emp_killstreak(local_player) {
...
update_distance_to_closest_emp(localclientnum, new_value) {
...
emp_turret_init(localclientnum, oldval, newval, bnewent, binitialsnap, fieldname, bwastimejump) {
  self endon(#"death");
...
cleanup_fx_on_shutdown(localclientnum, handle) {
  self endon(#"kill_fx_cleanup");
  self waittill(#"death");
  stopfx(localclientnum, handle);
}

emp_turret_deploy_start(localclientnum, oldval, newval, bnewent, binitialsnap, fieldname, bwastimejump) {
  self endon(#"death");
...
emp_turret_deploy(localclientnum) {
  self endon(#"death");
  self useanimtree("generic");

 ...
 * Script: mp_common\gametypes\_prop_controls.gsc
**************************************************/
...
notifyonplayercommandremove(command, key) {
  self notify(command + "_" + key);
}

function_255e9614(command, key) {
...
function_9024fce3(buttonfunc, command) {
  while(true) {
...
function_21e5a72f(buttonfunc, command) {
  while(true) {
    while(!self[[buttonfunc]]()) {
      waitframe(1);
    }
...
set_pitch_roll_for_ground_normal(traceignore) {
...
function_c0665212(shouldignore) {
...
function_5b73beef(shouldignore) {
  foreach(player in level.players) {
...
get_ground_normal(traceignore, debug) {
...
function_6de7bc19(success, type, player, origin1, text1, origin2, text2, origin3, text3) {
...
damageclonewatch(damage, attacker, direction_vec, point, meansofdeath, modelname, tagname, partname, weapon, idflags) {
...
function_d8b858d4(var_2953b091, fade_in_time, fade_out_time) {
  level endon(#"game_ended");
  self endon(#"disconnect");
...
function_7278d508(attacker, damageorigin, weapon, damage, meansofdeath) {
...
function_d04b961(attacker, var_28a0d57c, meansofdeath, damage, damageorigin, weapon) {
  if(!isDefined(level.var_fe458fef)) {
    level.var_fe458fef = [];
  }
...
function_5d3e918a(damageorigin) {
  if(!isDefined(level.var_fe458fef)) {
    return;
  }
...
function_d3b8a20e(damageorigin) {
  index = function_5d3e918a(damageorigin);
...
safesetalpha(hudelem, var_9c73f6bb) {
  if(isDefined(hudelem)) {

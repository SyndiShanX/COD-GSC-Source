 ...
 * Script: mp_common\gametypes\escort.gsc
***********************************************/
...
debug_draw_approximate_robot_path_to_goal(&goalpatharray) {
  if((isDefined(getdvarint(#"scr_escort_debug_robot_path", 0)) ? getdvarint(#"scr_escort_debug_robot_path", 0) : 0) == 0) {
    return;
  }
...
debug_draw_current_robot_goal(goal) {
  if((isDefined(getdvarint(#"scr_escort_debug_robot_path", 0)) ? getdvarint(#"scr_escort_debug_robot_path", 0) : 0) == 0) {
    return;
  }
...
debug_draw_find_immediate_goal_override(immediategoal) {
  if((isDefined(getdvarint(#"scr_escort_debug_robot_path", 0)) ? getdvarint(#"scr_escort_debug_robot_path", 0) : 0) == 0) {
    return;
  }
...
debug_draw_blocked_path_kill_radius(center, radius) {
  if((isDefined(getdvarint(#"scr_escort_debug_robot_path", 0)) ? getdvarint(#"scr_escort_debug_robot_path", 0) : 0) == 0) {
    return;
  }
...
delete_on_endgame_sequence() {
  self endon(#"death");
  level waittill(#"endgame_intermission");
  self delete();
}

get_robot_path_array() {
...
calc_robot_path_length(robotorigin, patharray) {
  distance = 0;
...
robot_damage(einflictor, eattacker, idamage, idflags, smeansofdeath, weapon, vpoint, vdir, shitloc, psoffsettime, boneindex, modelindex) {
...
robot_damage_none(einflictor, eattacker, idamage, idflags, smeansofdeath, weapon, vpoint, vdir, shitloc, psoffsettime, boneindex, modelindex) {
  return false;
}
...
reached_closest_nav_mesh_goal_but_still_too_far_and_blocked(goalonnavmesh) {
...
get_closest_point_on_nav_mesh_for_current_goal() {
...
get_closest_point_on_nav_mesh(point) {
...
set_goal_to_point_on_path(recursioncount = 0) {
  self.goaljustblocked = 0;
...
on_update_use_rate_robot_move(team, progress, change) {
...
setup_reboot_object(robot, triggername) {
  trigger = getent(triggername, "targetname");
...
setup_goal_object(robot, triggername) {
  trigger = getent(triggername, "targetname");
...
explode_robot_after_wait(wait_time) {
  robot = self;
  wait wait_time;
...
kill_anything_blocking_goal(goal) {
...
destroy_supply_crate_blocking_goal(dirtogoal) {

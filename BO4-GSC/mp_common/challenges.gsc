 ...
 * Script: mp_common\challenges.gsc
***********************************************/
...
#include scripts\killstreaks\mp\uav;
...
force_challenge_stat(stat_name, stat_value) {
...
fix_tu6_weapon_for_diamond(stat_name) {
  player = self;
...
completed_specific_challenge(target_value, challenge_name) {
  challenge_count = self stats::get_stat_challenge(challenge_name);
  return challenge_count >= target_value;
}

tally_completed_challenge(target_value, challenge_name) {
...
killedbaseoffender(objective, weapon, inflictor) {
...
combat_robot_damage(eattacker, combatrobotowner) {
...
processspecialistchallenge(statname) {
...
private function_7ec2f2c(slot_index, killed) {
  slot = self loadout::get_loadout_slot(slot_index);
  slot.killed = killed;
}

private function_861fe993(slot_index) {
  slot = self loadout::get_loadout_slot(slot_index);
  return slot.killed;
}

function_3ee91387(weapon, playercontrolled, groundbased, countaskillstreakvehicle) {
  self supplypod::hintobjectivehint_updat(weapon);
}

function_19e8b086(killcount, weapon, var_2e4a8800) {
  self specialist_multikill(killcount, weapon, var_2e4a8800);
}

private specialist_multikill(killcount, weapon, scoreevents) {

 ...
 * Script: zm\weapons\zm_weap_hand_gaia.gsc
***********************************************/
  ...
  if(isDefined(var_99588c0f)) {
    ...
    if(isDefined(level.var_f521216c) && level.var_f521216c && var_f9486bd2 > 0) {
      level notify(#"hash_1e5cf7b7965152f3", {
        #var_f9486bd2: var_f9486bd2, #e_player: self
      });
    }
  }
...
function_794f3059(v_end) {
  v_to_goal = v_end - self.origin;
  v_face_angles = vectortoangles(v_to_goal);
  return v_face_angles;
}

function_dced5aef(e_target, weapon = level.weaponnone, n_damage, b_charged, v_dir) {
  self endon(#"disconnect");
...
function_dd7bc108(weapon) {
  ...
  self notify(#"hash_4c8edf52fbfca691", {
    #var_e0ae28d: var_d29931f9
  });
...
function_33f9ab00(a_e_targets, weapon) {
  self endon(#"death");
  v_pos = self.origin;
...
spike_explode(v_pos, weapon) {
  ...
  wait 0.1;
  level notify(#"gaia_spike_exploded", {
    #v_origin: v_pos
  });
  a_e_targets = function_3874b38f();
...
function_5bca6886(e_target, v_dir) {
  self endon(#"death");
  e_target endon(#"death");
  player = self.owner;
...
function_85d88e17(e_target, b_charged, v_dir, n_damage) {
  self endon(#"death");
  e_target endon(#"death");

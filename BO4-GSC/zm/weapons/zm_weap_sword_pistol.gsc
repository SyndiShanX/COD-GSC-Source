 ...
 * Script: zm\weapons\zm_weap_sword_pistol.gsc
***********************************************/
...
private function_6fe1307b(weapon) {
...
function_fe3f086c(e_target, leftswing, weapon = level.weaponnone) {
  if(isactor(e_target)) {
...
function_5e2c9b64(e_target, leftswing, weapon = level.weaponnone) {
  util::wait_network_frame();
  self thread swordpistol_rumble(3);
  self thread function_7d2970bc(e_target, weapon, 2500);
}

function_787cd537(first_time, leftswing, weapon = level.weaponnone) {
  self endon(#"disconnect");
...
function_7d2970bc(e_target, weapon, n_damage) {
  if(isalive(e_target)) {
...
function_5ff8ad0b(weapon, var_fb504c3b) {
  if(!isDefined(var_fb504c3b)) {
    return;
  }
...
private function_be6cef79(weapon) {
  self endon(#"weapon_change", #"disconnect", #"bled_out");
...
private function_e47a52e0(weapon) {
  self endon(#"disconnect");
  ...
  n_time_elapsed = 0;
  level notify(#"viper_bite_projectile", {
    #player: self
  });
...
private function_e29f395c(e_zombie, weapon) {
...
private function_72070f02(player, weapon) {
...
function_37106789(player, weapon) {
  self endon(#"death");
...
function_571d2d19(player, weapon) {
  self endon(#"death");
...
private function_3295542a(player, weapon) {
...
swordpistol_rumble(var_b2e05bae) {
  self endon(#"death");
...
private function_68ff89f7(w_swordpistol) {
  self endon(#"weapon_change", #"disconnect", #"bled_out");

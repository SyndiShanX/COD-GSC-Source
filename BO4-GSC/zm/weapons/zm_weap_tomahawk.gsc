 ...
 * Script: zm\weapons\zm_weap_tomahawk.gsc
***********************************************/
...
private watch_for_tomahawk_throw() {
...
private watch_for_tomahawk_charge() {
  self endon(#"disconnect");
...
private get_grenade_charge_power(player) {
...
private tomahawk_thrown(e_grenade) {
  self endon(#"disconnect");
  ...
  if(a_ai_zombies.size) {
    while(a_ai_zombies.size) {
      if(isalive(a_ai_zombies[0])) {
        ...
        if(var_65f2e452 >= 5) {
          if(var_643ef1e3 >= 5) {
            self notify(#"hash_3669499a148a6d6e", {
...
private check_for_time_out(e_grenade) {
  self endon(#"disconnect");
...
private tomahawk_ricochet_attack(var_65f2e452, e_grenade, var_bc201c9e, e_ignore) {
  self endon(#"disconnect");
...
private function_d81951f5(mdl_tomahawk, var_65f2e452) {
  var_f500f73e = [];
  a_ai_zombies = util::get_array_of_closest(self.origin, getaiteamarray(level.zombie_team), undefined, undefined, 900);
...
private function_e865484a(mdl_tomahawk, a_ai_zombie, var_65f2e452) {
  self endon(#"disconnect");
...
private function_c7ddedb2(mdl_tomahawk, ai_zombie, var_bfed4a7 = 0.25) {
  self endon(#"disconnect");
...
tomahawk_return_player(mdl_tomahawk, var_65f2e452, n_move_speed = 1600) {
  self endon(#"disconnect");
...
private function_e2c00ed6(mdl_tomahawk, n_move_speed, n_total_time) {
...
private function_723cb8bd(mdl_tomahawk) {
  self endon(#"disconnect");
  mdl_tomahawk endon(#"death");
...
private calculate_tomahawk_damage(ai_target_zombie, n_tomahawk_power, tomahawk) {
...
private tomahawk_rumble(var_b2e05bae) {
  if(var_b2e05bae) {
    switch (var_b2e05bae) {
      case 3:

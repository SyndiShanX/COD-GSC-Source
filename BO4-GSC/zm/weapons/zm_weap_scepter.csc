 ...
 * Script: zm\weapons\zm_weap_scepter.csc
***********************************************/
...
function_69ccb98b(localclientnum) {
  if(!isDefined(self)) {
    return;
  }
...
function_da5829be(localclientnum, oldval, newval, bnewent, binitialsnap, fieldname, bwastimejump) {
  if(!isDefined(self)) {
    return;
  }
...
function_f11b8d1d(localclientnum, oldval, newval, bnewent, binitialsnap, fieldname, bwastimejump) {
  if(newval == 1) {
...
beacon_fx(localclientnum, oldval, newval, bnewent, binitialsnap, fieldname, bwastimejump) {
  if(newval == 1) {
    self.b_beacon_fx = 1;
...
scepter_rumble(localclientnum, oldvalue, newvalue, bnewent, binitialsnap, fieldname, wasdemojump) {
  if(newvalue) {
    switch (newvalue) {
      case 2:

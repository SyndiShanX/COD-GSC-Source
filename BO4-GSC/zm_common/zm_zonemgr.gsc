...
zone_wait_till_enabled(zone_name) {
  if(!zone_is_enabled(zone_name)) {
    level waittill(zone_name);
  }
}

function_8caa21df(a_str_zone_names) {
...
function_28cabfbf(a_str_zone_names) {
  if(!isDefined(a_str_zone_names)) {
    a_str_zone_names = [];
  } else if(!isarray(a_str_zone_names)) {
    a_str_zone_names = array(a_str_zone_names);
...
get_zone_from_position(v_pos, ignore_enabled_check = 0, registerchaos_round_end_vo_done_wait_ = 0) {
...
function_49d8d29f(v_pos, ignore_enabled_check) {
  zone = undefined;
...
function_66bf6a43(v_pos, ignore_enabled_check) {
  keys = getarraykeys(level.zones);
...
function_74a82172(location, zone_name, ignore_enabled_check = 0) {
...
function_adc70eee(location, zone_name, ignore_enabled_check = 0) {
  if(!zone_is_enabled(zone_name) && !ignore_enabled_check) {
    return false;
  }
...
add_adjacent_zone(zone_name_a, zone_name_b, flag_name, one_way = 1, zone_tag_a, zone_tag_b) {
...
_debug_show_zone(zone, color, alpha) {
  if(isDefined(zone)) {
...
function_d4cf2b9b(force_update = 0) {

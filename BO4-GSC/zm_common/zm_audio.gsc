...
playerexert(exert, notifywait = 0) {
...
get_mod_kill(impact, mod, weapon, zombie, instakill, dist, player) {
  close_dist = 4096;
  med_dist = 15376;
  far_dist = 160000;
...
function_350fc8cc(weapon, zombie, mod, player) {
...
function_6049efce(weapon, n_kill_count = 10, var_b0f9afde = 6) {
...
function_f9928a31(weapon, n_kill_count = 6) {
...
loadplayervoicecategories(table) {
  index = 0;
...
checkintvalid(value, defaultvalue = 0) {
  if(!isDefined(value)) {
    return defaultvalue;
  }
...
zmbvoxadd(category, subcategory, suffix, percentage = 100, cooldown = 0, var_50219c90 = 0, var_7aadcbd7 = 0, toself = 0, var_884fa3fe = 0) {
...
private function_bf1b121a(timeout, n_range, var_618a04) {
  __timeout__ = timeout;
...
function_bca32e49(category, subcategory, force_variant, b_wait_if_busy = 0) {
...
private _sndvoxoverride(b_toggle) {
...
function_43f17c2(category, subcategory) {
...
success(category, subcategory) {
  self.var_a9a870e2[category][subcategory] = 0;
  function_250becf4(category, subcategory);
}

function_9c83876e(category, subcategory) {
...
function_cd71e220(category, subcategory) {
  if(!isDefined(level.var_79bbe09a)) {
    level.var_79bbe09a = [];
  } else if(!isarray(level.var_79bbe09a)) {
    level.var_79bbe09a = array(level.var_79bbe09a);
...
function_6191af93(str_category, var_39acfdda, var_6e8fc6db, var_60626fa8, var_2bff7297 = 0) {
...
function_e1666976(str_category, var_39acfdda) {
  if(isDefined(self.var_c13233ee)) {
    if(isDefined(self.var_c13233ee[str_category])) {
...
private function_a3932430(str_category, var_39acfdda) {
  if(isDefined(self.var_c13233ee)) {
    if(isDefined(self.var_c13233ee[str_category])) {
      if(isDefined(self.var_c13233ee[str_category][var_39acfdda])) {
...
can_speak(var_7faa9b94 = 0, var_7d84f04b = 0) {
  if(var_7d84f04b) {
...
play_vo_internal(str_sound, e_to_player) {
...
function_631a26f0(str_category, var_39acfdda) {
...
function_31230d39(str_category, var_39acfdda) {
...
function_87714659(func, str_category, var_39acfdda) {
...
isvoxoncooldown(player, category, subcategory) {
...
function_63f85f39(category, subcategory) {
  if(isDefined(level.sndplayervox)) {
...
zmbvoxgetlinevariant(prefix, suffix, var_651f2941, force_variant) {
...
function_c028678a(var_7faa9b94 = 0) {
  var_4d6b74e0 = 0;
...
musicstate_create(statename, playtype = 1, musname1, musname2, musname3, musname4, musname5, musname6) {
...
sndmusicsystem_playstate(state, var_ceca37b = 0) {
...
sndmusicsystem_queuestate(state) {
...
function_a3c4af48(str_location) {
  self notify(#"hash_29bd5874900989d6");
  level thread function_6c53ab30(str_location);
  self thread location_vox(str_location);
}

function_3442c81a(str_location, var_f9761829, b_repeat = 0, var_57b726a7 = 1, var_22324b12 = 2) {
...
function_6c53ab30(str_location) {
  if(!isDefined(level.var_4d771f19)) {
    return;
  }
...
function_b9d832a0(var_790f33a7) {
...
function_8557c25d(str_location, b_toggle) {
  if(!isDefined(level.var_4d771f19)) {
    return;
  }
...
sndmusicsystem_eesetup(state, origin1, origin2, origin3, origin4, origin5) {
  sndeearray = array();
...
sndmusicsystem_eewait(origin, state) {
...
sndmusicsystem_eeoverride(arg1, arg2) {
...
secretuse(notify_string, color, qualifier_func, arg1, arg2) {
  waittillframeend();
...
sndannouncervoxadd(type, suffix) {
...
sndannouncerplayvox(type, player, str_sound, var_e08a84d6, b_wait_if_busy = 0, var_92885147 = 0) {
...
sndplayerhitalert_playSound(str_alias) {
  self endon(#"disconnect");
...
checkforvalidmod(str_meansofdeath) {
...
checkforvalidaitype(e_victim) {
  return true;
}

isfatal(e_victim, damage) {
...
function_713192b1(str_location, var_39acfdda) {

...
entityisarchetype(entity, archetype) {
  if(!isDefined(entity)) {
    return false;
  }
...
setturrettarget(target, turretidx = 0, offset = (0, 0, 0)) {
  self turretsettarget(turretidx, target, offset);
}

fireturret(turretidx, target, offset = (0, 0, 0)) {
...
getairfollowingposition(userelativeangletohost) {
  assert(isairborne(self));
...
getairfollowinglength(targetent) {
  distance = undefined;
...
getairfollowingoffset(targetent, index) {
  numberofpoints = 8;
  pitchrange = 90;
...
javelin_losetargetatrighttime(target, gunnerindex) {
  self endon(#"death");
...
javelin_losetargetatrighttimeprojectile(proj, target) {
  self endon(#"death");
  proj endon(#"death");
  wait 2;
...
burning_thread(attacker, inflictor) {
  self endon(#"death");
...
iff_notifymeinnsec(time, note) {
  self endon(#"death");
  wait time;
  self notify(note);
}

iff_override(owner, time = 60) {
  self endon(#"death");
...
iff_override_team_switch_behavior(team) {
  self endon(#"death");
...
clearallmovement(zerooutspeed = 0) {
  self cancelaimove();
...
update_damage_fx_level(idamage) {
...
shared_callback_damage(einflictor, eattacker, idamage, idflags, smeansofdeath, weapon, vpoint, vdir, shitloc, vdamageorigin, psoffsettime, damagefromunderneath, modelindex, partname, vsurfacenormal) {
...
should_emp(vehicle, weapon, meansofdeath, einflictor, eattacker) {
...
should_burn(vehicle, weapon, meansofdeath, einflictor, eattacker) {
...
startinitialstate(defaultstate = "combat") {
  params = spawnStruct();
...
start_scripted(disable_death_state, no_clear_movement) {
  params = spawnStruct();
...
get_state_callbacks_for_role(rolename = "default", statename) {
  if(isDefined(self.state_machines[rolename])) {
    return self.state_machines[rolename].states[statename];
  }
...
add_state(name, enter_func, update_func, exit_func) {
  if(isDefined(self.current_role)) {
...
init_state_machine_for_role(rolename = "default") {
...
register_custom_add_state_callback(func) {
...
defaultstate_death_enter(params) {
...
death_radius_damage_special(radiusscale, meansofdamage) {
  self endon(#"death");
...
defaultstate_death_update(params) {
  self endon(#"death");
...
defaultstate_scripted_enter(params) {
...
defaultstate_scripted_exit(params) {
  if(params.no_clear_movement !== 1) {
    clearalllookingandtargeting();
    clearallmovement();
  }
...
function_e0887c67(params) {
  self notify(#"endpath");
  self.attachedpath = undefined;
}

defaultstate_combat_enter(params) {}

defaultstate_combat_exit(params) {}

defaultstate_emped_enter(params) {
  self vehicle::toggle_tread_fx(0);
  self vehicle::toggle_exhaust_fx(0);
  self vehicle::toggle_sounds(0);
...
defaultstate_emped_exit(params) {
...
defaultstate_emped_reenter(params) {
  return true;
}

defaultstate_off_enter(params) {
  self vehicle::toggle_tread_fx(0);
  self vehicle::toggle_exhaust_fx(0);
  self vehicle::toggle_sounds(0);
...
function_329f45a4(current_state, to_state, connection, params) {
  if(!isDefined(self)) {
    return false;
  }
...
function_6664e3af(current_state, to_state_name, connection, params) {
  if(isalive(self)) {
    driver = self getseatoccupant(0);
...
defaultstate_driving_enter(params) {
...
defaultstate_driving_exit(params) {
  self enableaimassist();
  self.turretrotscale = 1;
...
defaultstate_pain_enter(params) {
  clearalllookingandtargeting();
  clearallmovement();
}

defaultstate_pain_exit(params) {
  clearalllookingandtargeting();
  clearallmovement();
}
...
_cmp_val(left, right, descending) {
  if(descending) {
...
_sort_by_score(left, right, descending) {
  return _cmp_val(left.score, right.score, descending);
}

positionquery_filter_random(queryresult, min, max) {
  foreach(point in queryresult.data) {
...
positionquery_postprocess_sortscore(queryresult, descending = 1) {
...
positionquery_filter_engagementdist(queryresult, enemy, engagementdistancemin, engagementdistancemax, engagementdistance) {
  if(!isDefined(enemy)) {
    return;
  }
...
positionquery_filter_distawayfromtarget(queryresult, targetarray, distance, tooclosepenalty) {
...
distancepointtoengagementheight(origin, enemy, engagementheightmin, engagementheightmax) {
  if(!isDefined(enemy)) {
    return undefined;
  }
...
positionquery_filter_engagementheight(queryresult, enemy, engagementheightmin, engagementheightmax) {
  if(!isDefined(enemy)) {
    return;
  }
...
function_7ae52016(val) {
  if(isairborne(self)) {
    self setrotorspeed(float(val));
  }
}

private function_e057db25(var_2d1cbdd9, goalpos, vararg) {
  switch (vararg.size) {
    case 8:
...
private function_4ab1a63a(goal) {
...
private function_4646fb11(goal) {

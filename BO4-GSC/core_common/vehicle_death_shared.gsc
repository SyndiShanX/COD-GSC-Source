 ...
 * Script: core_common\vehicle_death_shared.gsc
************************************************/
...
on_vehicle_damage(var_bff7298b) {
...
death_radius_damage(meansofdamage = "MOD_EXPLOSIVE", attacker, weapon) {
  self endon(#"death");
...
watch_for_crash_detour_scene(nd_crash_path) {
  self endon(#"delete");
...
monitor_ground_vehicle_crash_collision() {
...
ground_vehicle_explode(b_delete_me = 0) {
  self endon(#"death");
  self vehicle::do_death_fx();
...
vehicle_damage_filter_damage_watcher(driver, heavy_damage_threshold) {
...
vehicle_damage_filter_exit_watcher(driver) {
  self waittill(#"exit_vehicle", # "death", # "end_damage_filter");
}

vehicle_damage_filter(vision_set, heavy_damage_threshold, filterid = 0, b_use_player_damage = 0) {
  self endon(#"death", # "exit_vehicle", # "end_damage_filter");
...
flipping_shooting_death(attacker, hitdir) {
  if(isDefined(self.delete_on_death)) {
    if(isDefined(self)) {
      self delete();
    }
...
flipping_shooting_crash_movement(attacker, hitdir) {
  self endon(#"crash_done", # "death");
  self cancelaimove();
  self function_d4c687c9();
  self vehclearlookat();
...
deletewhensafe(time = 4) {
  self thread delayedremove_thread(time, 1);
}

delayedremove_thread(time, shoulddelete) {
  if(!isDefined(self)) {
    return;
  }

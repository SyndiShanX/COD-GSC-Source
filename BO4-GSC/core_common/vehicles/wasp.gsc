...
state_guard_can_enter(from_state, to_state, connection) {
...
test_get_back_queryresult(queryresult) {
...
should_fly_forward(distancetogoalsq) {
...
drone_allowfriendlyfiredamage(einflictor, eattacker, smeansofdeath, weapon) {
...
wasp_tower_init(str_tower_targetnames, n_spawn_count) {
...
private _wasp_tower_init(wasp_tower, n_spawn_count) {
...
private wasp_tower_spawn(sp_wasp, n_spawn_count) {
  a_wasps = [];
...
wasp_tower_launch(str_tower_targetnames, n_spawn_count) {
  a_sp_wasp_towers = getEntArray(str_tower_targetnames, "targetname");
...
private _wasp_tower_launch(wasp_tower) {
...
wasp_tower_destroy(str_targetname, str_fxanim_destruction) {

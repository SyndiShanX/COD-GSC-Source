...
function_e2010f4c(entity, var_515373f2) {
...
addaioverridedamagecallback(entity, callback, addtofront) {
...
removeaioverridedamagecallback(entity, callback) {
  assert(isentity(entity));
  assert(isfunctionptr(callback));
...
clearaioverridedamagecallbacks(entity) {
  entity.aioverridedamage = [];
}

addaioverridekilledcallback(entity, callback) {
  assert(isentity(entity));
  assert(isfunctionptr(callback));

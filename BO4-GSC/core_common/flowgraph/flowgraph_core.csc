...
get_node_output_param_index(node_def, param_name) {
...
get_node_input_param_index(node_def, param_name) {
...
get_graph_def(graph_name, force_refresh = 0) {
...
mychanges_watcher() {
  ...
  self.target run({
    #localclientnum: self.localclientnum, #flowgraph_asset: self.def.name
  });
}

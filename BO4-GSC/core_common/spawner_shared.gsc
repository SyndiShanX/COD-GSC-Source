...
global_spawn_throttle(n_count_per_network_frame = 4) {
...
update_target_array(str_target) {
...
get_goal(str_goal, str_key = "targetname") {
...
spawn(b_force = 0, str_targetname, v_origin, v_angles, bignorespawninglimit) {
  self endon(#"death");
  e_spawned = undefined;
  force_spawn = 0;
  makeroom = 0;
...
teleport_spawned(v_origin = self.origin, v_angles = self.angles, b_reset_entity = 1) {
  if(isactor(self)) {
...
add_global_spawn_function(team, spawn_func, ...) {
  if(!isDefined(level.spawn_funcs)) {
    level.spawn_funcs = [];
  }
...
add_ai_spawn_function(spawn_func, ...) {
  if(!isDefined(level.spawn_funcs)) {
    level.spawn_funcs = [];
  }
...
function_89a2cd87(archetype, spawn_func, ...) {
  if(!isDefined(level.spawn_funcs)) {
    level.spawn_funcs = [];
  }
...
remove_global_spawn_function(team, func) {
...
remove_spawn_function_ai_group(str_aigroup, func_spawn, param_1, param_2, param_3, param_4, param_5) {
...
simple_spawn(name_or_spawners, spawn_func, ...) {
  spawners = [];
...
simple_spawn_single(name_or_spawner, spawn_func, ...) {
...
autoexec init_female_spawn() {
  level.female_percent = 0;
  set_female_percent(30);
}

set_female_percent(percent) {

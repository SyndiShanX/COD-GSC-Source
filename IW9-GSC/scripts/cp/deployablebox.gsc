 ...
 * Script: scripts\cp\deployablebox.gsc
***********************************************/
...
init() {
  ...
  level thread _id_34D70A7AB78B1421::armor_crate_init();
...
box_createinteraction(box) {
  ...
  thread _id_38DAD4E3A40930F8(box);
  return box;
}

_id_38DAD4E3A40930F8(box) {
  ...
  while(isDefined(self)) {
    ...
    if(isPlayer(_id_B25B5C45202C880C))
      box notify("captured", _id_B25B5C45202C880C);
  }
}
...
_id_5D89FF9FC37BE576() {
  _id_D12A341B40354143 = level.players.size;
  _id_FCEB11555ACDE199 = 0;

  foreach(player in level.players) {
    if(!isDefined(player._id_AC19F9B9C6C841B9)) {
      continue;
    }
    if(istrue(player._id_AC19F9B9C6C841B9 == "chopper"))
      _id_FCEB11555ACDE199 = 1;
  }

  if(istrue(_id_FCEB11555ACDE199))
    _id_D12A341B40354143 = 1;

  return _id_D12A341B40354143;
}
...
markeractivate(lifeid, boxtype, _id_055AA0066A9F3E9F, damagecallback, deathcallback, _id_C40683E34FE54EC7) {
  ...
  if(getdvarint("dvar_952C135E1269D137", 0) == 0)
    level.boxsettings[boxtype].maxuses = _id_5D89FF9FC37BE576();
  else
  ...
  if(isDefined(level.crafting_table_data) && isDefined(level.crafting_table_data[boxtype])) {
    ...
    if(owner _id_3BCAA2CBAF54ABDD::try_take_player_currency(_id_DE062DAE32F4B236))
      return;
  }
}
...
throwcrate(_id_B7CFF5FEF8FB1DEE) {
  self endon("disconnect");
  _id_3B64EB40368C1450::set("throw_crate", "equipment_primary", 0);
  _id_3B64EB40368C1450::set("throw_crate", "equipment_secondary", 0);

  while(self isgestureplaying("ges_plyr_gesture005"))
    waitframe();

  _id_E872403D3880FAEF = "";

  switch (_id_B7CFF5FEF8FB1DEE.weapon_name) {
    case "iw8_ammo_marker_cp":
      _id_E872403D3880FAEF = "munitions_crate";
      break;
    case "iw8_armor_marker_cp":
      _id_E872403D3880FAEF = "armor";
      break;
  }

  begindeployableviamarker(undefined, _id_E872403D3880FAEF, _id_B7CFF5FEF8FB1DEE, _id_B7CFF5FEF8FB1DEE.weapon_name);
  _id_3B64EB40368C1450::_id_C9D0B43701BDBA00("throw_crate");
}
...
box_handledeath() {
  ...
  if(isDefined(self._id_648A4325C2688078))
    self._id_648A4325C2688078 delete();
...
boxthink() {
  ...
  for(;;) {
    ...
    if(isPlayer(_id_6DFB045EE2B42AAD) && _id_6DFB045EE2B42AAD.team == self.owner.team) {
      if(isDefined(self.disabled_use_for) && istrue(self.disabled_use_for[_id_6DFB045EE2B42AAD getentitynumber()])) {
        _id_6DFB045EE2B42AAD thread scripts\cp\utility::hint_prompt("cant_refill_again", 1, 2);
        continue;
      }

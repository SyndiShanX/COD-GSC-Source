 ...
 * Script: scripts\cp\equipment.gsc
***********************************************/
...
makeexplosiveusabletag(tagname, isgrenade) {
  ...
  for(;;) {
    ...
    if(isDefined(weaponname)) {
      ...
      if(isDefined(name)) {
        if(!self.owner _id_7EF95BBA57DC4B82::hasequipment(name)) {
          self.owner _id_7EF95BBA57DC4B82::giveequipment(name, "primary");
          self.owner _id_7EF95BBA57DC4B82::setequipmentammo(name, 0);
        }
      }
    }

    owner notify("pickup_equipment", weaponname);
    equipmentref = getequipmentreffromweapon(makeweapon(weaponname));

    if(isDefined(equipmentref) && self.owner scripts\cp\cp_equipment::hasequipment(equipmentref))
...
_id_D0E0B1A0DC489379() {
  if(isDefined(level._id_C4EA99FA46D27C12)) {
    foreach(_id_656F0AE440B1B5D5 in level._id_C4EA99FA46D27C12) {
      if(isDefined(_id_656F0AE440B1B5D5._id_3DBA99677FD840CD))
        _id_656F0AE440B1B5D5[[_id_656F0AE440B1B5D5._id_3DBA99677FD840CD]]();
    }
  }
...
_waituntilinteractfinished(streakinfo) {
  ...
  if(streakinfo.interactstate == 2) {
    _id_3504D7A7F76AD9D5 = getdvarfloat("perk_hack_equipment_success_time", 0.5);
...
_updatehackprogressomnvar() {
  ...
  _id_84FDEEC8CD47B94C = getdvarfloat("perk_hack_equipment_time", 3.0) * 1000;
  _id_6CDA039F5C3A871C = getdvarfloat("perk_hack_equipment_success_time", 0.5) * 1000;
...
_processusethink(equipment) {
  ...
  _id_FBC7CCE627CB4D4F = (getdvarfloat("perk_hack_equipment_time", 3.0) - getdvarfloat("perk_hack_equipment_success_time", 0.5)) * 1000;
...
_toggletabletallows(_id_DA3010AF8F6BE463) {
  scripts\cp\utility\player::_freezelookcontrols(_id_DA3010AF8F6BE463);
...
_cleanuptabletallows() {
  level endon("game_ended");
  self endon("disconnect");
  scripts\engine\utility::waittill_any_3("death", "tabletPutAway");
...
_id_7C70DC615DA72C51() {
  ...
  self._id_911CA765F697742A = _id_3D6404612A600581;
}

_id_2DD3214261E60026() {
  if(isDefined(self._id_911CA765F697742A)) {
    _id_7EF95BBA57DC4B82::clearallequipment();

    foreach(slot, data in self._id_911CA765F697742A) {
      _id_7EF95BBA57DC4B82::giveequipment(data.equipment, slot);
      _id_7EF95BBA57DC4B82::setequipmentammo(data.equipment, data.count);
    }

    self._id_911CA765F697742A = undefined;
  }
}

_id_A6C819E1C09A2472() {
  foreach(equipment in self.equipment) {
    if(_id_06CB0B5230400C66(equipment))
      return 0;
  }
...
_id_06CB0B5230400C66(_id_019CD48B2DAF2547) {
  return _id_7EF95BBA57DC4B82::hasequipment(_id_019CD48B2DAF2547) && _id_7EF95BBA57DC4B82::getequipmentammo(_id_019CD48B2DAF2547) < _id_7EF95BBA57DC4B82::getequipmentmaxammo(_id_019CD48B2DAF2547);
}
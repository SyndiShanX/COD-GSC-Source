...
init() {
  loadweaponranktable();
  _id_8215D55CE8CAEE38 = getdvarint("online_mp_weapon_xpscale", 1);
  ...
  for(;;) {
    ...
    if(!isai(player)) {
      if(level.weaponxpenabled) {
        _id_69A07D6024AEB70B = getdvarint("online_mp_party_weapon_xpscale");
        ...
        value = getdvarint("dvar_7E1907B7E66376A7", 40000);
        player.maxweaponxpcap = value;
        player.totalweaponxpearned = 0;
        player.xpperweapon = [];

        foreach(_id_AB501F397D3CD312, value in level.weaponranktable.maxweaponranks)
        player.xpperweapon[_id_AB501F397D3CD312 + "_mp"] = 0;
      }
    }
  }
}
...
loadweaponranktable() {
  ...
  if(isDefined(_id_0D2CA933F911B449)) {
    foreach(weaponname in _id_0D2CA933F911B449) {
      ...
      if(!isDefined(_id_AB501F397D3CD312) || _id_AB501F397D3CD312 == "" || !isDefined(maxrank) || _id_AB501F397D3CD312 == "iw9_la_mike32biolab") {
        continue;
      }
      maxrank = int(maxrank);
...
weaponshouldgetxp(weapon) {
  ...
  if(!_id_F486E79B42C6CBEC && self.pers["rank"] < 3 && !getdvarint("force_ranking"))
    return 0;
...
giveplayerweaponxp(objweapon, type, xp) {
  ...
  if(_id_26F853388D329915 != type) {
    ...
    xp = _id_187A04151C40FB72::getscoreinfovalue(type);
  }
  ...
  scripts\cp\cp_analytics::logevent_givecpweaponxp(objweapon, _id_814960839FF2DC12, _id_EBFEBB497C5F0C29, xp, type);
  ...
  if(_id_A92627842B1B7644 < _id_EBFEBB497C5F0C29) {
    ...
    _id_12239DCA1AEF95CD = _id_187A04151C40FB72::getscoreinfovalue(_id_9060BFFE66826B8C);
    _id_187A04151C40FB72::giverankxp(_id_9060BFFE66826B8C, _id_12239DCA1AEF95CD);
  }
...
getweaponrankxpmultipliertotal() {
  ...
  _id_82547F9421D4DB3D = getdvarfloat("dvar_E5FE74384334CE18", 1);
  return _id_39FBCC95C12E7F4D * _id_17ADAB2B5FACD077 * _id_A37B5D08129F42C2 * _id_82547F9421D4DB3D * _id_C75E49D88DF27EBB;
}

getgametypeweaponxpmultiplier() {
  if(getdvarint("dvar_78653010D584AA6E", 0) == 0) {
    if(!isDefined(level.gametypeweaponxpmodifier))
      level.gametypeweaponxpmodifier = float(tablelookup("mp/gametypesTable.csv", 0, scripts\cp\utility::getgametype(), 19));
...
getgametypekillsperhouravg() {
  if(getdvarint("dvar_78653010D584AA6E", 0) == 0) {
    if(!isDefined(level.gametypekillsperhouravg))
      level.gametypekillsperhouravg = int(tablelookup("mp/gametypesTable.csv", 0, scripts\cp\utility::getgametype(), 20));
...
getgametypekillspermatchmaximum() {
  if(!isDefined(level.gametypekillspermatchmax)) {
    timelimit = getdvarint(_func_2EF675C13CA1C4AF("scr_", scripts\cp\utility::getgametype(), "_timelimit"));

    if(timelimit == 0)
      timelimit = 900;

    winlimit = getdvarint(_func_2EF675C13CA1C4AF("scr_", scripts\cp\utility::getgametype(), "_winlimit"));

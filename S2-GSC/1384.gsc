...
func_5A93() {
  ...
  maps / mp / agents / humanoid / _humanoid_util::func_8318(self.origin, self.angles, var_00, 0, 0, undefined, 1, 1, "klaus_getup");
...
func_5A9B(param_00) {
  var_01 = 75;
  for(;;) {
    if(distance(self.origin, param_00.origin) < var_01) {
      break;
    }
...
func_5A8C() {
  ...
  for(;;) {
    if(common_scripts\utility::func_562E(self.var_5A8B)) {
      foreach(var_01 in level.players) {
        var_02 = distance2d(var_01.origin, self.origin) < 256 && abs(var_01.origin[2] - self.origin[2]) < 64 && !lib_0547::func_577E(var_01);
        if(var_02) {
          var_01 lib_0547::func_7454(3);
          var_03 = lib_0547::func_408F();
          foreach(var_05 in var_03) {
            if(distance(var_05.origin, var_01.origin) > 128) {
              continue;
            }

            var_05 dodamage(500, var_01.origin, var_01);
          }
        }
    ...
    if(common_scripts\utility::func_562E(self.var_5A8B)) {
      foreach(var_01 in level.players) {
        ...
        if(var_01 istouching(self)) {
          var_02 = 200;
          var_03 = 1 - distance(self.origin + (0, 0, 42), var_01.origin) / var_02;
          if(var_03 < 0) {
            var_03 = 0;
          }

          var_01 klaus_knockback(self.origin + (0, 0, 42), var_03 * var_02 + 100);
        }
      }
    }

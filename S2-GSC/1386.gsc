...
init() {
  ...
  func_532B("armor", 500, "armor_buy", &"ZOMBIES_BUY_ARMOR", ::func_4775, ::func_95D2, undefined, undefined, undefined, &"ZOMBIES_HINT_ARMOR");
...
restore_tactical_usage_on_spawn() {
  ...
  for(;;) {
    ...
    if(lib_0547::func_5866(self.zmb_plr_tactical)) {
      ...
      self setweaponammoclip(self.zmb_plr_tactical, var_00);
    }
  }
}
...
func_5295(param_00) {
  ...
  var_02 = common_scripts\utility::func_46B7(var_01.targetname, "targetname");
  foreach(var_04 in var_02) {
    ...
    foreach(var_0B in var_04.var_9DC2) {
      var_0B usetouchtriggerrequirefacingposition(1, var_04.var_F5F.origin + (0, 0, 48));
      var_0B.var_8C82 = 1;
...
func_532C(param_00, param_01) {
  ...
  var_03 = getEntArray(var_02.targetname, "targetname");
  var_04 = undefined;
  if(var_03.size == 4) {
    if(var_02.name == "perk_all") {
      var_05 = common_scripts\utility::func_46B7("perkmachine_look_at_loc", "targetname");
      var_06 = common_scripts\utility::func_4461(var_03[0].origin, var_05);
  ...
  foreach(var_0C in var_03) {
    var_0D = common_scripts\utility::func_4461(var_0C.origin, var_0A, 100);
    var_0C.var_741E = var_0D;
    var_0C.var_6F63 = var_02.name;
    var_02.var_537C = 1;
    if(var_02.name == "perk_all" && isDefined(var_0C.look_at_loc)) {
      var_0C usetouchtriggerrequirefacingposition(1, var_0C.look_at_loc.origin);
    } else {
      var_0C usetouchtriggerrequirefacingposition(1, var_0C.var_82EF.origin + (0, 0, 52));
    }
    ...
    if(var_02.name == "perk_random") {
      var_0C thread func_7A5A(var_02.name);
      continue;
    }

    if(var_02.name == "perk_all") {
      var_0C thread allperkmachinethink(var_02.name);
      continue;
    }

    var_0C thread func_6F61(var_02.name);
  }
}

func_45A0(param_00, param_01) {
  var_02 = function_01AC(param_00, self.origin);
  var_03 = [];
...
assignperkallmachinetoplayer(param_00) {
  ...
  if(1) {
    self.var_65DC = lib_0552::func_7BE1(param_00, self, 1, self.look_at_loc.origin);
...
func_10E2(param_00) {
  param_00 endon("disconnect");
  if(1) {
    self.var_65DC = lib_0552::func_7BF3(param_00, self, 1, self.var_82EF.origin + (0, 0, 52));
  }
  ...
  if(1) {
    ...
    if(common_scripts\utility::func_562E(param_01)) {
      ...
      if(var_03.var_6078 > -1) {
        if(var_03.var_6731 >= var_03.var_6078 && level.players.size == 1) {
          var_05 = "sold_out";
          var_06 = 0;
...
func_A149(param_00, param_01) {
  ...
  for(;;) {
    ...
    param_00 common_scripts\utility::waittill_any("perk_discount_applied", "used_vending_machine_discount", "update_trigger_cost", "perkmachine_activated", "take_perk");
  }
}
...
func_6F62(param_00) {
  ...
  for(;;) {
    ...
    if(var_04) {
      ...
      var_05 = lib_0380::func_2889("zmb_perk_machine_open", undefined, self.origin + (0, 0, 100));
      lib_0378::func_8D14(var_05);
      self.var_82EF setscriptablepartstate("machine_main", "opening");
      wait(getanimlength(%zmb_perk_machine_open));
      self.var_82EF setscriptablepartstate("machine_main", "opened");
    } else {
      var_06 = lib_0380::func_2889("zmb_perk_machine_close", undefined, self.origin + (0, 0, 100));
...
should_open_based_on_distance(param_00, param_01) {
  ...
  return distancesquared(param_00.origin, self.origin) < param_01;
}
...
func_6F61(param_00, param_01) {
  ...
  for(;;) {
    ...
    if(var_03.var_6078 > -1 && var_03.var_6731 >= var_03.var_6078 && level.players.size == 1) {
    ...
    if(var_05 func_4B7E(param_00)) {
      ...
      level notify("perkmachine_player", var_03, self.origin, var_05);
...
func_7A5A(param_00) {
  ...
  for(;;) {
    ...
    if(level.var_6F5C["quickrevive"].var_6078 > -1) {
      ...
      if(isDefined(var_04.numselfrevivedowns) && var_04.numselfrevivedowns >= level.var_6F5C["quickrevive"].var_6078 && level.players.size == 1) {
    ...
    if(var_04 maps / mp / gametypes / zombies::func_11C2(var_08)) {
      ...
      level notify("perkmachine_player", level.var_6F5C[var_07], self.origin, var_04);
      var_04 lib_0547::func_7ACD();
      if(level.var_6F5C[var_07].var_6078 > -1 && level.players.size == 1) {
        level.var_6F5C[var_07].var_6731++;
        if(!isDefined(var_04.numperkbuys)) {
          var_04.numperkbuys = [];
        }

        if(!isDefined(var_04.numperkbuys[level.var_6F5C[var_07].name])) {
          var_04.numperkbuys[level.var_6F5C[var_07].name] = 0;
        }

        var_04.numperkbuys[level.var_6F5C[var_07].name]++;
      }
...
func_6F5D(param_00, param_01) {
  ...
  var_03 = self.var_741E.angles * (1, 0, 0) + param_00.angles * (0, 1, 0);
  ...
  if(common_scripts\utility::func_562E(param_01)) {
    playFX(common_scripts\utility::func_44F5("zmb_pm_random_use"), self.fake_scriptable.origin, anglesToForward(self.fake_scriptable.angles), anglestoup(self.fake_scriptable.angles));
  } else {
  ...
  if(isDefined(level.groundrefent)) {
    ...
    var_07.angles = level.groundrefent.angles;
  ...
  if(common_scripts\utility::func_562E(level.zmb_player_safe_teleport_on_perk_buy)) {
    param_00 setOrigin(getclosestpointonnavmesh(self.var_741E.origin, param_00) + (0, 0, 1));
  }
}
...
func_A183(param_00) {
  level endon("game_over");
  for(;;) {
    level common_scripts\utility::waittill_any("player_spawned", "player_disconnected");
...
func_4772(param_00) {
  ...
  foreach(var_03 in level.var_6F5C) {
    if(param_00 == 0 || common_scripts\utility::func_562E(var_03.var_537C)) {
      if(var_03.name != "armor" && var_03.name != "perk_random" && var_03.name != "perk_all") {
        func_4792(var_03.name);
        var_01++;
      }
    }
  }
...
givefreemachineperks(param_00, param_01, param_02) {
  ...
  foreach(var_07 in level.var_6F5C) {
    var_05 = common_scripts\utility::func_F6F(var_05, var_07.name);
  }
...
func_95D0(param_00) {
  ...
  foreach(var_02 in level.var_6F5C) {
    if(param_00 == 0 || common_scripts\utility::func_562E(var_02.var_537C)) {
      if(var_02.name != "armor" && var_02.name != "perk_random" && var_02.name != "perk_all") {
        func_95D7(var_02.name);
      }
    }
  }
}
...
func_95D8(param_00) {
  ...
  for(var_02 = 0; var_02 < self.var_6F67.size; var_02++) {
    ...
    var_03.index = var_02;
    self setclientomnvar(func_4603(var_03.name), var_02);
  }
  ...
  if(isDefined(self.characterclientindex)) {
    ...
    foreach(var_06 in self.var_6F67) {
      var_07 = var_06.index;
      var_08 = level.var_6F5C[var_06.name].perkid;
      if(isDefined(var_08)) {
...
func_4793(param_00, param_01) {
  ...
  var_02.index = self.var_6F67.size;
  self.var_6F66[param_00] = var_02;
  self.var_6F67[var_02.index] = var_02;
  self setclientomnvar(func_4603(param_00), var_02.index);
  var_03 = undefined;
  var_04 = func_420E(lib_0547::func_4BA7("specialty_class_hoarder_zm"));
  while(self.var_6F67.size > var_04) {
    var_03 = self.var_6F67[0].name;
    func_95D7(self.var_6F67[0].name);
  }
  ...
  if(isDefined(self.characterclientindex)) {
    ...
    foreach(var_07 in self.var_6F67) {
      var_08 = var_07.index;
      var_09 = level.var_6F5C[var_07.name].perkid;
      if(isDefined(var_09)) {
...
func_35B0() {
  ...
  for(;;) {
    ...
    var_09 = common_scripts\utility::func_40B0(var_00.origin, var_09, undefined, var_08, var_07);
    foreach(var_0B in var_09) {
      var_0B maps\mp\zombies\_zombies_traps::mark_electrified();
      var_0B dodamage(var_06, var_0B.origin, var_00, var_00, "MOD_MELEE", "electric_cherry_zm");
      playFX(common_scripts\utility::func_44F5("zmb_elec_cherry_zombie"), var_0B.origin + (0, 0, 40));
    }
  }
}
...
func_5F1E() {
  ...
  foreach(var_02 in var_00) {
    ...
    if(isDefined(var_02.target)) {
      ...
      if(isDefined(var_03)) {
        var_02 usetouchtriggerrequirefacingposition(1, var_03.origin);
      } else {
        var_02 useTriggerRequireLookAt(1);
      }
    }

    if(isDefined(var_03) && isDefined(var_03.target)) {
      var_02.var_191E = getEnt(var_03.target, "targetname");
    }
...
func_5F23(param_00) {
  ...
  for(;;) {
    ...
    if(param_00.var_6F93) {
      var_03 = distance2d(self.origin, param_00.origin);
      if(!lib_0547::func_577E(param_00) && !lib_0547::func_57E1(param_00) || param_00.sessionstate == "spectator") {
...
func_09DF(param_00, param_01) {
  ...
  var_02.x = 8 + self.var_6F66.size - 1 * 16;
  var_02.y = -32;
...
func_4602(param_00, param_01) {
  ...
  if(isDefined(var_02.var_8F63) && level.players.size == 1) {
...
func_0F66(param_00) {
  ...
  for(;;) {
    ...
    param_00 common_scripts\utility::waittill_any("perkmachine_activated", "take_armor", "take_perk", "perk_discount_applied", "update_armor_hints");
  }
}

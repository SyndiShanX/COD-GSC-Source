 ...
 * Script: maps\mp\zombies\sg_events_v1\escape_sequence.gsc
************************************************************/
...
initialize_markers() {
  ...
  foreach(var_02 in level.zmf_shattered_hardpoint_structs) {
    var_03 = bulletTrace(var_02.origin + (0, 0, var_00), var_02.origin - (0, 0, var_00), 0);
    if(distance(var_02.origin, var_03["position"]) > var_00) {
      continue;
    }

    var_02.origin = var_03["position"] + (0, 0, 0.25);
  }
}
...
run_escape(param_00) {
  ...
  if(isDefined(var_02.angles)) {
    var_04.angles = var_04.angles;
  }

  var_03.capture_health = 100;
  var_03.health = 100;
  var_03.maxhealth = 100;
  ...
  var_08.waypoint_origin = var_08.origin + (0, 0, 80);
  var_08.waypoints = [];
...
capture_escape_point(param_00, param_01) {
  ...
  while(isDefined(param_00) && param_00.capture_health > 0) {
    ...
    param_00.capture_health = int(max(0, param_00.maxhealth - int(param_00.maxhealth * var_05 / param_01)));
    wait(var_04);
  }
  ...
  foreach(var_09 in level.players) {
    if(isalive(var_09) && distance2d(var_09.origin, var_02) < var_03) {

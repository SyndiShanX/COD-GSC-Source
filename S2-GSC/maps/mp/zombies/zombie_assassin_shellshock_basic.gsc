 ...
 * Script: maps\mp\zombies\zombie_assassin_shellshock_basic.gsc
****************************************************************/
  ...
  foreach(var_08 in var_05) {
    if(lib_0547::func_5565(var_08, "vm_zom_jack_in_the_box")) {
      ...
      foreach(var_0B in common_scripts\utility::func_F92(level.players)) {
        ...
        if(common_scripts\utility::func_562E(var_0B.sessionstate == "spectator")) {
          continue;
        }
      ...
      var_0D = 215 * vectorNormalize(anglesToForward(vectortoangles(var_09.origin - self.origin)));
      var_0D = (var_0D[0], var_0D[1], var_02);
      var_0E = magicgrenademanual("jack_in_box_decoy_zm", getclosestpointonnavmesh(self.origin, self) + (0, 0, 32), var_0D, 10, self, 1);
    ...
    var_0E = spawn("script_model", self.origin + (randomint(64) - 64, randomint(64) - 64, 0));
...
do_shellshock_jack_buff(param_00) {
  ...
  for(;;) {
    ...
    foreach(var_03 in var_01) {
      ...
      if(distance(self.origin, var_03.origin) > 112) {
        continue;
      }
...
do_jack_blood_beam(param_00) {
  ...
  wait 0.05;
  playFX(common_scripts\utility::func_44F5("stunning_burst"), self.origin);
  ...
  playFX(common_scripts\utility::func_44F5("zmb_isl_med_trap_gib_rnr"), param_00.origin);
  param_00 dodamage(param_00.health + 1, self.origin, undefined, undefined, "MOD_PROJECTILE", "fliegerfaust_zm");
  var_02 delete();
  var_01 delete();
}

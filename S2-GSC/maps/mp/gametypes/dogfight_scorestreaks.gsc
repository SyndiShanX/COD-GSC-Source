 ...
 * Script: maps\mp\gametypes\dogfight_scorestreaks.gsc
*******************************************************/

init_dogfight_killstreaks() {
  level thread scorestreakdvarhostmigration();
  level.killstreakfuncs["dogfight_flak"] = ::tryusedogfightflak;
...
tryusedogfightflak(param_00) {
  ...
  if(getdvarint("dogfightFlakDamageScaleEnabled", 1) || getdvarint("dogfightFlakDamageWeightEnabled", 1)) {
    ...
    var_01 = var_03.origin[2];
    var_02 = var_03.origin[2] + var_03.height;
  }
...
fighter_scorestreak_watcher(param_00) {
  ...
  for(;;) {
    var_01 = param_00 common_scripts\utility::waittill_any_return("activate_fighter_scorestreak", "activate_fighter_scorestreak_keyboard");
    if(isDefined(var_01)) {
...
dogfight_flakgetzonescale(param_00, param_01) {
  var_02 = 1;
  if(self.origin[2] > param_01) {
    var_02 = 1;
  } else if(self.origin[2] < param_00) {
    var_02 = 0;
  } else {
    var_02 = self.origin[2] - param_00 / param_01 - param_00;
  }
    ...
    if(level.var_3CE1 < 30) {
      ...
      if(isDefined(var_02)) {
        if(getdvarint("dogfightFlakDamageWeightEnabled", 1)) {
          ...
          var_0A = (randomfloatrange(-500, 500) + var_03.origin[0], randomfloatrange(-500, 500) + var_03.origin[1], randomfloatrange(-500, 500) + var_03.origin[2]);
        }
      } else {
...
dogfight_flakgetnearfighterexplosionlocation(param_00, param_01, param_02, param_03, param_04, param_05, param_06) {
  ...
  var_0C = (var_08[0] + param_00.origin[0], var_08[1] + param_00.origin[1], var_08[2] + var_0B + param_00.origin[2]);
  return var_0C;
}
  ...
  for(;;) {
    ...
    if(isDefined(var_03)) {
      ...
      if(getdvarint("dogfightFlakDamageWeightEnabled", 1)) {
        ...
        var_0A = (randomfloatrange(-100, 100) + var_0D.origin[0], randomfloatrange(-100, 100) + var_0D.origin[1], randomfloatrange(-100, 100) + var_0D.origin[2]);
      }
      ...
      if(1) {
        ...
        earthquake(0.7, 0.5, var_03.origin, 800, var_03.occupied_player);
      }
    }
...
getflaktargets() {
  ...
  foreach(var_02 in level.pp_array) {
    if(level.teambased) {
      if(isDefined(var_02.team) && var_02.team == common_scripts\utility::func_416F(self.team)) {

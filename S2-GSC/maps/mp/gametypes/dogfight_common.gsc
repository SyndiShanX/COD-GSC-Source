 ...
 * Script: maps\mp\gametypes\dogfight_common.gsc
*************************************************/
...
ondogfightstart() {
  ...
  if(isDefined(level.dogfighthardboundary) && isDefined(level.dogfightsoftboundary)) {
    ...
    if(function_0296(level.dogfightcenter)) {
      ...
      setup_boundaries_with_origin(var_00.origin);
    }
  } else {
...
setup_minimap(param_00, param_01) {
  if(isDefined(param_01)) {
    ...
    maps\mp\_compass::setupminimap(param_01);
  }
}
...
get_formation_spawn_info_with_angle(param_00) {
  var_01 = level.dogfightformationspawnstruct.origin + (0, 0, level.dogfightformationspawnstruct.height * 0.5);
  var_02 = (0, param_00, 0);
  var_03 = (1, 0, 0);
...
configure_formation_spawn() {
  ...
  for(var_07 = 1; var_07 < 9; var_07++) {
    var_0A = level.formation_nodes[var_07].origin - level.formation_nodes[0].origin;
...
onteamswitch(param_00) {
  ...
  var_01 = self;
  var_02 = var_01.team;
  var_03 = var_01 common_scripts\utility::waittill_any_return_no_endon_death("joined_team", "joined_spectators", "disconnect");
...
dogfight_wait_for_match_timer(param_00, param_01) {
  ...
  setup_minimap(param_00, param_01);
  wait(level.prematchperiodend - 3);
  function_03C6(100, 187.5, 284.09, 143.75);
}
...
common_damage_state_fx_update_watcher(param_00, param_01) {
  ...
  self.vfx_obj_list = [];
  for(;;) {
    if(function_0279(var_02) || var_02.health <= param_01 || common_scripts\utility::func_562E(level.match_ended)) {
...
common_damage_state_fx_watcher(param_00, param_01) {
  ...
  for(;;) {
    var_07 = var_02.health / var_02.maxhealth;
    var_08 = -1;
...
fighter_get_crash_animation(param_00, param_01) {
  ...
  foreach(var_0A in var_07) {
    ...
    var_0C = rotatevector(var_0B, param_00.angles);
...
add_target_icon(param_00, param_01, param_02, param_03, param_04, param_05) {
  ...
  if(!isDefined(self) || !isalive(param_00) || var_06 != self || var_07 != param_00.birthtime || isDefined(self.var_1C8) && !fighter_is_alive()) {
    return;
  }
...
dogfightshowboundaryoverlay() {
  if(isDefined(self.var_1B68)) {
    ...
    self.var_1B68.x = 0;
    self.var_1B68.y = 0;
    self.var_1B68.sort = -5;
    self.var_1B68.horzalign = "fullscreen";
    self.var_1B68.vertalign = "fullscreen";
    self.var_1B68 setshader("overlay_raid_boundary", 640, 480);
    self.var_1B68.hidewheninmenu = 1;
    self.var_1B68.showinkillcam = 0;
    self.var_1B68.alpha = 0;
  }
...
dogfightwatchboundaryoverlay(param_00) {
  ...
  for(;;) {
    ...
    self.var_1B68.alpha = clamp(0.3 + max(var_01, var_02), 0, 1);
    wait 0.05;
  }
}
...
createvehicleinternal(param_00, param_01, param_02) {
  ...
  if(common_scripts\utility::func_562E(param_00.use_original_classname)) {
    ...
    var_04 = function_01E0(param_00.model, var_03, param_00.var_1C8, param_00.origin, param_00.angles, param_02);
  } else {
    var_04 = function_01E0(param_00.model, var_03, param_00.var_1C8, param_00.origin, param_00.angles);
  }

  var_04.var_1C8 = param_00.var_1C8;
  var_04.maxhealth = 9000;
  var_04.health = 9000;
  var_04.var_6A = 0;
...
get_spawn_center() {
  return level.dogfightspawnmaxstruct.origin + (0, 0, level.dogfightspawnmaxstruct.height * 0.5);
}
...
get_spawn_height(param_00) {
  var_01 = undefined;
  if(param_00) {
    var_01 = level.dogfightspawnminstruct.height;
  } else {
    var_01 = level.dogfightspawnmaxstruct.height;
  }
...
get_formation_spawn_origin_angles(param_00) {
  ...
  var_02 = undefined;
  var_03 = (0, 0, 0);
  if(level.dogfightformationspawnstruct.height > 0) {
    var_04 = level.dogfightformationspawnstruct.height * 0.5;
...
is_spawn_targeted(param_00) {
  foreach(var_02 in level.players) {
    ...
    var_04 = anglesToForward(var_02.occupied_plane.angles);
...
player_spawn_helper(param_00) {
  ...
  if(isbot(param_00) || function_01EF(param_00)) {
    param_00.maxvisiondistsqrd = 576000000;
  }
...
get_turret_weapon_info(param_00) {
  ...
  if(level.teambased && param_00 == "axis") {
...
spawn_player_p47(param_00) {
  var_01 = get_spawner(param_00.team);
  var_02 = createvehicleinternal(var_01, param_00.team, param_00);
...
spawn_fighter(param_00) {
  ...
  if(var_01) {
    ...
    var_06 = get_formation_spawn_origin_angles(param_00.team);
    var_04 = var_06[0];
    var_07 = var_06[1];
  ...
  if(function_01EF(param_00)) {
    param_00.agentname = &"RAIDS_DOGFIGHT_AGENTNAME";
    foreach(var_0A in level.players) {
      if(!level.teambased || var_0A.team == common_scripts\utility::func_416F(param_00.agentteam)) {
  ...
  var_0C = get_turret_weapon_info(param_00.team);
  ...
  if(isPlayer(param_00)) {
    foreach(var_0A in level.players) {
      ...
      if((!level.teambased || var_0A.team == common_scripts\utility::func_416F(param_00.team)) && isDefined(var_0E) && var_0E fighter_is_alive()) {
        var_0E thread add_target_icon(param_00, 0, 1, 0, undefined, var_05);
      }

      if(!level.teambased || var_0A.team == common_scripts\utility::func_416F(param_00.team)) {
        var_03 thread add_target_icon(var_0A, 0, 1, 0, undefined, var_05);
      }
    }
    ...
    if(isDefined(level.var_A4E)) {
      foreach(var_11 in level.var_A4E) {
        ...
        if(isDefined(var_0E) && !level.teambased || var_11.agentteam == common_scripts\utility::func_416F(param_00.team)) {
          var_0E thread add_target_icon(param_00, 0, 1, 0, undefined, var_05);
        }
      }
    }
...
validate_boundaries() {
  ...
  var_05 = level.dogfightsoftboundarystruct.origin;
  var_06 = var_05 + (0, 0, level.dogfightsoftboundarystruct.height);
  var_07 = level.dogfighthardboundarystruct.origin + (0, 0, level.dogfighthardboundarystruct.height * 0.5);
  var_08 = level.dogfightsoftboundarystruct.origin + (0, 0, level.dogfightsoftboundarystruct.height * 0.5);
  var_09 = 0.0001;
  var_0A = level.dogfightspawnminstruct.origin;
  var_0B = var_0A + (0, 0, level.dogfightspawnminstruct.height);
  var_0C = level.dogfightspawnmaxstruct.origin;
  var_0D = var_0C + (0, 0, level.dogfightspawnmaxstruct.height);
  var_0E = level.dogfightspawnminstruct.origin + (0, 0, level.dogfightspawnminstruct.height * 0.5);
  var_0F = level.dogfightspawnmaxstruct.origin + (0, 0, level.dogfightspawnmaxstruct.height * 0.5);
}
...
get_dogfight_boundary_height(param_00) {
  var_01 = undefined;
  if(param_00) {
    var_01 = level.dogfightsoftboundarystruct.height;
  } else {
    var_01 = level.dogfighthardboundarystruct.height;
  }
...
get_dogfight_boundary_center(param_00) {
  var_01 = undefined;
  if(param_00) {
    var_01 = level.dogfighthardboundarystruct.origin + (0, 0, level.dogfighthardboundarystruct.height * 0.5);
  } else {
    var_01 = level.dogfightsoftboundarystruct.origin + (0, 0, level.dogfightsoftboundarystruct.height * 0.5);
  }
...
is_plane_outside_soft_boundary_radius() {
  var_00 = 0;
  var_01 = distance2dsquared(self.origin, get_dogfight_boundary_center(1));
...
get_plane_vertical_soft_boundary_percent() {
  ...
  if(is_plane_outside_soft_boundary_top() || is_plane_outside_soft_boundary_bottom()) {
    var_01 = get_dogfight_boundary_center(1);
    var_02 = abs(self.origin[2] - var_01[2]);
...
get_plane_horizontal_soft_boundary_percent() {
  ...
  if(is_plane_outside_soft_boundary_radius()) {
    var_01 = 0;
    var_02 = distance2dsquared(self.origin, get_dogfight_boundary_center(1));
...
is_plane_outside_hard_boundary() {
  ...
  var_01 = distance2dsquared(self.origin, var_00);
  var_02 = abs(self.origin[2] - var_00[2]);
  ...
  for(;;) {
    ...
    if(param_00 is_plane_outside_soft_boundary()) {
      ...
      if(common_scripts\utility::func_562E(var_07) && var_03 <= gettime()) {
        var_08 = vectorNormalize(param_00.origin - var_05);
        var_09 = vectorNormalize(param_00.angles);
        var_0A = vectorNormalize(rotatevectorinverted(var_08, param_00.angles));
...
plane_boundary_blowup(param_00) {
  var_01 = self;
  var_01.var_2AB1 = 1;
  param_00 dodamage(param_00.maxhealth * 10, param_00.origin, undefined, undefined, "MOD_EXPLOSIVE");
}
...
common_health_regen(param_00, param_01, param_02, param_03, param_04, param_05) {
  ...
  wait(param_00);
  var_08 = param_02 * var_06.maxhealth / 100;
  var_08 = int(clamp(var_08, param_05, var_06.maxhealth));
  while(var_06.health > param_05 && var_06.health < var_06.maxhealth) {
    var_09 = 1;
    if(isDefined(param_04)) {
      var_09 = [[param_04]]();
    }

    var_0A = int(clamp(var_08 * var_09, param_05, var_06.maxhealth));
    var_06.health = var_06.health + var_0A;
    if(isDefined(var_07)) {
      var_0B = var_06.health / var_06.maxhealth;
      var_07.health = int(var_07.maxhealth * var_0B);
    }

    var_06 common_clear_pending_assists(var_0A);
    if(var_06.health >= var_06.maxhealth) {
...
fighter_on_health_update_func() {
  ...
  if(isDefined(var_01)) {
    var_02 = var_00.health / var_00.maxhealth;
...
fighter_play_crash_anim(param_00, param_01, param_02, param_03) {
  ...
  param_00 cameralinkTo(param_01, "tag_player", 0);
}
...
fighter_death(param_00, param_01, param_02, param_03, param_04, param_05, param_06) {
  ...
  if(var_10) {
    if(function_01EF(param_00)) {
      ...
      if(isDefined(var_0E)) {
        param_00 cameralinkTo(var_0E, "tag_player", 0);
      }
...
fighter_damage_callback(param_00, param_01, param_02, param_03, param_04, param_05, param_06, param_07, param_08, param_09, param_0A, param_0B) {
  ...
  if(isDefined(param_01.team) && level.teambased && var_0C.team == param_01.team && !level.dogfightfriendlyfireenabled) {
    return;
  }
  ...
  var_0C.health = var_0C.health - int(param_02);
  var_0C.var_5B8A = gettime();
  var_0C fighter_on_health_update_func();
  if(1 && level.dogfighthealthregenscale > 0 && var_0C.health > 1) {
  ...
  if(isDefined(var_0D)) {
    var_10 = var_0C.health / var_0C.maxhealth;
    var_11 = int(var_0D.maxhealth * var_10);
    var_0D dodamage(var_0D.health - var_11, param_01.origin, param_01, param_00, param_04, param_05, param_08);
  }
...
common_award_pending_assists(param_00, param_01) {
  ...
  if(isDefined(var_02.damageregistry) && var_02.damageregistry.size > 0) {
    ...
    foreach(var_06, var_07 in var_03) {
      ...
      var_0F = int(100 * var_07 / var_02.maxhealth);
      var_10 = "assist_low";
      ...
      var_0A thread maps\mp\_events::processassistevent(param_01, var_10);
    }
  }
}
...
bot_planeshootthink(param_00) {
  ...
  for(;;) {
    ...
    foreach(var_08 in level.players) {
      if(!level.teambased || var_08.team != var_01.team) {
...
bot_planeflythink(param_00) {
  ...
  for(;;) {
    ...
    if(isDefined(var_04)) {
      ...
      if(isDefined(var_05) && distancesquared(var_04.origin, var_05) >= var_07) {
        var_03 = var_05;
      }
    } else {
      var_03 = var_05;
    ...
    var_08 = vectorNormalize(var_03 - param_00.origin);
    var_09 = 9437184;
    if(function_01EF(var_01) && isDefined(var_04)) {
      var_09 = 0;
    }

    if(var_09 > 0 && distancesquared(param_00.origin, var_03) < var_09) {
      var_08 = var_08 * -1;
    }
    ...
    var_0B = var_0A[1];
    var_0C = var_0A[0];
    if(level.teambased && common_scripts\utility::func_562E(var_01.informationtransition) && isDefined(level.defender_spawn_angles) && isDefined(level.attacker_spawn_angles)) {
...
agent_spawn(param_00, param_01, param_02, param_03, param_04, param_05, param_06) {
  ...
  var_07.maxhealth = 100;
  var_07.health = var_07.maxhealth;
  var_07.name = "Agent";
  var_07.var_565F = 1;
  var_07.var_4B60 = 0;

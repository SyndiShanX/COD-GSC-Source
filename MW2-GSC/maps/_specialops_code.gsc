 ...
 * Script: maps\_specialops_code.gsc
********************************************************/
  ...
  if(so_hud_can_toggle(hudelem)) {
    switch (self.so_infohud_toggle_state) {
      ...
      case "none":
        ...
        break;
      case "off":
        hudelem fade_over_time(0, 0.5);
        break;
      default:
        assertmsg("so_create_hud_item_delay_draw() encountered a setting for player.so_infohud_toggle_state (" + self.so_infohud_toggle_state + ") it didn't recognize.");
    }
  } else {
    hudelem fade_over_time(1, 0.5);
  }
...
determine_waiting_message(player, touch_style) {
  switch (touch_style) {
    case "all":
      player display_waiting_message();
      break;
    case "any":
      player display_waiting_message();
      break;
    case "freeze":
      player display_frozen_message();
      break;
  }
}
...
so_dog_death(attacker) {
  ...
  switch (level.dog_death_type) {
    case "soon":
      so_special_failure_hint_set("@NEW_DOG_DEATH_TOO_SOON_ALT", "ui_dog_death");
      break;
    case "late":
      so_special_failure_hint_set("@NEW_DOG_DEATH_TOO_LATE_ALT", "ui_dog_death");
      break;
    default:
      so_special_failure_hint_set("@NEW_DOG_DEATH_DO_NOTHING_ALT", "ui_dog_death");
      break;
  }

 ...
 * Script: maps\af_chase_knife_fight_code.gsc
********************************************************/
...
spawn_price() {
  ...
  level.price = maps\_vehicle_aianim::convert_guy_to_drone(guy);
...
shepherd_stumbles() {
  ...
  for(;;) {
    ...
    if(trace_passed && flag("shepherd_can_run")) {
      break;
    }
  }
...
gun_crawl_price_falls() {
  ...
  anim_node anim_first_frame_solo(level.shepherd, "turn_buckle");
...
gun_crawl_fight_idle() {
  ...
  new_anim_node.origin = out_of_sight_offset;
...
convert_shepherd_to_drone() {
  ...
  level.shepherd = maps\_vehicle_aianim::convert_guy_to_drone(level.shepherd);
  Objective_OnEntity(obj("get_shepherd"), level.shepherd);
  level.compass_ent = level.shepherd;

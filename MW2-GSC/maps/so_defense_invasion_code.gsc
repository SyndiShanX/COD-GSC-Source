 ...
 * Script: maps\so_defense_invasion_code.gsc
********************************************************/
...
hunter_enemies_refill(refill_at, min_fill, max_fill, refill_max) {
  ...
  while(isDefined(level.hunter_refill_active) && level.hunter_refill_active) {
    if(!isDefined(level.hunters_active) || (level.hunters_active <= refill_at)) {
      ...
      switch (spawn_option) {
        case "bank":
          respawn_amount = hunter_enemies_refill_group("bank_enemies", respawn_amount, "north");
          break;
        case "gas":
          respawn_amount = hunter_enemies_refill_group("gas_station_enemies", respawn_amount);
          break;
        case "taco":
          respawn_amount = hunter_enemies_refill_group("taco_enemies", respawn_amount, "south");
          break;
        case "burger":
          respawn_amount = hunter_enemies_refill_group("burger_town_enemies", respawn_amount, "south");
          break;
        default:
...
hunter_enemies_refill_group(enemy_group, respawn_amount, smoke_dir) {
  if(isDefined(smoke_dir)) {
    if((randomfloat(1.0) < level.smoke_chance) || !level.hunter_refill_used_smoke) {
      ...
      switch (smoke_dir) {
        case "north":
          thread maps\so_defense_invasion::enable_smoke_wave_north(4);
          break;
        case "south":
          thread maps\so_defense_invasion::enable_smoke_wave_south(4);
          break;
        default:
          break;
      }

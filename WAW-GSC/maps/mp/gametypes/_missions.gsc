...
processChallenge(baseName, progressInc) {
  ...
  */
  ...
  if(progress >= level.challengeInfo[refString]["maxval"]) {
    ...
    statName = level.challengeInfo[refString]["fullname"];

    statLen = statName.size;

    postShipStatsFix = getdvarint("ffotd_stats_fix");

    if((postShipStatsFix == 0) || (statLen < 2) || (getSubStr(statName, statLen - 2, statLen - 1) != "_")) {
      self setStatLBByName(statName, 1);
    } else {
      columnName = getSubStr(statName, statLen - 1, statLen);
      statName = getSubStr(level.challengeInfo[refString]["fullname"], 0, statLen - 2);
      self setStatLBByName(statName, 1, columnName);
    }
  ...
  for(i = 0; i < players.size; i++) {
    ...
    if(isDefined(mapname) && mapname != "") {
      ...
      if(isdefined(players[i].squadid)) {
        // cap it at 30 minutes to avoid exploiting
        totalTimePlayed = int(min(players[i].timePlayed["total"], level.timeplayedcap));

        if(players[i].squadid == 0) {
          players[i] setStatLBByName(mapname2, totalTimePlayed, "Time as Lone Wolf");
        } else {
          players[i] setStatLBByName(mapname2, totalTimePlayed, "Time In Squad");
        }
      }
    }

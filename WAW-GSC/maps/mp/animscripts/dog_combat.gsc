  ...
  for(;;) {
    ...
    if(dog_cant_kill_in_one_hit(player)) {
      ...
      if(use_low_attack()) {
        ...
        // 1.6 is about as big as you want to go or the dog anim will "stall"attack_time = 1.2 + randomfloat(0.4);
        debug_anim_print("dog_combat::meleeBiteAttackPlayer() - Settingcombat_run_attack");
      ...
      debug_anim_print("dog_combat::meleeBiteAttackPlayer() - Settingcombat_attack_player");
...
handleMeleeFinishAttackNoteTracks(note, player) {
  switch (note) {
    ...
    case "dog_early":
      ...
      debug_anim_print("dog_combat::handleMeleeFinishAttackNoteTracks() - Settingcombat_attack_player_early");
      self setanimstate("combat_attack_player_early");
      break;

    case "dog_lunge":
      thread set_melee_timer(player);
      debug_anim_print("dog_combat::handleMeleeFinishAttackNoteTracks() - Settingcombat_attack_player_lunge");
...
dog_cant_kill_in_one_hit(player) {
  // right now we want the dogs not to do the singleplayer "instant-kill"return true;
  ...
  if(isDefined(self.enemy)) {
    ...
    if(vectordot(dirToEnemy, forward) > 0) {
      ...
      if((dirToEnemy[0] * forward[1] - dirToEnemy[1] * forward[0]) > 0) {
        debug_anim_print("dog_combat::attackMiss() - Settingcombat_attack_miss_right");
        self setanimstate("combat_attack_miss_right");
      } else {
        debug_anim_print("dog_combat::attackMiss() - Settingcombat_attack_miss_left");

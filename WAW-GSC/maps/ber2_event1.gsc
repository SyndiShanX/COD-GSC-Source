...
event1_smoky_hallway() {
  ...
  while(1) {
    ...
    guy thread ignore_triggers(0.2);
...
event1_knees_execution_watcher_anims(animSpot, notifystring, victim) {
  ...
  if(flag("execution_interrupted")) {
    ...
    self playSound("Ber2_IGD_036A_GER3");
    self waittillmatch("single anim", "end");
  }
...
street_action_tanks() {
  ...
  tank thread tank1_strat();
  tank2 thread tank2_strat();
  tank3 thread tank3_strat();
}
tank1_strat() {
...
tank_move_with_player(goSpeed, accel, decel, ender) {
  ...
  while(isDefined(self)) {
    ...
    for(i = 0; i < players.size; i++) {
      ...
      if(
        (Distance2D(players[i].origin, tankOrgAdjusted) < movedist) || (players[i].origin[0] < self.origin[0])) {
        foundOne = true;
        break;
      }
    }
...
building_collapse_think() {
  ...
  level thread street_executions_playerspeed();
  thread building_collapse_kill_ais();
  thread building_collapse_hit1_fx(pieces);
  maps\_anim::anim_ents(pieces, "hit1", undefined, undefined, animSpot, "buildingcollapse_rig");
  ...
  level thread building_collapse_players_shock();
...
street_execution_waitfor_spawn(timeToSpawnGroundGuys) {
  if(GetTime() < timeToSpawnGroundGuys) {
    wait((timeToSpawnGroundGuys - GetTime()) * .001);
  }
}
...
street_execution_gunshotFX(executioner) {
  ...
  if(isDefined(fxSpot_head) && is_mature()) {
    forward = anglesToForward((executioner GetTagAngles("tag_flash")));
    playFX(level._effect["headshot"], fxSpot_head, forward);
  }
...
subway_gate_bugplayer() {
  ...
  bugLines[0] = "Ber2_IGD_076A_REZN";
  bugLines[1] = "Ber2_IGD_077A_REZN";
  bugLines[2] = "Ber2_IGD_078A_REZN";
  bugLines[3] = "Ber2_IGD_201A_REZN";

 ...
 * Script: maps\_zombiemode_dogs.gsc
**************************************/
...
dog_round_tracker() {
  ...
  next_dog_round = randomintrange(5, 8);
  ...
  while(1) {
    ...
    if(GetDVarInt("force_dogs") > 0) {
      next_dog_round = level.round_number;
    }

    if(level.round_number == next_dog_round) {
      sav_func = level.round_spawn_func;
      dog_round_start();
      level.round_spawn_func = ::dog_round_spawning;

      next_dog_round = level.round_number + randomintrange(4, 6);

      get_players()[0] iprintln("Next dog round: " + next_dog_round);

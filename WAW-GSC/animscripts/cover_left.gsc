...
set_animarray_standing_left() {
  ...
  if(self usingTopLoadingWeapon()) {
    array["reload"] = array(%CornerStndL_topload);
    array["lean_reload"] = % CornerStndL_lean_topload;
  } else {
  ...
  if(self usingBoltActionWeapon()) {
    array["lean_rechamber"] = % CornerStndL_lean_rechamber;
  }
...
set_animarray_crouching_left() {
  ...
  if(self usingTopLoadingWeapon()) {
    array["reload"] = array(%CornerCrL_toploadA, %CornerCrL_toploadB);
  } else {

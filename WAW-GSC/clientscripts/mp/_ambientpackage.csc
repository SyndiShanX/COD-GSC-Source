...
scriptOriginPoolThread() {
  for(;;) {
    ...
    // But because this isn't being done with entities, there's no sound notification, // so on the client side, we'll poll ourselves, to see if we've finished playing, and if so

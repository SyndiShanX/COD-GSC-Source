...
avoidgasclouds(var_0) {
  foreach(var_2 in level.missile_strike_gas_clouds) {
    if(!isDefined(var_2)) {
      continue;
    }
    if(distancesquared(var_0.origin, var_2.origin) < 90000) {
      return 0;
    }
  }
...
avoidenemiesbydistance(var_0) {
  ...
  var_2 = var_2 * var_2;
  var_3 = min(var_0.mindistsquared[var_1], var_2);
  var_4 = var_3 / var_2;
  return var_4 * 100;
}
...
avoidbadspawns(var_0) {
  if(isDefined(self.spawninfo) && isDefined(self.spawninfo.badspawn) && self.spawninfo.badspawn) {
    if(self.spawninfo.spawnpoint == var_0) {
      return 100;
    }

    if(isDefined(self.spawninfo.pastbadspawncount)) {
      for(var_1 = 0; var_1 < self.spawninfo.pastbadspawncount; var_1++) {
        if(self.spawninfo.pastbadspawns[var_1] == var_0) {
          return 100;
        }
      }
    }

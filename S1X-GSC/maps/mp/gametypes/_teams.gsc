...
setPlayerModels() {
  if(level.multiTeamBased) {
    for(i = 0; i < level.teamNameList.size; i++) {
      assert(isDefined(game[level.teamNameList[i]]));
      setTeamModels(level.teamNameList[i], game[level.teamNameList[i]]);
    }
  } else {
...
playerCostume() {
  ...
  prof_begin("playerCostume");
  self SetCostumeModels(self.costume);
  prof_end("playerCostume");

  if(isDefined(self.perks) && !GetDvarInt("virtualLobbyActive", 0)) {
    if(self _hasPerk("specialty_exo_shoulderlauncher")) {
      if(!self _hasPerk("specialty_wildcard_dualtacticals")) {
        self attach("npc_exo_launcher_base_L", "", true);
      }
      if(self _hasPerk("specialty_wildcard_duallethals")) {
        self attach("npc_exo_launcher_base_R", "", true);
      }
    } else {
      if(!self _hasPerk("specialty_wildcard_dualtacticals")) {
        self attach("npc_exo_arm_launcher_L", "J_Elbow_LE", true);
      }
      if(self _hasPerk("specialty_wildcard_duallethals")) {
        self attach("npc_exo_arm_launcher_R", "J_Elbow_RI", true);
      }
    }
  }

  prof_begin("attachCostume");
  self.voice = "american";
  self SetClothType("vestlight");
  prof_end("attachCostume");
  return true;
}

validCostume(costume) {
  if(!isDefined(costume)) {
    return false;
  }

  valid = ValidateCostume(costume);

  for(i = 0; i < costume.size; i++) {
    if(costume[i] != valid[i]) {
      return false;
    }
  }
...
getPracticeRoundCostume() {
  ...
  player_costume = (level.practice_round_costume + player_index) % level.practice_round_max_costumes;

  costume = GetCostumeFromTable(level.practiceRoundCostumeTableName, player_costume + 1);
  return costume;
}
...
getHardcoreCostume() {
  ...
  costume = GetCostumeFromTable(level.hardcoreCostumeTableName, col_index);
  return costume;
}
...
playerModelForWeapon(weapon, secondary) {
  ...
  costumeFound = self playerCostume();
  assert(costumeFound);

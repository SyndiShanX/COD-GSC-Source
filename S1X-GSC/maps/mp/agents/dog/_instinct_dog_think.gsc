 ...
 * Script: maps\mp\agents\dog\_instinct_dog_think.gsc
******************************************************/
...
setupDogState() {
  ...
  self.keepPursuingTargetRadiusSq = 3000 * 3000;
...
UpdateMoveState() {
  ...
  if(self.moveState == "follow") {
    ...
    if(bRefreshGoal || (distFromOwnerSq > self.ownerRadiusSq && bOwnerHasMoved) || self.owner.prevStance != currStance || (self.prevMoveState != "idle" && self.prevMoveState != self.moveState)) {
      PickAgain = true;
      TempGoalPos = self findPointNearOwner();

      trigger = spawn("trigger_radius", self.owner.origin, 0, 256, 256);
      nodesNearPlayer = GetNodesInTrigger(trigger);
      nodesNearPlayer = SortByDistance(nodesNearPlayer, self.owner.origin);
      ...
      while(PickAgain == true) {
        ...
        foreach(dog in level.InstinctDogs) {
          if(PickAgain == false) {
            OtherGoalPos = dog ScrAgentGetGoalPos();
            DistanceBetweenPoints = Distance(TempGoalPos, OtherGoalPos);
            if(DistanceBetweenPoints < 60) {
              PickAgain = true;
            }
          }
        }
        ...
        if(PickAgain == true) {
          if(nodesNearPlayer.size > 0) {
            for(i = 0; i < nodesNearPlayer.size; i++) {
              GoodNode = true;
              foreach(dog in level.InstinctDogs) {
                if(GoodNode == true) {
                  OtherGoalPos = dog ScrAgentGetGoalPos();

                  DistanceBetweenPoints = Distance(nodesNearPlayer[i].origin, OtherGoalPos);
                  DistanceBetweenPointToPlayer = Distance(nodesNearPlayer[i].origin, self.owner.origin);
                  if(DistanceBetweenPoints < 60 || DistanceBetweenPointToPlayer < 128) {
                    GoodNode = false;
                  }
                }
              }
              if(GoodNode == true) {
                self ScrAgentSetGoalPos(nodesNearPlayer[i].origin);
                PickAgain = false;
                break;
              }
            }
          } else {
            empGoalPos = GetRandomNodeDestination(self.owner.origin, self.owner.angles);
            self ScrAgentSetGoalPos(empGoalPos.origin);
            PickAgain = false;
          }
        } else {
          self ScrAgentSetGoalPos(TempGoalPos);
        }

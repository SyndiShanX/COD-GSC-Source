...
setup_bot_dom() {
  ...
  if(flags.size > 3) {
    ...
    foreach(teleport_zone, teleport_zone_flag_set in flags_in_zone) {
      ...
      bot_cache_entrances_to_gametype_array(teleport_zone_flag_set, teleport_zone + "_flag", level.bot_ignore_precalc_paths);
    }
  } else {
    Assert(flags.size == 3);
  ...
  foreach(flag in flags) {
    ...
    bot_add_missing_nodes(flag, flag);
...
bot_wait_for_event_flag_swap(flags) {
  ...
  bot_cache_entrances_to_gametype_array(flags, "flag", level.bot_ignore_precalc_paths, true);
  bot_cache_flag_distances(flags);

  foreach(dom_flag in flags) {
    dom_flag.nodes = GetNodesInTrigger(dom_flag);
    bot_add_missing_nodes(dom_flag, dom_flag);
  }

  foreach(player in level.participants) {
    if(IsAITeamParticipant(player)) {
      player.force_new_goal = true;
    }
  }
}
...
bot_choose_flag() {
  ...
  if(attacking) {
    ...
    if(no_flags_captured_yet && !self has_override_flag_targets()) {
      ...
      if(allies_capturing_closest < self min_num_bots_assaulting_first_flag()) {
        flag_num = 0;
      } else {
        ...
        if(self.strategy_level == 0) {
          ...
          chance_to_take_closest = 40;
          chance_to_take_middle = 40;
          chance_to_take_farthest = 20;
        }
...
defend_flag(flag_to_defend) {
  ...
  if(bot_dom_debug_should_capture_all()) {
    ...
    optional_params["entrance_points_index"] = get_flag_label(flag_to_defend);
    optional_params["nearest_node_to_center"] = flag_to_defend.nearest_node;
...
bot_dom_leader_dialog(dialog, location) {
  if(IsSubStr(dialog, "losing") && dialog != "losing_score" && dialog != "losing_time") {
    flag_script_label = GetSubStr(dialog, dialog.size - 2);
    flag_losing = get_specific_flag_by_label(flag_script_label);
    AssertEx(isDefined(flag_losing), "Flag not found for string '" + dialog + "'");
    if(self bot_allow_to_capture_flag(flag_losing)) {
      ...
      if(!isDefined(self.last_losing_flag_react) || ((GetTime() - self.last_losing_flag_react) > 10000)) {
        if(self bot_is_protecting()) {
          is_close_to_flag = DistanceSquared(self.origin, flag_losing.origin) < 700 * 700;
          was_told_to_protect_flag = bot_is_protecting_flag(flag_losing);
          if(is_close_to_flag || was_told_to_protect_flag) {
    ...
    flag_script_label = GetSubStr(dialog, dialog.size - 2);
    flag_taken = get_specific_flag_by_label(flag_script_label);
          ...
          if(DistanceSquared(self.origin, script_goal) < squared(script_goal_radius)) {
            ...
            if(isDefined(nearest_node_bot)) {
              ...
              foreach(defend_node in flag.nodes) {
                if(!NodesVisible(defend_node, nearest_node_bot, true)) {
...
get_specific_flag_by_letter(flag_letter) {
  Assert(flag_letter == "A" || flag_letter == "B" || flag_letter == "C");
  flag_label = "_" + ToLower(flag_letter);
  get_specific_flag_by_label(flag_label);
}

get_specific_flag_by_label(flag_label) {
  for(i = 0; i < level.flags.size; i++) {
    if(level.flags[i].script_label == flag_label) {
      return level.flags[i];
    }
  }
}
...
bot_should_defend_flag(potential_flag, num_flags_defending) {
  max_num_players_defending_this_flag = get_max_num_defenders_wanted_per_flag(num_flags_defending);
  players_defending_flag = get_players_defending_flag(potential_flag);
  return (players_defending_flag.size < max_num_players_defending_this_flag);
}

get_max_num_defenders_wanted_per_flag(num_flags_defending) {
  team_limit = bot_get_max_players_on_team(self.team);
...
get_players_defending_flag(potential_flag) {
  ...
  foreach(other_player in level.participants) {
    ...
    if(other_player.team == self.team && other_player != self && IsTeamParticipant(other_player)) {
      if(IsAI(other_player)) {
        ...
        time_since_captured = GetTime() - potential_flag.last_time_secured[self.team];
        if(time_since_captured < 10000) {
          continue;
        }

        if(DistanceSquared(potential_flag.origin, other_player.origin) < flag_protect_radius * flag_protect_radius) {
          bots_defending = array_add(bots_defending, other_player);
        }
      }
    }

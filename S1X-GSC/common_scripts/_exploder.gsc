...
setupExploders() {
  ...
  potentialExploders = getEntArray("script_origin", "classname");
  for(i = 0; i < potentialExploders.size; i++) {
    if(isDefined(potentialExploders[i].script_prefab_exploder)) {
      potentialExploders[i].script_exploder = potentialExploders[i].script_prefab_exploder;
    }
...
fire_effect() {
  ...
  if(isDefined(firefxSound)) {
    loop_fx_sound_with_angles(firefxSound, origin, (0, 0, 0), 1, ender);
  }
...
cannon_effect() {
  ...
  if(level.createFX_enabled) {
    SetFXKillOnDelete(self.looper, true);
  }
  if(self.v["delay"] >= 0) {
    TriggerFX(self.looper);
  } else {
    TriggerFX(self.looper, preroll);
  }
  exploder_playSound();
}
...
activate_exploder(num, players, startTime) {
  ...
  if(isDefined(level.createFXexploders) && !level.createFX_enabled) {
    ...
    if(isDefined(exploders)) {
      foreach(ent in exploders) {
        if(!ent check_exploder_platform()) {
          continue;
        }
        ent activate_individual_exploder();
...
deactivate_clientside_exploder(exploderName, players, shouldKill) {
  if(!is_valid_clientside_exploder_name(exploderName)) {
    PrintLn("^1ERROR: Exploder Index '" + exploderName + "' is not a valid exploder index >= 0");
    return;
  }
  ...
  exploder_num = Int(exploderName);
  StopClientExploder(exploder_num, players, shouldKill);
}

...
CONST_soda_pop_time = 0.2;
CONST_soda_count = 12;
CONST_soda_launch_force = 1000;
CONST_soda_random_factor = 0.3;
CONST_soda_splash_dmg_scaler = 3;
...
init() {
  ...
  array_thread(getEntArray("sliding_door", "targetname"), ::sliding_door);
...
metal_detector() {
  ...
  bounds = [];
  bound_x_min = min(bound_org_1.origin[0], bound_org_2.origin[0]);
  bounds[0] = bound_x_min;
  bound_x_max = max(bound_org_1.origin[0], bound_org_2.origin[0]);
  bounds[1] = bound_x_max;
  bound_y_min = min(bound_org_1.origin[1], bound_org_2.origin[1]);
  bounds[2] = bound_y_min;
  bound_y_max = max(bound_org_1.origin[1], bound_org_2.origin[1]);
  bounds[3] = bound_y_max;
  bound_z_min = min(bound_org_1.origin[2], bound_org_2.origin[2]);
  bounds[4] = bound_z_min;
  bound_z_max = max(bound_org_1.origin[2], bound_org_2.origin[2]);
  bounds[5] = bound_z_max;
...
sliding_door() {
  ...
  doors = getEntArray(self.target, "script_linkname");

  origins = [];
  foreach(d in doors) {
    if(d.classname == "script_origin") {
      origins[origins.size] = d;
      continue;
    }
    d door_init(self.open_time);
  }
  doors = array_remove_array(doors, origins);
  ...
  while(1) {
    ...
    touch_ents = array_combine(level.characters, vehicles);

    touching = 0;
    foreach(ent in touch_ents) {
    ...
    if(touching > 0) {
      ...
      close_time = 1;
      thread close_all_doors(doors, close_time);
    }
    wait(0.05);
  }
}
...
door_init(open_time) {
  assert(!isDefined(self.start_position));

  self.start_position = self.origin;
  self.sliding_door_state = "closed";

  assertEx(isDefined(self.target), "mp sliding doors need to have a target ent which is where it should open to");
  ref = getent(self.target, "targetname");
  assert(isDefined(ref));
  assert(!isDefined(self.open_position));

  self.open_position = ref.origin;
  self.open_velocity = Distance(self.open_position, self.origin) / open_time;
}

open_all_doors(door_arr) {
  foreach(door in door_arr) {
    if(door.sliding_door_state == "open" || door.sliding_door_state == "opening") {
      continue;
    }

    door thread open_door();
  }
}
...
open_door() {
  ...
  open_time = Distance(self.origin, self.open_position) / self.open_velocity;
  ...
  assert(isDefined(self.start_position));
  assert(isDefined(self.open_position));
  self MoveTo(self.open_position, open_time);
...
close_all_doors(door_arr, close_time) {
  foreach(door in door_arr) {
    if(door.sliding_door_state == "closed" || door.sliding_door_state == "opening") {
      continue;
    }

    assert(isDefined(door.start_position));
    door MoveTo(door.start_position, close_time);

    self playSound("glass_door_close");

    door.sliding_door_state = "closed";
  }
}
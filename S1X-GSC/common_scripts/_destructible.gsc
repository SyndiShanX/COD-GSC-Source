...
dest_cover(destructibleName, numchunks, chunk_efxname, chunkhealth, damaged_model, chunk_soundalias) {
  ...
  destructible_create(destructibleName, "tag_origin", 1, undefined, 32, "no_melee");
  if(isDefined(damaged_model)) {
    destructible_state(undefined, damaged_model, undefined, undefined, 32, "no_melee");
  }
  for(i = 0; i < numchunks; i++) {
    ...
    if(isDefined(chunk_soundalias)) {
      destructible_sound(chunk_soundalias);
    }
...
destructible_getType(destructibleType) {
  ...
  if(!isDefined(level.destructible_functions[destructibleType])) {
    println("^1WARNING: Destructible object 'destructible_type' " + destructibleType + "' is not valid. Have you Repackaged Zone/Script? Sometimes you need to rebuild BSP ents.");
    return -1;
  }
...
setup_destructibles() {
  ...
  if(IsSubStr(self.destructible_type, "dest_cover")) {
    self thread destructibleCoverWatcher();
  }

  self thread destructible_fx_spawnImmediate();
}
...
destructible_fx_spawn_immediate(tagName, fxName, useTagAngles, damageType, groupNum, fxCost, stateChangeKill) {
  destructible_fx(tagName, fxName, useTagAngles, damageType, groupNum, fxCost, true, stateChangeKill);
}

destructible_fx(tagName, fxName, useTagAngles, damageType, groupNum, fxCost, spawnImmediate, stateChangeKill) {
  ...
  if(!isDefined(spawnImmediate)) {
    spawnImmediate = false;
  }

  if(!isDefined(stateChangeKill)) {
    stateChangeKill = 0;
  }
  ...
  level.destructible_type[destructibleIndex].parts[partIndex][stateIndex].v["spawn_immediate"][groupNum][fx_size] = spawnImmediate;
  level.destructible_type[destructibleIndex].parts[partIndex][stateIndex].v["state_change_kill"][groupNum][fx_size] = stateChangeKill;
}
...
destructible_explode(force_min, force_max, rangeSP, rangeMP, mindamage, maxdamage, continueDamage, originOffset, earthQuakeScale, earthQuakeRadius, originOffset3d, delaytime, angularImpulse_min, angularImpulse_max) {
  destructibleIndex = (level.destructible_type.size - 1);
  partIndex = (level.destructible_type[destructibleIndex].parts.size - 1);
  stateIndex = (level.destructible_type[destructibleIndex].parts[partIndex].size - 1);
  ...
  level.destructible_type[destructibleIndex].parts[partIndex][stateIndex].v["explode_angularImpulse_min"] = angularImpulse_min;
  level.destructible_type[destructibleIndex].parts[partIndex][stateIndex].v["explode_angularImpulse_max"] = angularImpulse_max;
}
...
destructible_think() {
  ...
  for(;;) {
    ...
    if(type == "splash" || type == "energy") {
      ...
      if((baseModelName == self.model) && isDefined(self.script_dest_cover_dmg_model)) {
...
destructible_update_part(damage, modelName, tagName, point, direction_vec, attacker, damageType, partInfo) {
  ...
  for(;;) {
    ...
    if(partIndex == 0 && getDamageType(damageType) != "splash") {
      for(i = 0; i < level.destructible_type[self.destructibleInfo].parts.size; i++) {
        part = level.destructible_type[self.destructibleInfo].parts[i];
        ...
        partStateIndex = 0;
        if(isDefined(self.destructible_parts[i].v["currentState"])) {
          partStateIndex = self.destructible_parts[i].v["currentState"];
        }

        if(!isDefined(part[partStateIndex])) {
          continue;
        }

        if(!isDefined(part[partStateIndex].v["tagName"])) {
          continue;
        }

        childTagName = part[partStateIndex].v["tagName"];
    ...
    self notify("FX_State_Change_Kill" + partIndex);
    ...
    if(isDefined(level.destructible_type[self.destructibleInfo].parts[partIndex][actionStateIndex].v["explode"])) {
      ...
      angularImpulse_min = level.destructible_type[self.destructibleInfo].parts[partIndex][actionStateIndex].v["explode_angularImpulse_min"];
      angularImpulse_max = level.destructible_type[self.destructibleInfo].parts[partIndex][actionStateIndex].v["explode_angularImpulse_max"];
      ...
      self thread explode(partIndex, force_min, force_max, range, mindamage, maxdamage, continueDamage, originOffset, earthQuakeScale, earthQuakeRadius, attacker, originOffset3d, delaytime, angularImpulse_min, angularImpulse_max);
    }
...
isValidDamageCause(partIndex, stateIndex, damageType) {
  ...
  if((validType == "bullet") && damageType != "bullet") {
    return false;
  }
...
physics_launch(partIndex, stateIndex, point, initial_velocity) {
  physicsObject = self physics_object_create(partIndex, stateIndex);

  physicsObject PhysicsLaunchClient(point, initial_velocity);
}

physics_launch_with_impulse(partIndex, stateIndex, linearImpulse, angularImpulse) {
  physicsObject = self physics_object_create(partIndex, stateIndex);

  physicsObject PhysicsLaunchClientWithImpulse(linearImpulse, angularImpulse);
}

physics_object_create(partIndex, stateIndex) {
...
explode(partIndex, force_min, force_max, range, mindamage, maxdamage, continueDamage, originOffset, earthQuakeScale, earthQuakeRadius, attacker, originOffset3d, delaytime, angularImpulse_min, angularImpulse_max) {
  ...
  if(isDefined(level.destructible_type[self.destructibleInfo].parts)) {
    for(i = (level.destructible_type[self.destructibleInfo].parts.size - 1); i >= 0; i--) {
      ...
      currentHealth = 0;
      if(isDefined(self.destructible_parts[i].v["health"])) {
        currentHealth = self.destructible_parts[i].v["health"];
      }

      defaultHealth = 0;
      if(isDefined(level.destructible_type[self.destructibleInfo].parts[i][stateIndex].v["health"])) {
        defaultHealth = level.destructible_type[self.destructibleInfo].parts[i][stateIndex].v["health"];
      }

      if(defaultHealth > 0 && currentHealth <= 0) {
        continue;
      }
      ...
      if(isDefined(level.destructible_type[self.destructibleInfo].parts[i][0].v["physicsOnExplosion"])) {
        if(level.destructible_type[self.destructibleInfo].parts[i][0].v["physicsOnExplosion"] > 0) {
          ...
          if(isDefined(angularImpulse_min) && isDefined(angularImpulse_max)) {
            angularImpulse = randomvectorrange(angularImpulse_min, angularImpulse_max);

            self thread physics_launch_with_impulse(i, stateIndex, initial_velocity, angularImpulse);
          } else {
  ...
  Assert(magnitude > 0);
  range *= .99;
  PhysicsExplosionSphere(explosionOrigin, range, 0, magnitude);
...
destructible_fx_kill_state_wait(part_indx) {
  final_global_state_index = level.destructible_type[self.destructibleInfo].parts[0].size - 1;

  self endon(("FX_State_Change_Kill" + part_indx));

  for(;;) {
    curr_global_state_index = -1;
    if(isDefined(self.destructible_parts[0].v["currentState"])) {
      curr_global_state_index = self.destructible_parts[0].v["currentState"];
    }
    if(curr_global_state_index == final_global_state_index) return 0;
    waitframe();
  }
}

destructible_fx_spawn_think(eModel, fxid, fx_tag, i, usetagangles, stateChangeKill) {
  waittillframeend;

  if(!isDefined(stateChangeKill)) stateChangeKill = 0;
  fxObj = undefined;
  fx_spawn_obj = undefined;
  if(isDefined(fx_tag)) {
    if(usetagangles) {
      playFXOnTag(fxid, eModel, fx_tag);
      wait(.05);
      if(stateChangeKill == 1 || stateChangeKill == 2) {
        self destructible_fx_kill_state_wait(i);
        if(stateChangeKill == 1) stopFXOnTag(fxid, eModel, fx_tag);
        else KillFXOnTag(fxid, eModel, fx_tag);
      }
    } else {
      fxOrigin = eModel GetTagOrigin(fx_tag);
      forward = (0, 0, 100);
      if(stateChangeKill == 1 || stateChangeKill == 2) {
        fx_spawn_obj = SpawnFx(fxid, fxOrigin, forward);
        fxObj = TriggerFX(fx_spawn_obj, 0.01);
      } else fxObj = playFX(fxid, fxOrigin, forward);
      wait(.05);
      if(stateChangeKill == 1 || stateChangeKill == 2) {
        self destructible_fx_kill_state_wait(i);
        if(stateChangeKill == 1) {
          fx_spawn_obj Delete();
        } else if(stateChangeKill == 2) {
          SetFXKillOnDelete(fx_spawn_obj, true);
          wait(.05);
          fx_spawn_obj delete();
        }
      }
    ...
    for(j = 0; j < level.destructible_type[self.destructibleInfo].parts[i].size; j++) {
      ...
      if(isDefined(part.v["fx_filename"])) {
        for(g = 0; g < part.v["fx_filename"].size; g++) {
          fx_filenames = part.v["fx_filename"][g];
          fx_tags = part.v["fx_tag"][g];
          fx_spawn_immediate_array = part.v["spawn_immediate"][g];

          if(isDefined(fx_filenames) && isDefined(fx_spawn_immediate_array)) {
            assert(isDefined(fx_tags));

            for(idx = 0; idx < fx_filenames.size; idx++) {
              if(fx_spawn_immediate_array[idx] == true) {
                stateChangeKill = part.v["state_change_kill"][g][idx];
                fxid = level.destructible_type[self.destructibleInfo].parts[i][j].v["fx"][g][idx];
                fx_tag = fx_tags[idx];
                fx_filename = fx_filenames[idx];
                usetagangles = level.destructible_type[self.destructibleInfo].parts[i][j].v["fx_useTagAngles"][g][idx];
                self thread destructible_fx_spawn_think(eModel, fxid, fx_tag, i, usetagangles, stateChangeKill);
              }
            }
          }
        }
...
destructible_fx_think(action_v, eModel, damageType, partIndex, groupNum) {
  ...
  for(idx = 0; idx < fx_size; idx++) {
    ...
    if(action_v["spawn_immediate"][groupNum][idx] == true) {
      continue;
    }

    fx = action_v["fx"][groupNum][idx];

    stateChangeKill = action_v["state_change_kill"][groupNum][idx];
    ...
    if(isDefined(action_v["fx_tag"][groupNum][idx])) {
      ...
      if(action_v["fx_useTagAngles"][groupNum][idx]) {
        self thread destructible_fx_spawn_think(eModel, fx, fx_tag, partIndex, true, stateChangeKill);
      } else {
        self thread destructible_fx_spawn_think(eModel, fx, fx_tag, partIndex, false, stateChangeKill);
      }
    } else {
      self thread destructible_fx_spawn_think(eModel, fx, undefined, partIndex, false, stateChangeKill);
    }
  }
...
doBuildDOT_damage(idx, trigger, vars) {
  ...
  self thread[[level.callbackPlayerDamage]](
    trigger, trigger, maxDamage, damageFlag, meansOfDeath, weapon, trigger.origin, (0, 0, 0) - trigger.origin, "none", 0
  );
}
...
doDOT_poisonDamage(trigger, damage) {
  if(isSP()) {} else {
    self thread[[level.callbackPlayerDamage]](
      trigger, trigger, damage, 0, "MOD_SUICIDE", "claymore_mp", trigger.origin, (0, 0, 0) - trigger.origin, "none", 0
    );
  }
}
...
destructibleCoverWatcher() {
  ...
  while(isDefined(self)) {
    if(isDefined(self.destructible_parts)) {
      chunkDestroyedCounter = 0;
      for(i = 1; i < self.destructible_parts.size; i++) {
        if(self.destructible_parts[i].v["currentState"] == 1) {
          chunkDestroyedCounter++;
        }
      }
      if(chunkDestroyedCounter == self.destructible_parts.size - 1) {
        break;
      }
    }

    dist = DistanceSquared(level.player.origin, self.origin);
    if(dist > self.script_dest_cover_dmg_dist * self.script_dest_cover_dmg_dist) {
      self setCanDamage(false);
    } else {
      self setCanDamage(true);
    }

...
_id_97AD() {
  level endon("game_ended");
  self.active = 0;
  self._id_13EFC = scripts\engine\utility::getStruct(self.target, "targetname");

  for(var_0 = scripts\engine\utility::getStruct(self._id_13EFC.target, "targetname"); isDefined(var_0.target); var_0 = scripts\engine\utility::getStruct(var_0.target, "targetname")) {}
  ...
  for(;;) {
    ...
    if(var_4 == self._id_164A.basename) {
      activate_zipline();
      break;
    }
  }
...
activate_zipline() {
  ...
  if(isDefined(level.zipline_negotiation)) {
    var_0 = scripts\engine\utility::drop_to_ground(self._id_13EFC.origin, 0, -512);
    var_1 = scripts\engine\utility::drop_to_ground(self._id_13EFB.origin, 0, -512);
...
player_zipline(var_0, var_1) {
  ...
  var_0 setvelocity(var_0._id_13EFD);

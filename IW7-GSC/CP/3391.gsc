...
_id_9593() {
  ...
  self.item = spawn("script_model", var_1.origin);
  self.item.angles = var_1.angles;
  var_2 = _id_7CF9(self.script_noteworthy);
  self.item setModel(var_2);
  var_3 = _id_7CFB(var_2);
  self.item.origin = self.item.origin + var_3;
  var_4 = _id_7CFA(self, var_2);
  self.item.angles = self.item.angles + var_4;
  self.item.hint_string = _id_7CF8(self, var_2);
  var_5 = getEntArray(self.target, "targetname");

  if(var_5.size > 0) {
    self.light = scripts\engine\utility::getclosest(self.origin, var_5);
    thread _id_12988();
  }
}
...
_id_3E93() {
  ...
  for(;;) {
    ...
    if(var_2.script_noteworthy == "small_ticket_prize") {
      ...
      if(isDefined(var_2.item)) {
        var_3 = _id_7CF9(var_2.script_noteworthy);
        var_2.item.hint_string = _id_7CF8(var_2, var_3);
      }
...
_id_12988() {
  ...
  for(var_0 = 0; var_0 < 4; var_0++) {
    self.light setlightintensity(65);
    wait(randomfloat(1));
...
turn_on_light() {
  if(isDefined(self.light)) {
    for(;;) {
      var_0 = level scripts\engine\utility::waittill_any_return_no_endon_death("power_on", "power_off");
...
_id_10149(var_0, var_1, var_2) {
  ...
  if(isDefined(var_0.item)) {
    playsoundatpos(var_0.item.origin, "zmb_prize_restock");
    playFX(level._effect["booth_respawn"], var_0.item.origin);
  } else {

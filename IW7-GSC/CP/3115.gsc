...
_id_98E5(var_0) {
  if(isDefined(self.spawner) && self.spawner _id_10863()) {
...
_id_3E29(var_0) {
  ...
  if(!scripts\mp\agents\zombie\zombie_util::_id_8B76()) {
    return anim.failure;
  }

  if(scripts\mp\agents\zombie\zombie_util::_id_138E7()) {
    return anim.failure;
  }

  if(scripts\mp\agents\zombie\zombie_util::_id_54BF()) {
    return anim.failure;
  }

  var_1 = scripts\engine\utility::is_true(self._id_B104) && isDefined(self._id_B100) && gettime() - self._id_B100 <= self._id_B0FE;

  if(!ispointonnavmesh(self.curmeleetarget.origin, self) && !scripts\asm\asm_bb::bb_moverequested()) {
    if(!scripts\mp\agents\zombie\zombie_util::_id_DD7C("offmesh")) {
      return anim.failure;
    }
  } else if(scripts\mp\agents\zombie\zombie_util::_id_54BE() || var_1) {
    if(!scripts\mp\agents\zombie\zombie_util::_id_DD7C("base")) {
      return anim.failure;
    }
  } else if(!scripts\mp\agents\zombie\zombie_util::_id_DD7C("normal"))
    return anim.failure;
...
_id_3E4F(var_0) {
  ...
  if(!_id_13D9D(self.curmeleetarget)) {
    return anim.failure;
  }
...
chaseenemy(var_0) {
  ...
  var_7 = scripts\mp\agents\zombie\zombie_util::_id_7FAA(var_1);

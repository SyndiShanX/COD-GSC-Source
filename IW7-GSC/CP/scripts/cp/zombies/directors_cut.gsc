...
try_play_lost_reel_vfx_on_machine() {
  if(scripts\engine\utility::is_true(level.lost_reel_vfx_on_machine)) {
    return;
  }
...
directors_cut_is_activated() {
  return level.directors_cut_is_activated;
}
...
disco_talisman_damage_monitor(var_0) {
  ...
  for(;;) {
    ...
    if(isPlayer(var_2)) {
      ...
      if(directors_cut_activated_for(var_11) && !scripts\engine\utility::is_true(var_11.got_disco_talisman)) {
...
try_abandoned_shooting_gallery(var_0, var_1) {
  ...
  if(scripts\engine\utility::is_true(var_1.played_abandoned_shooting_gallery)) {
    return;
  }

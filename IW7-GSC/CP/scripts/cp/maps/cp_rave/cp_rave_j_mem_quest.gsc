...
do_find_thing_1() {
  while(!scripts\engine\utility::is_true(level.met_kev)) {
    wait 0.1;
  }
...
startspeakereventspawning(var_0) {
  ...
  if(isDefined(var_2)) {
    ...
    foreach(var_16 in var_1) {
      ...
      var_16 scripts\cp\zombies\zombies_spawning::make_volume_inactive();
    }
  }
  ...
  foreach(var_19 in var_1) {
    var_19 scripts\cp\zombies\zombies_spawning::make_volume_active();
  }
}
...
play_ambient_kevin_smith_vo_jay_memory() {
  ...
  for(;;) {
    if(scripts\engine\utility::is_true(level.j_mem["j_mem_1"])) {
    ...
    } else if(scripts\engine\utility::is_true(level.j_mem["j_mem_2"])) {

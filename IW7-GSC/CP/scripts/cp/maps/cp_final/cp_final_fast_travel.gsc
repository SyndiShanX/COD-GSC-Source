...
activate_room_after_portal_use(var_0) {
  switch (var_0) {
    case "theater":
      ...
      if(!scripts\engine\utility::is_true(level.theater_open)) {
...
blast_doors_with_gun() {
  ...
  while(!scripts\engine\utility::is_true(level.portal_gun_init_done)) {
    wait 0.1;
  }
  ...
  var_19 = scripts\engine\utility::getclosest(var_3[0].origin, var_18, 1000);
  var_19 delete();
...
move_zombie_through_portal_tube(var_0) {
  if(scripts\engine\utility::is_true(var_0._id_11B2F)) {
    return;
  }
  var_0.isfasttravelling = 1;
...
teleport_ai_to_portal_safe_spot(var_0) {
  ...
  while(!isDefined(var_2)) {
    foreach(var_3 in var_1) {
      if(!scripts\engine\utility::is_true(var_3.in_use)) {
  ...
  if(isDefined(var_3) && scripts\engine\utility::is_true(var_3.in_use)) {
...
portal_cooldown_monitor() {
  ...
  while(!scripts\engine\utility::is_true(self.end_point.opened)) {
    wait 0.1;
  }
...
pappenaltyspawn() {
  if(scripts\engine\utility::is_true(level.spawned_phantom)) {
    if(randomint(100) < 25) {
      ...
      if(!positionwouldtelefrag(var_1)) {
        ...
        if(!isDefined(var_4)) {
          thread scripts\cp\maps\cp_final\cp_final_mpq::trigger_goon_event_single(var_2.position);
        }
      }
    } else

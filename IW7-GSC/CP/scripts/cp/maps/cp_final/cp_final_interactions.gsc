...
level_specific_wait_for_interaction_triggered(var_0) {
  ...
  for(;;) {
    ...
    if(!scripts\engine\utility::is_true(var_0.dontdelaytrigger)) {
      var_0.triggered = 1;
    ...
    if(isDefined(level.interactions[var_0.script_noteworthy].can_use_override_func)) {
      ...
      if(scripts\engine\utility::is_true(var_0.player_opened))
        var_2 = 0;
    }
...
registeratminteractions() {
  ...
  levelinteractionregistration(0, "atm_deposit", "atm", undefined, undefined, ::atm_deposit, 1000, 0, undefined);
...
use_sasquatch_func(var_0, var_1) {
  if(scripts\engine\utility::flag("fuses_charged") && !scripts\engine\utility::flag("picked_up_charged_fuses")) {
  ...
  } else if(scripts\engine\utility::flag("picked_up_uncharged_fuses") && !scripts\engine\utility::flag("placed_uncharged_fuses")) {
    foreach(var_4 in level.players)
    ...
    var_10.targetname = "fuses_to_power";
...
slidingdoorhint(var_0, var_1) {
  if(scripts\engine\utility::is_true(var_0.requires_power))
    return &"CP_FINAL_INTERACTIONS_SYSTEM_OFFLINE";
  else if(scripts\engine\utility::is_true(var_0.player_opened)) {
...
openslidingdoor(var_0, var_1) {
  ...
  while(scripts\engine\utility::is_true(var_0._id_42AF))
...
closeslidingdoor(var_0, var_1) {
  ...
  while(scripts\engine\utility::is_true(var_0._id_C62C))
    scripts\engine\utility::waitframe();
  ...
  if(isDefined(var_6)) {
    ...
    var_16 = scripts\engine\utility::get_array_of_closest(var_0.origin, level.characters, undefined, undefined, 40, 0);

    foreach(var_18 in var_16) {
      if(vectordot(vectorNormalize(var_6.origin - var_0.origin), var_15) > 0.75) {
        if(distance(var_0.origin, var_6.origin) > distance(var_18.origin, var_0.origin) + 12) {
          var_19 = scripts\engine\utility::getclosest(var_18.origin, [var_6, var_0]);

          if(isPlayer(var_18)) {
            if(!var_18 scripts\cp\utility::is_valid_player()) {
              var_18 setvelocity(var_19.origin);
              continue;
            }
          } else if(var_18 scripts\cp\utility::agentisinstakillimmune())
            continue;
          else {
            var_18.died_poorly = 1;
            var_18.died_poorly_health = var_18.health;
          }

          var_18 setvelocity(var_19.origin);
          var_18 dodamage(var_18.maxhealth, var_6.origin, undefined, undefined, "MOD_UNKNOWN", "iw7_zombieDoors_zm");
        }
      }
    }
  }

  if(!scripts\engine\utility::is_true(var_0.nointeraction))

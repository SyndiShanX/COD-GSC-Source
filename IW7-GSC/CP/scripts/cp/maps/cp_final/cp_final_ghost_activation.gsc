  ...
  for(;;) {
    foreach(var_1 in level.players) {
      if(distance2dsquared(var_1.origin, (5734, -6217, 120)) < 10000) {
        if(!scripts\engine\utility::is_true(var_1.secret_button_combo_watcher_active)) {
...
set_up_secret_button_notify(var_0) {
  if(scripts\engine\utility::is_true(var_0.set_up_secret_button_notify)) {
    return;
  }
...
wait_for_player_activation() {
  ...
  for(;;) {
    ...
    foreach(var_4 in level.players) {
      if(scripts\engine\utility::is_true(var_4.inlaststand)) {
        var_2 = 0;
        break;
      }

      if(scripts\engine\utility::is_true(var_4.iscarrying)) {
        var_2 = 0;
        break;
      }

      if(scripts\engine\utility::is_true(var_4.isusingsupercard)) {
        var_2 = 0;
        break;
      }
...
register_waves_movement() {
  scripts\cp\maps\cp_zmb\cp_zmb_ghost_wave::register_moving_target_wave(1, 2.0, 4.0, 0.5);
  scripts\cp\maps\cp_zmb\cp_zmb_ghost_wave::register_moving_target_wave(2, 2.0, 4.0, 0.5);
  scripts\cp\maps\cp_zmb\cp_zmb_ghost_wave::register_moving_target_wave(3, 2.0, 4.3, 0.5);
...
explode_combo_group(var_0, var_1) {
  ...
  var_2 = get_combo_group_index_for(var_0);
  var_3 = get_moving_targets_in_combo(var_0);
  delete_combo_group_at(var_2);
  var_4 = 0;
  ...
  if(var_4 >= 10 && !scripts\engine\utility::is_true(var_1.combo_achievement_awarded)) {

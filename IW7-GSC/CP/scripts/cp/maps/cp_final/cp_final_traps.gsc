...
create_laser_beam_fx(var_0, var_1, var_2) {
  ...
  if(var_5 > 0.5) {
    ...
    var_7 = playFXOnTag(scripts\engine\utility::getfx("vfx_zb_laser_lens"), var_4, "tag_origin");
    ...
    if(isPlayer(var_4)) {
      if(var_4 scripts\cp\utility::is_valid_player()) {
        if(!isDefined(var_4.padding_damage)) {
          ...
          if(!scripts\engine\utility::is_true(var_3) && (var_5 < var_7 || var_6 > var_7)) {
            scripts\engine\utility::waitframe();
            continue;
          }
...
kill_fx_on_death(var_0, var_1) {
  ...
  var_2.targetname = "laser_beam_kill_effect";
...
watch_for_obtain_helmet() {
  ...
  for(;;) {
    ...
    if(scripts\engine\utility::is_true(var_2.is_off_grid)) {
      continue;
    }
    ...
    if(scripts\engine\utility::flag("pulled_out_helmet")) {
      if(!scripts\engine\utility::is_true(level.brute_helm_out_of_bounds)) {
        continue;
      }
...
kill_zombies(var_0) {
  self endon("stop_dmg");
  self._id_2B30.fx = spawnfx(level._effect["blackhole_suction"], self._id_2B30.origin, anglesToForward(self._id_2B30.angles), anglestoup(self._id_2B30.angles));
  triggerfx(self._id_2B30.fx);
...
fridge_lure_enemies() {
  ...
  for(;;) {
    ...
    foreach(var_3 in var_1) {
      ...
      if(scripts\engine\utility::is_true(var_3.fridge_trap_marked)) {
        continue;
      }
...
electric_trap_damage(var_0, var_1) {
  ...
  for(;;) {
    ...
    if(scripts\engine\utility::is_true(var_4.is_turned) || !cp_final_should_be_affected_by_trap(var_4, 0, 1)) {
      continue;
    }
    if(scripts\engine\utility::is_true(var_4.electrocuted)) {
      continue;
    }
...
use_rain_trap(var_0, var_1) {
  ...
  foreach(var_5 in var_3) {
    scripts\cp\cp_interaction::add_to_current_interaction_list(var_5);
    level thread scripts\cp\cp_interaction::interaction_cooldown(var_5, var_17);
  }
...
rain_trap_damage(var_0, var_1) {
  ...
  for(;;) {
    ...
    if(scripts\engine\utility::is_true(var_4.applyacidraindamage)) {
      continue;
    }
    if(isPlayer(var_4)) {

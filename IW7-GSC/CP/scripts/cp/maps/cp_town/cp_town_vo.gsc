...
one_to_one_dialogue_vo_handler(var_0, var_1, var_2, var_3, var_4, var_5, var_6) {
  ...
  foreach(var_12 in var_10) {
    ...
    if(isDefined(var_14) && var_7.vo_prefix == var_14 || var_13 || getdvarint("scr_solo_dialogue", 0) == 1) {
      var_16 = scripts\cp\cp_vo::create_vo_data(var_15, var_3, var_5, var_6);
      var_7 scripts\cp\cp_vo::set_vo_system_playing(1);
      var_7 scripts\cp\cp_vo::set_vo_currently_playing(var_16);

      if(isDefined(var_16.alias) && scripts\engine\utility::is_true(var_9)) {
        if(isDefined(level.elvira)) {
          scripts\engine\utility::play_sound_in_space(var_16.alias, level.elvira.origin, 0, level.elvira);
          var_17 = scripts\cp\cp_vo::get_sound_length(var_16.alias);
          var_18 = scripts\engine\utility::getStruct("elvira_talk", "script_noteworthy");
          var_18 thread scripts\cp\maps\cp_town\cp_town_elvira::elvira_talks(var_17, 1);
          wait(scripts\cp\cp_vo::get_sound_length(var_16.alias));
        } else
          var_7 scripts\cp\cp_vo::play_vo(var_16);
      } else
        var_7 scripts\cp\cp_vo::play_vo(var_16);

      var_7 scripts\cp\cp_vo::pause_between_vo(var_16);

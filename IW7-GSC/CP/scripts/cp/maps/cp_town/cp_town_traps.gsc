...
freeze_players() {
  ...
  for(;;) {
    ...
    if(isPlayer(var_0)) {
      if(!scripts\engine\utility::is_true(var_0.padding_damage)) {
...
pool_dmg_players(var_0) {
  ...
  for(;;) {
    ...
    if(scripts\engine\utility::is_true(var_3.is_turned) || !scripts\cp\utility::should_be_affected_by_trap(var_3, 0, 1)) {
      continue;
    }
    if(var_3 is_crog()) {
      continue;
...
elvira_trap_dmg(var_0) {
  ...
  for(;;) {
    ...
    if(scripts\engine\utility::is_true(var_2.is_turned) || !scripts\cp\utility::should_be_affected_by_trap(var_2, 0, 1)) {
      continue;
    }
    if(var_2 is_crog()) {
      continue;

...
use_crafting_station(var_0, var_1) {
  ...
  if(!scripts\engine\utility::is_true(var_0.blueprint_added) && !isDefined(var_1.has_blueprint)) {
    var_1 iprintlnbold("Need to add a blueprint first");
  ...
  if(scripts\engine\utility::is_true(var_0.blueprint_added) && isDefined(var_1.crafting_piece)) {
    if(is_valid_crafting_piece(var_1, var_0)) {
      ...
      var_1 iprintlnbold("Wrong part for this blueprint!");
      return;
    }
  }
}
  ...
  if(isDefined(level.chem_pieces) && level.chem_pieces.size > var_0.parts_added) {
    ...
    if(var_0.parts_added == 3) {
      ...
      foreach(var_10 in level.chemical_compounds_created) {
        playFX(level._effect["generic_pickup"], var_10.origin);
        var_10.model show();
      }
...
show_chem_parts_based_on_found_piece(var_0) {
  foreach(var_2 in level.chemistry_set_parts) {
    ...
    if(issubstr(var_2.targetname, var_0)) {
...
pickup_crafting_piece(var_0, var_1) {
  ...
  playFX(level._effect["generic_pickup"], var_0.item.origin);
  var_0.item hide();
  var_0.item.hidden = 1;
  scripts\cp\cp_interaction::remove_from_current_interaction_list(var_0);
...
reset_crafting_piece_on_disconnect_or_bad_part() {
  ...
  var_1 = scripts\engine\utility::waittill_any_return_no_endon_death("disconnect", "bad_part");
  var_0.item show();
...
reset_blueprint_on_disconnect() {
  ...
  var_1 = scripts\engine\utility::waittill_any_return_no_endon_death("disconnect");

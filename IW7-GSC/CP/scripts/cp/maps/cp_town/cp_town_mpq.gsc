...
set_backstory_interaction_active_for_players() {
  foreach(var_1 in level.players) {
    if(scripts\engine\utility::is_true(var_1.played_backstory_vo)) {
...
launchcode_step_3() {
  level thread scripts\cp\maps\cp_town\cp_town_crab_boss_fight::crab_boss_gets_to_combat_pos();
...
garage_door_activation_func(var_0, var_1) {
  ...
  if(scripts\engine\utility::is_true(level.garage_door_open)) {
    return;
  }
  ...
  if(!isDefined(level.punch_card_added)) {
    ...
    if(!isDefined(level.terminal_unlocked)) {
      ...
      if(var_5) {
        ...
        level.body_made = 1;
        var_9 = scripts\engine\utility::random(level.players);

        if(isDefined(var_9.vo_prefix)) {
          switch (var_9.vo_prefix) {
            case "p1_":
...
input_wrong_digit(var_0, var_1) {
  ...
  if(var_1.times_input_wrong_digits == 3) {
    ...
    var_1 thread scripts\cp\cp_vo::try_to_play_vo("boss_phase_6_fail", "town_comment_vo");
...
player_complete_nuclear_code(var_0, var_1) {
  ...
  var_1 thread scripts\cp\cp_vo::try_to_play_vo("boss_phase_6_success", "town_comment_vo");
...
phase3_watch_gauge_trigs() {
  ...
  for(;;) {
    ...
    if(isDefined(var_1) && isPlayer(var_1) && var_4 == "MOD_MELEE" && var_1.currentmeleeweapon == "iw7_knife_zm_crowbar") {
      ...
      if(scripts\engine\utility::is_true(self.gauge.stuck)) {
...
phase3_place_bomb_parts(var_0, var_1) {
  ...
  if(scripts\engine\utility::flag("teleporter_charged")) {
    ...
    var_1 thread scripts\cp\cp_vo::try_to_play_vo("key_phase_3_success_bomb_teleport", "town_comment_vo");
    var_1 scripts\cp\cp_interaction::remove_from_current_interaction_list(var_0);
...
phase3_charge_bomb_teleporter(var_0) {
  ...
  if(!scripts\engine\utility::is_true(var_0.played_charging_vo)) {

...
backstory_activation(var_0, var_1) {
  ...
  if(scripts\engine\utility::is_true(var_1.playing_backstory)) {
    return;
  }
  ...
  if(var_2 != "" && !scripts\engine\utility::array_contains(var_1.completed_aliases_for_backstory_achievement, var_2)) {
    ...
    if(!scripts\engine\utility::is_true(level.defeated_crogboss)) {
...
registeratminteractions() {
  ...
  town_register_interaction(0, "atm_deposit", "atm", undefined, undefined, ::atm_deposit, 1000, 1, undefined);
...
town_wait_for_interaction_triggered(var_0) {
  ...
  for(;;) {
    ...
    if(isDefined(level.interactions[var_0.script_noteworthy].can_use_override_func)) {
      ...
      if(scripts\engine\utility::is_true(var_1.has_zis_soul_key) || scripts\engine\utility::is_true(level.placed_alien_fuses)) {
        if(var_4 == 3) {
      ...
      } else if(var_4 == level.pap_max && (!scripts\engine\utility::is_true(level.has_picked_up_fuses) || !isDefined(level.placed_alien_fuses))) {
        scripts\cp\cp_interaction::interaction_show_fail_reason(var_0, &"COOP_INTERACTIONS_UPGRADE_MAXED");
        wait 0.1;
        continue;
      ...
      if(!scripts\engine\utility::is_true(level.town_gns_complete)) {
...
can_upgrade(var_0, var_1) {
  ...
  if(scripts\engine\utility::is_true(self.has_zis_soul_key) || scripts\engine\utility::is_true(level.placed_alien_fuses)) {
...
pap_anomaly_logic() {
  while(!scripts\engine\utility::is_true(level.anomaly_revealed)) {
    wait 1;
  }
...
usegeneratorfieldcenter(var_0, var_1) {
  ...
  } else if(scripts\engine\utility::flag("picked_up_uncharged_fuses") && scripts\engine\utility::flag("vial_filled")) {
    foreach(var_7 in level.players) {
...
usepapanomaly(var_0, var_1) {
  ...
  if(scripts\engine\utility::is_true(var_1.playing_game)) {
    return;
  }
  if(scripts\engine\utility::is_true(var_0.hidden)) {
    return;
  }
  if(!scripts\engine\utility::is_true(var_0.revealed)) {
    return;
  }
  if(scripts\engine\utility::is_true(var_0.teleporter_active)) {
...
papanomalyhint(var_0, var_1) {
  ...
  } else if(scripts\engine\utility::is_true(var_0.cooling_down)) {
...
addcutieattachment(var_0, var_1) {
  if(var_1 scripts\cp\cp_weapon::has_weapon_variation("iw7_cutie_zm")) {
    ...
    var_1 scripts\cp\cp_interaction::interaction_show_fail_reason(var_0, &"COOP_PILLAGE_CANT_USE");
}
...
useplungerammo(var_0, var_1) {
  ...
  if(issubstr(var_2, "iw7_cutie_zm")) {
...
runcutielogic(var_0, var_1) {
  ...
  if(var_3 || var_2) {
    ...
    for(;;) {
      ...
      if(var_0 useButtonPressed() || scripts\engine\utility::is_true(var_0.playingperkgesture)) {
        continue;
      }
      if(scripts\engine\utility::is_true(var_0.fired_fov_beam)) {
        continue;
      }
      if(var_3 && !scripts\engine\utility::is_true(var_0.disablecrank)) {
        var_4 = 0;
        ...
        if(var_0 getweaponammoclip(var_1) >= 5) {
          while(!scripts\engine\utility::is_true(var_0.disablecrank) && var_0 adsButtonPressed(1) && var_0 scripts\cp\utility::getvalidtakeweapon() == var_1) {
            var_5 = 1;
...
runcutieplungergesture(var_0, var_1, var_2) {
  if(!scripts\engine\utility::is_true(var_0.disableplunger)) {
    level endon("game_ended");
    var_0 endon("disconnect");
...
fast_travel_hint(var_0, var_1) {
  ...
  if(!scripts\engine\utility::is_true(var_0.activated)) {
...
fast_travel_use(var_0, var_1) {
  ...
  if(scripts\engine\utility::flag("fast_travel_powered") && !scripts\engine\utility::is_true(var_0.activated)) {
    var_0.activated = 1;
  ...
  if(!scripts\engine\utility::flag("fast_travel_ready")) {
    foreach(var_4 in level.fast_travel_devices) {
      if(!scripts\engine\utility::is_true(var_4.activated)) {
        return;
      }
    }

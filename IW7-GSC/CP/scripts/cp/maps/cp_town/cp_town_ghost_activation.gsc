...
watch_radiation_extractor_ticks() {
  ...
  for(;;) {
    ...
    if(scripts\engine\utility::is_true(level.charge_machine)) {
...
slow_mo_sphere(var_0) {
  ...
  self.sacred_ground._id_FB2F = scripts\engine\utility::play_loopsound_in_space("town_mute_circle_lp", var_0.origin);
...
watch_for_damage_on_machine() {
  ...
  for(;;) {
    ...
    if(scripts\engine\utility::is_true(var_1.fired_fov_beam)) {
...
cp_town_gns_4_setup() {
  ...
  level.all_perk_list = ["perk_machine_boom", "perk_machine_flash", "perk_machine_fwoosh", "perk_machine_more", "perk_machine_rat_a_tat", "perk_machine_revive", "perk_machine_run", "perk_machine_deadeye", "perk_machine_tough", "perk_machine_change", "perk_machine_zap"];
...
cp_town_enter_ghosts_n_skulls_func(var_0) {
  var_0 visionsetnakedforplayer("cp_zmb_ghost_path", 0);
...
paintings_activation_function(var_0, var_1) {
  ...
  if(!scripts\engine\utility::is_true(var_1.weeping_angels_puzzle)) {
    return;
  }
...
watch_for_damage_on_cipher_letter(var_0) {
  ...
  for(;;) {
    ...
    if(scripts\engine\utility::is_true(self.completed_cipher_letter)) {
      self.model.maxhealth = 99999;
      self.model.health = 99999;
      continue;
    }
...
register_waves_movement() {
  scripts\cp\maps\cp_zmb\cp_zmb_ghost_wave::register_moving_target_wave(1, 1.0, 2.0, 0.7);
  scripts\cp\maps\cp_zmb\cp_zmb_ghost_wave::register_moving_target_wave(2, 1.0, 2.0, 0.7);
  scripts\cp\maps\cp_zmb\cp_zmb_ghost_wave::register_moving_target_wave(3, 2.0, 4.0, 0.7);
...
medusa_watch_for_player_melee(var_0) {
  ...
  for(;;) {
    ...
    if(!scripts\engine\utility::is_true(var_2.triggered_rad_extractor_device)) {
...
activatefiguredamage(var_0, var_1, var_2) {
  ...
  var_0 endon("end_thread_for_" + var_0.model.model);
  ...
  for(;;) {
    ...
    if(scripts\engine\utility::is_true(var_0.got_hit_by_player)) {
      continue;
    }
    if(var_0.model.health < 0) {
      var_0.model.health = 5;
      var_0.model.maxhealth = 5;
      var_0.model setscriptablepartstate("figure_effect", "death");
      var_4 playlocalsound("town_kill_black_ghost_success");
      var_0.got_hit_by_player = 1;
      ...
      var_0 notify("end_thread_for_" + var_0.model.model);
    }
  }
}

showhiddenfigurestoplayer(var_0, var_1, var_2, var_3) {
  var_3 notify("one_instance_of_" + var_1.script_noteworthy);
  var_3 endon("one_instance_of_" + var_1.script_noteworthy);
  var_3 endon("death");
  ...
  if(getDvar("scr_hidden_figures_activate", "") == "") {
  ...
  var_1.model hidefromplayer(var_3);
}

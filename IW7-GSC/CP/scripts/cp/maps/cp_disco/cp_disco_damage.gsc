...
cp_disco_onzombiedamaged(var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9, var_10, var_11) {
  ...
  if(var_4 != "MOD_SUICIDE") {
    if(!isDefined(var_1) || !scripts\engine\utility::is_true(var_1.battackzombies)) {
  ...
  var_15 = scripts\engine\utility::is_true(var_1.inlaststand);
  var_16 = scripts\engine\utility::is_true(var_12.is_suicide_bomber);
  var_3 = var_3 | 4;
  var_17 = isDefined(var_1) && isPlayer(var_1);
  var_18 = scripts\engine\utility::isbulletdamage(var_4) || var_4 == "MOD_EXPLOSIVE_BULLET" && var_8 != "none";
  var_19 = var_18 && scripts\cp\utility::isheadshot(var_5, var_8, var_4, var_1);
  var_20 = scripts\engine\utility::is_true(self.battleslid);
  var_21 = scripts\engine\utility::is_true(level.insta_kill) && !scripts\cp\utility::agentisinstakillimmune();
  ...
  var_23 = !var_15 && var_19 && var_18 && var_1 scripts\cp\utility::is_consumable_active("headshot_explosion");
  var_24 = var_14 && var_1 scripts\cp\utility::is_consumable_active("increased_melee_damage");
  var_25 = var_14 && var_1 scripts\cp\utility::is_consumable_active("shock_melee_upgrade");
  var_26 = scripts\cp\utility::isaltmodeweapon(var_5);
  var_27 = scripts\cp\utility::agentisfnfimmune();
  var_28 = scripts\cp\utility::agentisinstakillimmune();
  ...
  if(var_17) {
    ...
    if(scripts\engine\utility::is_true(var_1.snake_super))
  ...
  if(!var_14 && scripts\cp\agents\gametype_zombie::checkaltmodestatus(var_5) && var_17 && !isDefined(var_1.linked_to_coaster) && var_1 scripts\cp\utility::is_consumable_active("sniper_soft_upgrade"))
    var_33 = var_1 scripts\cp\utility::coop_getweaponclass(var_5) == "weapon_sniper";

  var_34 = !var_27 && scripts\engine\utility::is_true(level.explosive_touch) && isDefined(var_4) && var_4 == "MOD_UNKNOWN";
  var_35 = !var_28 && (var_20 || var_21 || var_25 || var_34 || var_23 || var_24 || var_33);
  var_36 = isDefined(self.isfrozen);

  if(scripts\cp\powers\coop_armageddon::isfirstarmageddonmeteorhit(var_5) && !var_28) {
    ...
    if(var_32) {
      ...
      if(scripts\engine\utility::is_true(var_1.isreaping)) {
    ...
    if(var_25) {
      ...
      var_1 thread scripts\cp\zombies\zombie_damage::stun_zap(self getEye(), self, self.maxhealth, "MOD_UNKNOWN", undefined, var_25);
    }
    ...
    var_37 = level.wave_num;
    var_38 = scripts\cp\agents\gametype_zombie::is_grenade(var_5, var_4);
    var_39 = scripts\engine\utility::is_true(self.is_burning) && !var_18;
    var_40 = var_19 && var_1 scripts\cp\utility::is_consumable_active("sharp_shooter_upgrade");
    var_41 = var_18 && var_1 scripts\cp\utility::is_consumable_active("bonus_damage_on_last_bullets");
    var_42 = var_18 && var_1 scripts\cp\utility::is_consumable_active("damage_booster_upgrade");
    var_43 = var_18 && isDefined(var_1.special_ammo_weapon) && var_1.special_ammo_weapon == var_5;
    var_44 = var_17 && var_1 scripts\cp\utility::has_zombie_perk("perk_machine_boom");
    var_45 = var_17 && var_1 scripts\cp\utility::has_zombie_perk("perk_machine_smack");
    var_46 = scripts\cp\agents\gametype_zombie::is_axe_weapon(var_5);
    var_47 = scripts\engine\utility::array_contains(level.melee_weapons, var_5);
    var_29 = getweaponbasename(var_5);
    var_48 = var_17 && (var_5 == "iw7_katana_zm" || var_29 == "iw7_katana_zm_pap1" || var_29 == "iw7_katana_zm_pap2");
    var_49 = var_17 && issubstr(var_5, "shuriken");
    var_50 = weaponclass(var_5) == "spread" && var_1 scripts\cp\cp_weapon::has_attachment(var_5, "smart");
    var_51 = weaponclass(var_5) == "spread" && !var_50 && (var_1 scripts\cp\cp_weapon::has_attachment(var_5, "arkpink") || scripts\cp\cp_weapon::has_attachment(var_5, "arkyellow"));
    var_52 = var_19 && var_18 && var_1 scripts\cp\cp_weapon::has_attachment(var_5, "highcal");
    ...
    if(isDefined(var_2) && isDefined(var_8) && !var_21 && var_18) {
      var_53 = scripts\cp\zombies\zombie_armor::process_damage_to_armor(var_12, var_1, var_2, var_8, var_7);
    ...
    if(var_17) {
      if(var_14) {
        ...
        if(scripts\engine\utility::is_true(var_1.kung_fu_mode)) {
          if(var_54 && !isDefined(self.launched)) {
            var_55 = 3500;
            ...
            if(lengthsquared(var_7) < 1) {
              ...
              var_56 = vectorNormalize((var_56[0], var_56[1], 0));
            ...
            var_57 = 1;
            thread kung_fu_damage_everyone_in_radius(self.origin, 96, var_1, var_57);
          }
        } else if(var_46 || var_45) {
          if(var_46) {
            var_1 thread scripts\cp\utility::add_to_notify_queue("axe_melee_hit", var_5, self, var_2);

            if(var_54 && !isDefined(self.launched)) {
              thread scripts\cp\agents\gametype_zombie::launch_and_kill(var_1, var_5, var_45);
              return;
            }
          } else if(var_54)
            self.slappymelee = 1;
      ...
      if(scripts\engine\utility::is_true(var_1.crane_super)) {
        var_55 = 3500;
        self.ragdollhitloc = var_8;
        ...
        var_58 = playfxontagforclients(level._effect["screen_blood"], var_1, "tag_eye", var_1);
      }
    ...
    if(var_41) {
      var_59 = int(var_1 getweaponammoclip(var_1 getcurrentweapon()) + 1);
      var_60 = weaponclipsize(var_1 getcurrentweapon());

      if(var_59 <= 4)
        var_2 = var_2 * 2;
    }

    if(var_18 && scripts\engine\utility::is_true(var_1.reload_damage_increase))
      var_2 = var_2 * 2;

    if(var_38)
      var_2 = var_2 * min(2 + var_37 * 0.5, 10);
  ...
  if(isDefined(var_1.perk_data) && var_1.perk_data["damagemod"].bullet_damage_scalar == 2.0 && var_18)
    var_2 = var_2 * 1.33;
...
handlegoreeffect(var_0, var_1, var_2) {
  ...
  if(!scripts\engine\utility::is_true(var_0.is_cop)) {
    var_3 = var_0 gettagorigin("j_spine4");
    playFX(level._effect["bloody_death"], var_3, (1, 0, 0));
...
cp_disco_onzombiekilled(var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8) {
  ...
  if(isPlayer(var_1)) {
    var_10 = getweaponbasename(var_4);
    var_11 = var_4 == "iw7_nunchucks_zm";
    var_12 = var_10 == "iw7_nunchucks_zm_pap1";
    var_13 = var_10 == "iw7_nunchucks_zm_pap2";
    ...
    var_14 = var_3 == "MOD_EXPLOSIVE_BULLET" && (isDefined(var_6) && var_6 == "none") || var_3 == "MOD_EXPLOSIVE" || var_3 == "MOD_GRENADE_SPLASH" || var_3 == "MOD_PROJECTILE" || var_3 == "MOD_PROJECTILE_SPLASH";
    ...
    if(isDefined(var_1.itempicked)) {
      foreach(var_16 in level.powers) {
        if(var_16.weaponuse == var_4) {
          if(var_16.weaponuse == var_1.itempicked) {
  ...
  var_19 = 0;
  var_20 = 0;
  var_21 = scripts\engine\utility::is_true(self.is_suicide_bomber);
  ...
  if(scripts\engine\utility::isbulletdamage(var_3) && (getweaponbasename(var_4) == "iw7_atomizer_mp" || scripts\engine\utility::is_true(self.atomize_me)) || var_3 == "MOD_UNKNOWN" && getweaponbasename(var_4) == "iw7_harpoon3_zm") {
    if(!var_21 && !var_18 && !var_19 && !var_20) {
  ...
  if(isDefined(self.near_crystal) && !var_21) {
    if(isDefined(level.closest_crystal_func))
      var_22 = level[[level.closest_crystal_func]](self);
    else
      var_22 = undefined;

    if(isDefined(var_22)) {
      if(isDefined(level.crystal_killed_notify))
        thread scripts\cp\agents\gametype_zombie::delayminiufocollection(self.origin, var_4, var_22);
    }
  }
...
disco_process_kill_rewards(var_0, var_1, var_2, var_3, var_4, var_5) {
  ...
  if(isDefined(var_7)) {
    ...
    if(var_11) {
      ...
      if(scripts\engine\utility::is_true(var_7.kung_fu_mode)) {
        if(randomint(100) > 30)
...
callback_discozombieplayerdamage(var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9, var_10, var_11) {
  ...
  if(var_19) {
    if(var_1 == self) {
      ...
      if(scripts\engine\utility::is_true(self.kung_fu_shield)) {
        return;
      }

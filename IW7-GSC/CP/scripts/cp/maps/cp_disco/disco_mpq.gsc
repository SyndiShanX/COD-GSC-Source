...
phase_1() {
  ...
  if(isDefined(level.spoke_to_pam_first_after_wave_five) && !scripts\engine\utility::is_true(level.played_first_pam_dialogue)) {
...
active_symbol_logic() {
  ...
  for(;;) {
    ...
    if(scripts\engine\utility::is_true(var_3.wearing_rat_king_eye)) {
      break;
    } else
      wait 0.1;
  }
...
p2t2_3_poster_puzzle() {
  ...
  var_1 = spawnStruct();
  var_1.light = getEnt("spotlight_light", "targetname");
  var_1.poster = getEnt(var_1.light.target, "targetname");
...
watch_for_spotlight_power(var_0) {
  ...
  playsoundatpos(var_0.light.origin, "power_buy_rooftop_spotlight_turn_on");
  var_1 = var_0.light.model;
  var_0.light setModel("cp_disco_searchlight_swivel_on");
...
mpqrelicusefunc(var_0, var_1) {
  ...
  switch (var_0.relic) {
    ...
    case "heart":
      ...
      if(!scripts\engine\utility::is_true(var_1.has_heart)) {
    ...
    case "eye":
      ...
      if(!scripts\engine\utility::is_true(var_1.has_eye)) {
...
p2t3_0_missing_reel() {
  ...
  var_18 = scripts\engine\utility::drop_to_ground(var_5[0].origin, 30, -100);
  var_19 = spawn("script_model", var_18 + (0, 0, 32));
  var_20 = scripts\engine\utility::spawn_tag_origin(var_19.origin + anglesToForward(var_19.angles) * 2, (0, 0, 0));
...
missing_reel_pickup_players(var_0, var_1) {
  ...
  if(scripts\engine\utility::is_true(var_1.inlaststand) || scripts\engine\utility::is_true(var_1.in_afterlife_arcade)) {
...
watchforallplayersinroom() {
  ...
  for(;;) {
    ...
    if(isDefined(level.clock_interaction) && scripts\engine\utility::is_true(level.clock_interaction.clock_active)) {
      continue;
    }
    if(isDefined(level.clock_interaction_q2) && scripts\engine\utility::is_true(level.clock_interaction_q2.clock_active)) {
      continue;
    }
    if(isDefined(level.clock_interaction_q3) && scripts\engine\utility::is_true(level.clock_interaction_q3.clock_active)) {
      continue;
    }
    foreach(var_4 in level.players) {
      if(scripts\engine\utility::is_true(var_4.isrewinding)) {
        var_2 = 0;
        break;
      }

      if(scripts\engine\utility::is_true(var_4.isfasttravelling)) {
        var_2 = 0;
        break;
      }

      if(scripts\engine\utility::is_true(var_4.is_off_grid)) {
        var_2 = 0;
        break;
      }
    }

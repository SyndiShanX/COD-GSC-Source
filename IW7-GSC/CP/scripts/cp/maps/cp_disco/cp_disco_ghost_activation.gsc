...
do_sky_steps() {
  scripts\engine\utility::flag_wait("rooftop_walkway_open");
  prepare_sky_steps();
...
register_waves_movement() {
  ...
  scripts\cp\maps\cp_zmb\cp_zmb_ghost_wave::register_moving_target_wave(2, 1.0, 1.6, 0.7);
  scripts\cp\maps\cp_zmb\cp_zmb_ghost_wave::register_moving_target_wave(3, 1.0, 1.2, 0.7);
...
cp_disco_process_moving_target_hit_with_right_color(var_0, var_1) {
  var_1 playlocalsound("ghosts_8bit_match3_positive");
  reveal_moving_target_color(var_0);
  var_2 = get_moving_targets_in_same_subgroup(var_0.subgroup);

  if(all_moving_targets_are_revealed(var_2)) {
    var_1 playlocalsound("ghosts_8bit_match3_explo");
    explode_moving_targets(var_2, var_1);
  }
}

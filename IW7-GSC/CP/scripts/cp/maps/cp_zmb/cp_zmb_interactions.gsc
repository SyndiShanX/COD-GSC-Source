...
register_interactions() {
  ...
  scripts\cp\cp_interaction::register_interaction("atm_deposit", "atm", 1, undefined, scripts\cp\zombies\zombies_atm::interaction_atm_deposit, 1000, 1, scripts\cp\zombies\zombies_atm::init_atm);
  ...
  scripts\cp\cp_interaction::register_interaction("crafted_medusa", "craftable", 1, undefined, scripts\cp\cp_weapon_medusa::give_crafted_medusa, 0);
  scripts\cp\cp_interaction::register_interaction("crafted_electric_trap", "craftable", 1, undefined, scripts\cp\crafted_trap_electric::give_crafted_trap, 0);
  scripts\cp\cp_interaction::register_interaction("crafted_boombox", "craftable", 1, undefined, scripts\cp\crafted_trap_boombox::give_crafted_boombox, 0);
  scripts\cp\cp_interaction::register_interaction("crafted_revocator", "craftable", 1, undefined, scripts\cp\crafted_trap_revocator::give_crafted_revocator, 0);
  scripts\cp\cp_interaction::register_interaction("crafted_gascan", "craftable", 1, undefined, scripts\cp\crafted_trap_gascan::give_crafted_gascan, 0);
  ...
  } else if(isDefined(level.escape_interaction_registration_func))
...
init_cryobomb() {
  ...
  var_0.item = var_0.trigger;
  var_0.item.hint_string = level.interaction_hintstrings["iw7_forgefreeze_zm"];
}
  ...
  for(;;) {
    ...
    if(scripts\cp\cp_interaction::interaction_is_ark_quest_station(var_4) && (!isDefined(var_4.crystals) || isDefined(var_4.crystals) && var_4.crystals.size < 1)) {
      ...
      if(!scripts\cp\cp_weapon::can_use_attachment("arkpink", self getcurrentweapon())) {
        scripts\cp\cp_interaction::reset_interaction();
        continue;
      } else if(scripts\engine\utility::is_true(self.has_white_ark)) {
        scripts\cp\cp_interaction::reset_interaction();
        continue;
      }
    }

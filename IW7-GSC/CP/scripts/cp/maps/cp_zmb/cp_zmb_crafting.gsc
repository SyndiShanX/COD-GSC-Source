...
init_crafting() {
  ...
  level.crafting_item_max_drop_time = 60000;
...
crafting_station_power(var_0) {
  ...
  if(scripts\engine\utility::is_true(self.requires_power) && isDefined(self.power_area))
    level scripts\engine\utility::waittill_any("power_on", self.power_area + " power_on");

  if(isDefined(self.light))
    self.light setlightintensity(0.65);
...
crafting_item_pickup(var_0, var_1) {
  ...
  if(isDefined(var_0.item.beingpickedup)) {
    return;
  }
  scripts\cp\cp_interaction::remove_from_current_interaction_list(var_0);
  ...
  playFX(level._effect["souvenir_pickup"], var_0.item.origin);
  var_0.item delete();
  ...
  if(isDefined(var_1.current_crafting_struct)) {
    ...
    var_3.item = spawn("script_model", var_2);
    var_3.item.angles = (90, 0, 0);
    var_3.item.og_angles = (90, 0, 0);
    ...
    var_3.crafting_icon = "";
    var_3.item setModel(var_3.crafting_model);
    var_1.current_crafting_struct = var_0;
    var_1 create_player_crafting_item_icon(var_0);
    var_3.item thread crafting_item_timeout(var_3);
    var_0 = undefined;
    scripts\cp\cp_interaction::add_to_current_interaction_list(var_3);
    wait 0.3;
    var_4 = "red";

    if(var_3.item.model == "zmb_coin_space")
      var_4 = "blue";
    else if(var_3.item.model == "zmb_coin_ice")
      var_4 = "green";

    var_3.item setscriptablepartstate("fx", var_4);
  } else {

...
dialogue_vo_handler(var_0, var_1, var_2, var_3, var_4, var_5, var_6) {
  ...
  foreach(var_10 in var_8) {
    ...
    if(var_7 && isDefined(level.vo_alias_data[var_10].dialogueprefix)) {
      var_12 = level.vo_alias_data[var_10].dialogueprefix;
    ...
    foreach(var_15 in level.players) {
      if(isDefined(var_12) && var_15.vo_prefix == var_12 || var_11 || getdvarint("scr_solo_dialogue", 0) == 1) {
        var_16 = scripts\cp\cp_vo::create_vo_data(var_13, var_3, var_5, var_6);
        var_15 scripts\cp\cp_vo::set_vo_system_playing(1);
        var_15 scripts\cp\cp_vo::set_vo_currently_playing(var_16);
        var_15 scripts\cp\cp_vo::play_vo(var_16);
        var_15 scripts\cp\cp_vo::pause_between_vo(var_16);
        var_15 scripts\cp\cp_vo::unset_vo_currently_playing();
        break;
      }
    }
  ...
  foreach(var_15 in level.players)
  var_15 scripts\cp\cp_vo::set_vo_system_playing(0);
  ...
  for(;;) {
    ...
    switch (var_0) {
      case "moon":
        ...
        var_3 thread scripts\cp\cp_vo::add_to_nag_vo("nag_board_windows", "zmb_comment_vo", 180, 60, 20, 1);
...
starting_vo() {
  ...
  if(scripts\cp\cp_music_and_dialog::can_play_dialogue_system()) {
    ...
    level.players[0] thread scripts\cp\cp_vo::try_to_play_vo("spawn_intro", "zmb_comment_vo", "high", 20, 0, 0, 1);
    level thread willard_intro_vo();
  }
}
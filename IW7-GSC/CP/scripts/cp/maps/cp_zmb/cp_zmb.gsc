...
main() {
  ...
  level.additional_laststand_weapon_exclusion = ["iw7_cpbasketball_mp", "iw7_cpskeeball_mp", "iw7_cpclowntoothball_mp", "iw7_horseracepistol_zm_blue", "iw7_horseracepistol_zm_yellow", "iw7_horseracepistol_zm_red", "iw7_horseracepistol_zm_green", "iw7_shootgallery_zm", "iw7_blackholegun_mp", "iw7_penetrationrail_mp", "iw7_atomizer_mp", "iw7_glr_mp", "iw7_claw_mp", "iw7_shootgallery_zm_blue", "iw7_shootgallery_zm_yellow", "iw7_shootgallery_zm_red", "iw7_shootgallery_zm_green"];
  ...
  level.alt_mode_weapons_allowed = ["iw7_g18_zm", "iw7_g18_zmr"];
...
setup_pap_camos() {
  ...
  level.no_pap_camos = ["axe", "forgefreeze"];
}
...
setup_tutorial_requirements(var_0) {
  switch (var_0) {
    case "quest_neil":
      if(scripts\engine\utility::is_true(level.neil_head_added))
        return 1;
      else
        return 0;
    default:
...
gator_mouth() {
  ...
  for(;;) {
    var_3 = level scripts\engine\utility::waittill_any_return_no_endon_death("power_on", "mars power_on", "power_off");
...
mouth_trig_kill(var_0) {
  ...
  while(gettime() < var_1) {
    ...
    if(scripts\engine\utility::is_true(var_2.isrewinding)) {
      continue;
    }
...
launch_players(var_0) {
  ...
  for(var_4 = 0; var_4 < 40; var_4++) {
    foreach(var_6 in level.players) {
      if(scripts\engine\utility::is_true(var_6.isrewinding)) {
        continue;
      }
...
gold_teeth_hint_func(var_0, var_1) {
  if(!isDefined(var_1.last_ticket_item_outlined)) {
    var_1.last_ticket_item_outlined = level.gold_tooth_3_pickup[0];
  ...
  } else if(var_1.last_ticket_item_outlined != level.gold_tooth_3_pickup[0]) {
...
zmb_power_gate_handler() {
  ...
  var_19 moveTo(var_19.origin - (0, 0, 37), 2, 0.1, 0.1);

  wait 1;
  var_21 = getEntArray("first_gate_bollard_clip", "targetname");
...
cp_zmb_near_equipment_func(var_0) {
  ...
  if(isDefined(level.alldjcenterstructs)) {
    ...
    if(scripts\engine\utility::is_true(var_2[0].placed)) {
...
cp_zmb_should_do_damage_check_func(var_0, var_1, var_2, var_3, var_4, var_5) {
  if(scripts\engine\utility::is_true(var_5.rocket_feet))
    return 0;

  if(scripts\engine\utility::is_true(var_5.shredder_death))
    return 0;

  if(scripts\engine\utility::is_true(var_5.dischord_spin))
    return 0;

  if(scripts\engine\utility::is_true(var_5.head_is_exploding) && var_3 != "iw7_headcutterdummy_zm")
    return 0;
...
cp_zmb_wait_to_be_revived_func(var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9, var_10, var_11) {
  if((scripts\cp\utility::isplayingsolo() || scripts\engine\utility::is_true(level.only_one_player)) && scripts\engine\utility::is_true(level.the_hoff_revive)) {
...
register_cp_zmb_target_waves_movement() {
  ...
  scripts\cp\maps\cp_zmb\cp_zmb_ghost_wave::register_moving_target_wave(2, 2.0, 3.0, 0.6);

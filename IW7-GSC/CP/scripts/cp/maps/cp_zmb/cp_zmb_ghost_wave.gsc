...
start_death_trigger_monitor() {
  if(scripts\engine\utility::is_true(level.disable_gns_death_trigger)) {
    return;
  }
...
moving_targets_sequence() {
  ...
  for(var_0 = 1; var_0 <= level.gns_num_of_wave; var_0++) {
    var_1 = scripts\engine\utility::getStructArray("ghost_formation_" + get_formation_id_for_wave(var_0), "targetname");
...
move_group(var_0, var_1, var_2, var_3) {
  ...
  foreach(var_7 in var_1) {
    ...
    if(scripts\engine\utility::is_true(var_7.flying_to_portal)) {
      continue;
    }
...
get_active_moving_target_in_group(var_0) {
  foreach(var_2 in var_0) {
    ...
    if(scripts\engine\utility::is_true(var_2.flying_to_portal)) {
      continue;
    }
    return var_2;
  }
...
activate_moving_targets_default(var_0) {
  foreach(var_2 in level.moving_target_groups) {
    foreach(var_4 in var_2) {
      ...
      var_4 rotateTo(var_4.angles_to_face_when_activated, 1.0, 1.0);
    }
  }
...
all_moving_targets_hide_color() {
  foreach(var_1 in level.moving_target_groups) {
    foreach(var_3 in var_1) {
      ...
      if(scripts\engine\utility::is_true(var_3.flying_to_portal)) {
        continue;
      }
...
show_color(var_0) {
  var_0 endon("death");
  var_0 rotateTo(var_0.angles_to_face_when_activated, 1.0, 1.0);
  wait 1.0;
...
store_and_take_perks(var_0) {
  ...
  foreach(var_3, var_2 in var_0.zombies_perks) {
    if(scripts\engine\utility::is_true(var_0.zombies_perks[var_3]) && should_be_removed_for_gns(var_3)) {
...
get_formation_id_for_wave(var_0) {

...
shoot_ice_monster_eyes_monitor(var_0) {
  ...
  scripts\cp\zombies\zombie_analytics::log_shooting_icemonster_coaster(level.wave_num);
}
...
eye_damage_monitor(var_0, var_1) {
  ...
  for(;;) {
    ...
    if(isDefined(var_3) && scripts\engine\utility::is_true(var_3.linked_to_coaster)) {
...
spell_the_word_ghost() {
  ...
  var_1 delete();
  scripts\cp\zombies\zombie_analytics::log_ghost_spelled(level.wave_num);
}
...
get_1_9_8_4_kills_on_dance_floor() {
  ...
  scripts\cp\zombies\zombie_analytics::log_1984_kills(level.wave_num);
}
...
play_arcade_games_and_make_critical_shot() {
  ...
  scripts\cp\zombies\zombie_analytics::log_beating_arcade_games(level.wave_num);
}
...
visibility_monitor(var_0) {
  ...
  for(;;) {
    foreach(var_2 in level.players) {
      if(scripts\engine\utility::is_true(var_2.wearing_dischord_glasses)) {
...
brute_hit_arcade_cabinet_with_laser() {
  ...
  scripts\cp\zombies\zombie_analytics::log_brute_hits_cabinet(level.wave_num);
}
...
wait_for_player_activation() {
  ...
  for(;;) {
    ...
    foreach(var_6 in level.players) {
      if(scripts\engine\utility::is_true(var_6.inlaststand)) {
        var_4 = 0;
        break;
      }

      if(scripts\engine\utility::is_true(var_6.iscarrying)) {
        var_4 = 0;
        break;
      }

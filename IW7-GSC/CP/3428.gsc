...
start_crystal_path(var_0, var_1, var_2, var_3, var_4, var_5, var_6) {
  ...
  while(var_10) {
    ...
    var_16 = var_6 scripts\engine\utility::waittill_any_return("movedone", "fully_charged");

    if(scripts\engine\utility::is_true(var_6.fully_charged)) {
      for(;;) {
        var_14 = get_move_rate(var_6, var_6.origin, var_12.origin, 2000);
        var_6 moveTo(var_12.origin, var_14);

        if(can_use_struct_for_final_pos(var_0, var_12)) {
          var_17 = scripts\engine\utility::drop_to_ground(var_12.origin, 0, -400) + (0, 0, 40);
...
crystal_listener(var_0, var_1) {
  ...
  if(scripts\engine\utility::is_true(level.skip_crystal_logic)) {
    level.skip_crystal_logic = 0;
...
run_battery_logic(var_0, var_1, var_2, var_3, var_4) {
  ...
  if(var_1.gun == "iw7_dischord_zm") {
    ...
    if(scripts\engine\utility::is_true(level.skip_battery_logic)) {
      level.skip_battery_logic = 0;
      wait 0.25;
    } else
      var_0 dischord_battery_move();
  } else if(var_1.gun == "iw7_facemelter_zm") {
    var_6 = "melter";
    level.facemelter_battery = var_0;
    wait 0.25;
  } else if(var_1.gun == "iw7_shredder_zm") {
    var_6 = "shredder";
...
is_in_fake_phase_shift() {
  return scripts\engine\utility::is_true(self.wor_phase_shift);
}
...
dischord_target_listener(var_0) {
  ...
  while(!var_1) {
    ...
    if(isPlayer(var_3) && (scripts\engine\utility::is_true(var_3.wearing_dischord_glasses) || level.debug_dischord_targets)) {
  ...
  for(;;) {
    foreach(var_1 in level.dischord_targets) {
      foreach(var_3 in level.players) {
        if(scripts\engine\utility::is_true(var_3.wearing_dischord_glasses) || level.debug_dischord_targets) {
...
reapply_visionset_after_host_migration() {
  ...
  if(scripts\engine\utility::is_true(self.wearing_dischord_glasses)) {
...
listen_for_cryo_kills() {
  ...
  for(var_1 = 10; var_0 < var_1; level.total_cryo_kills++) {
...
trackplayersworammo(var_0, var_1, var_2) {
  ...
  for(;;) {
    ...
    if(scripts\engine\utility::is_true(level.infinite_ammo)) {
      continue;
    }
...
watchforplayerdeath(var_0, var_1, var_2) {
  ...
  for(;;) {
    ...
    var_4 = 0;
    var_6 = var_0 scripts\engine\utility::waittill_any_return_no_endon_death("player_entered_ala", "revive", "death");

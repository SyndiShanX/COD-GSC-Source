...
fast_travel_init() {
  ...
  level._id_5592 = ::disable_teleportation;
...
_id_6AF8(var_0) {
  ...
  scripts\cp\powers\coop_phaseshift::_id_6626(1, var_0);
  wait(var_0);
...
_id_126BF(var_0, var_1) {
  ...
  if(!isDefined(var_1)) {
    if(!scripts\engine\utility::is_true(self.used_once)) {
      self.used_once = 1;
      var_5 = 0;
...
is_in_pap_room(var_0) {
  return scripts\engine\utility::is_true(self.is_in_pap);
}
...
_id_6B8B() {
  ...
  self.light setlightintensity(10);
  _id_F556();
}
...
_id_D681() {
  ...
  for(;;) {
    ...
    if(isPlayer(var_1) && self.portal_is_open) {
      ...
      scripts\cp\zombies\zombie_analytics::_id_AF85(level.wave_num, self.script_area);

      if(scripts\engine\utility::is_true(level.wor_portal_change_time)) {

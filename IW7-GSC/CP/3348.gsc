...
_id_D684(var_0) {
  scripts\cp\cp_weapon::monitordamage(100, "trophy", ::_id_D686, ::_id_D689, 0);
}
...
_id_D683(var_0) {
  ...
  var_1._id_D682 = spawnfx(scripts\engine\utility::getfx("portal_open"), self.origin + (0, 0, 50), anglesToForward(self.angles), anglestoup(self.angles));
  triggerfx(var_1._id_D682);
  scripts\cp\utility::playsoundinspace("phase_portal_start", var_4);
  scripts\engine\utility::delaycall(1.0, ::playloopsound, "phase_portal_energy_lp");
}
...
_id_13B15(var_0) {
  ...
  for(;;) {
    ...
    if(scripts\engine\utility::is_true(var_3.isrewinding)) {
      wait 0.1;
      continue;
    }

    if(scripts\engine\utility::is_true(var_3.playing_game)) {
      wait 0.1;
      continue;
    }

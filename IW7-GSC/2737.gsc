...
giveplayerscore(var_0, var_1) {
  ...
  var_3 scripts\mp\utility\game::bufferednotify("earned_score_buffered", var_1);
...
giveteamscoreforobjective(var_0, var_1, var_2) {
  ...
  if(!level.splitscreen && var_3 != "none" && var_3 != level.waswinning && gettime() - level.var_AA1E > 5000 && scripts\mp\utility\game::getscorelimit() != 1) {
    level.var_AA1E = gettime();
    scripts\mp\utility\game::leaderdialog("lead_taken", var_3, "status");
...
func_12F4A(var_0) {
  ...
  var_1 = scripts\mp\utility\game::getwingamebytype();
...
processassist_regularmp(var_0, var_1, var_2) {
  ...
  if(!level.teambased) {
    ...
    thread scripts\mp\utility\game::giveunifiedpoints("assist_ffa", var_1, var_7);
  } else if(scripts\mp\utility\game::_hasperk("specialty_mark_targets") && (isDefined(var_4) && scripts\engine\utility::array_contains(var_4, self))) {
    if(var_2) {
    ...
    thread scripts\mp\utility\game::giveunifiedpoints("assist", var_1, var_7);
  }

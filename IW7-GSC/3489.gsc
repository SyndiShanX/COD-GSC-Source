...
func_128EE(var_0, var_1) {
  ...
  if(isDefined(level.var_AD89) || scripts\mp\killstreaks\helicopter::exceededmaxlittlebirds(var_2)) {
    self iprintlnbold(&"KILLSTREAKS_AIR_SPACE_TOO_CROWDED");
    return 0;
  } else if(!level.var_1A66.size) {
    self iprintlnbold(&"KILLSTREAKS_UNAVAILABLE_IN_LEVEL");
    return 0;
  } else if(scripts\mp\utility\game::currentactivevehiclecount() >= scripts\mp\utility\game::maxvehiclesallowed() || level.fauxvehiclecount + var_3 >= scripts\mp\utility\game::maxvehiclesallowed()) {
  ...
  level thread scripts\mp\utility\game::teamplayercardsplash(level.heliheight[var_2].teamsplash, self, self.team);
  return 1;
}
...
func_49E1(var_0) {
  ...
  var_8 thread scripts\mp\killstreaks\helicopter::func_E111();
...
func_10DBE(var_0) {
  ...
  var_0 thread scripts\mp\killstreaks\flares::func_6EAA(::func_AADF);
  var_0 thread scripts\mp\killstreaks\flares::func_6EAB(::func_AAE0);
}
...
func_AADC() {
  ...
  for(;;) {
    ...
    if(isPlayer(var_1)) {
      if(var_1 != self.owner && distance2d(var_1.origin, self.origin) <= self.var_11587 && !var_1 scripts\mp\utility\game::_hasperk("specialty_blindeye") && !(level.hardcoremode && level.teambased && var_1.team == self.team)) {
  ...
  for(;;) {
    ...
    if(self.damagetaken >= self.maxhealth) {
      if(isPlayer(var_1) && (!isDefined(self.owner) || var_1 != self.owner)) {
        ...
        thread scripts\mp\utility\game::teamplayercardsplash("callout_destroyed_little_bird", var_1);
        var_1 thread scripts\mp\utility\game::giveunifiedpoints("kill", var_9, 300);
...
func_AADF(var_0, var_1, var_2, var_3) {
  ...
  for(var_4 = 0; var_4 < var_3.size; var_4++) {
    if(isDefined(var_3[var_4]) && !var_2.hasdodged) {
      ...
      var_5 thread scripts\mp\killstreaks\flares::func_6E9F(5.0);
...
func_AACC() {
  ...
  for(;;) {
    ...
    for(var_6 = 0; var_6 < var_5; var_6++) {
      ...
      if(isDefined(var_7) && (!isDefined(var_7.spawntime) || (gettime() - var_7.spawntime) / 1000 > 5) && (isDefined(var_7.team) && var_7.team != "spectator") && scripts\mp\utility\game::isreallyalive(var_7)) {

...
main() {
  ...
  level.laststand_enter_gamemodespecificaction = ::_id_13F1F;
  ...
  level.rebuild_all_windows_func = scripts\cp\zombies\interaction_windowrepair::_id_DDB4;
  level._id_B079 = scripts\cp\zombies\zombies_spawning::_id_13FA2;
  ...
  level.endgame_write_clientmatchdata_for_player_func = ::_id_13F1E;
...
zombie_onstartgametype() {
  ...
  level._id_F480 = ::set_player_max_currency;
  ...
  level thread scripts\cp\zombies\zombie_doors::_id_97B1();
  level thread scripts\cp\zombies\zombie_power::_id_96F4();
  level thread scripts\cp\zombies\interaction_magicwheel::_id_94EF();
...
player_hotjoin() {
  ...
  if(var_0 == "cp_town") {
    if(!scripts\engine\utility::is_true(level.power_on)) {
...
onspawnplayer() {
  ...
  thread scripts\cp\zombies\zombies_spawning::_id_D1F7();
  ...
  for(;;) {
    ...
    foreach(var_1 in level.players) {
      if(scripts\cp\cp_laststand::player_in_laststand(var_1)) {
        ...
        var_1 thread scripts\cp\cp_vo::try_to_play_vo("respawn_round", "zmb_comment_vo", "high", 5, 0, 0, 1, 60);
      }
    }
  }
}
...
get_respawn_loc_rated(var_0, var_1) {
  ...
  foreach(var_8 in var_1) {
    ...
    foreach(var_11 in var_0) {
      ...
      if(scripts\engine\utility::is_true(var_11.inlaststand)) {
...
zombie_player_connect_black_screen() {
  ...
  if(var_1 == "cp_town" && !self issplitscreenplayer() && !isDefined(level.cp_town_bink_played)) {
    wait 2.0;
...
zombie_callbackplayerkilled(var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9) {
  scripts\cp\zombies\zombie_analytics::_id_AF84(self, var_8);
...
zombiehostmigrationstart() {
  ...
  foreach(var_2 in var_0) {
    ...
    if(!var_3 && !var_2 scripts\cp\utility::agentisinstakillimmune()) {
      if(scripts\engine\utility::is_true(var_2.scripted_mode)) {
        var_2.died_poorly = 1;
        var_2 suicide();
        continue;
      }

      if(scripts\engine\utility::is_true(var_2.ignoreme)) {
        var_2.died_poorly = 1;
        var_2 suicide();
        continue;
      }

      if(scripts\engine\utility::is_true(var_2.ignoreall)) {
        var_2.died_poorly = 1;
        var_2 suicide();
        continue;
      }

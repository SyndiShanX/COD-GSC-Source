...
cp_town_onzombiedamaged(var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9, var_10, var_11) {
  ...
  var_14 = scripts\engine\utility::is_true(var_12.hit_by_dodging_player);
  var_15 = var_4 == "MOD_MELEE";
  var_16 = scripts\engine\utility::is_true(var_1.inlaststand);
  var_17 = scripts\engine\utility::is_true(var_12.is_suicide_bomber);
  var_3 = var_3 | 4;
  var_18 = isDefined(var_1) && isPlayer(var_1);
  var_19 = scripts\engine\utility::isbulletdamage(var_4) || var_4 == "MOD_EXPLOSIVE_BULLET" && var_8 != "none";
  var_20 = var_19 && scripts\cp\utility::isheadshot(var_5, var_8, var_4, var_1);
  var_21 = scripts\engine\utility::is_true(self.battleslid);
  var_22 = scripts\engine\utility::is_true(level.insta_kill) && !scripts\cp\utility::agentisinstakillimmune();
  ...
  var_24 = !var_16 && var_20 && var_19 && var_1 scripts\cp\utility::is_consumable_active("headshot_explosion");
  var_25 = var_15 && var_1 scripts\cp\utility::is_consumable_active("increased_melee_damage");
  var_26 = var_15 && var_1 scripts\cp\utility::is_consumable_active("shock_melee_upgrade");
  var_27 = scripts\cp\utility::isaltmodeweapon(var_5);
  var_28 = scripts\cp\utility::agentisfnfimmune();
  var_29 = scripts\cp\utility::agentisinstakillimmune();
  var_30 = var_18 && var_1 scripts\cp\utility::has_zombie_perk("perk_machine_change");
  var_31 = scripts\cp\utility::agentisspecialzombie();
  ...
  if(var_18 && !var_28) {
    ...
    if(scripts\engine\utility::is_true(var_1.stimulus_active)) {
      ...
      foreach(var_33 in level.players) {
        if(var_33 == var_1) {
          if(distance2dsquared(var_33.origin, self.origin) <= 10000) {
            playFX(level._effect["stimulus_glow_burst"], self gettagorigin("j_spineupper"));
            playFX(level._effect["stimulus_shield"], var_33 gettagorigin("tag_eye"), anglesToForward(var_33.angles), anglestoup(var_33.angles), var_33);

            if(var_2 >= self.health) {
              if(scripts\engine\utility::is_true(var_33.inlaststand)) {
                scripts\cp\zombies\zombies_consumables::revive_downed_entities(var_33);
              }
            }

            if(var_33.health + var_2 / (level.players.size + 1) >= var_33.maxhealth) {
              var_33.health = var_33.maxhealth;
            } else {
              var_33.health = int(var_33.health + var_2 / (level.players.size + 1));
            }
          }
        ...
        if(distance2dsquared(var_33.origin, self.origin) <= 10000) {
          playFX(level._effect["stimulus_glow_burst"], self gettagorigin("j_spineupper"));
          playFX(level._effect["stimulus_shield"], var_33 gettagorigin("tag_eye"));
          ...
          if(int(var_33.health + var_2 / (level.players.size + 1)) >= var_33.maxhealth) {
            var_33.health = var_33.maxhealth;
            continue;
          }
  ...
  if(!var_15 && scripts\cp\agents\gametype_zombie::checkaltmodestatus(var_5) && var_18 && !isDefined(var_1.linked_to_coaster) && var_1 scripts\cp\utility::is_consumable_active("sniper_soft_upgrade")) {
    var_35 = var_1 scripts\cp\utility::coop_getweaponclass(var_5) == "weapon_sniper";
  }

  var_36 = !var_28 && scripts\engine\utility::is_true(level.explosive_touch) && isDefined(var_4) && var_4 == "MOD_UNKNOWN";
  var_37 = isDefined(var_5) && var_5 == "iw7_knife_zm_cleaver" && iscrog(self);

  if(isDefined(var_5) && var_5 == "iw7_knife_zm_cleaver" && !iscrog(self)) {
    scripts\cp\utility::add_to_notify_queue("cleaver_damage_zombie");
  }

  var_38 = !var_29 && (var_21 || var_22 || var_37 || var_26 || var_36 || var_24 || var_25 || var_35);
  var_39 = scripts\engine\utility::is_true(self.isfrozen);
  var_40 = var_18 && var_15 && var_1.currentmeleeweapon == "iw7_knife_zm_cleaver";

  if(var_40) {
    if(scripts\engine\utility::is_true(self.glowing) && var_2 >= self.health) {
  ...
  if(var_38 && !var_28) {
    ...
    if(var_26) {
      ...
      var_1 thread scripts\cp\zombies\zombie_damage::stun_zap(self getEye(), self, self.maxhealth, "MOD_UNKNOWN", undefined, var_26);
    }
    ...
    var_41 = level.wave_num;
    var_42 = scripts\cp\agents\gametype_zombie::is_grenade(var_5, var_4);
    var_43 = scripts\engine\utility::is_true(self.is_burning) && !var_19;
    var_44 = var_20 && var_1 scripts\cp\utility::is_consumable_active("sharp_shooter_upgrade");
    var_45 = var_19 && var_1 scripts\cp\utility::is_consumable_active("bonus_damage_on_last_bullets");
    var_46 = var_19 && var_1 scripts\cp\utility::is_consumable_active("damage_booster_upgrade");
    var_47 = var_19 && isDefined(var_1.special_ammo_weapon) && var_1.special_ammo_weapon == var_5;
    var_48 = var_18 && var_1 scripts\cp\utility::has_zombie_perk("perk_machine_boom");
    var_49 = var_18 && var_1 scripts\cp\utility::has_zombie_perk("perk_machine_smack");
    ...
    var_53 = var_20 && var_19 && var_1 scripts\cp\cp_weapon::has_attachment(var_5, "highcal");
    ...
    if(var_19 && scripts\engine\utility::is_true(var_1.reload_damage_increase)) {
      var_2 = var_2 * 2;
    }
  ...
  if(isDefined(var_1.perk_data) && var_1.perk_data["damagemod"].bullet_damage_scalar == 2.0 && var_19) {
    var_2 = var_2 * 1.33;
  }
  ...
  var_2 = scripts\cp\agents\gametype_zombie::fateandfortuneweaponscale(self, var_5, var_2, 0, var_31, 0, 0);
...
kung_fu_damage_everyone_in_radius(var_0, var_1, var_2, var_3) {
  ...
  foreach(var_6 in level.spawned_enemies) {
    if(scripts\engine\utility::is_true(var_6.kung_fu_punched)) {
      continue;
    }

...
getstructstate(var_0, var_1) {
  ...
  foreach(var_2, var_4 in level.ufotones) {
...
getufolightcolor(var_0) {
  ...
  foreach(var_1, var_3 in level.ufotones) {
...
ufo_damage_monitor(ufo) {
  ufo endon("death");
  ufo.maxhealth = 999999999;
  ufo.health = 999999999;
  ufo.fake_health = 3000;
  ufo setCanDamage(1);

  for(;;) {
    ufo waittill("damage", var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9, var_10);
  ...
  if(isDefined(ufo.turrets)) {
    foreach(turret in ufo.turrets) {
      turret delete();
    }
  }

  playFX(level._effect["ufo_explosion"], ufo.origin);
...
attach_alien_fuse(alien) {
  alien.available_fuse = [];
  alien.available_fuse[alien.available_fuse.size] = create_alien_fuse(alien, "tag_back_le");
  alien.available_fuse[alien.available_fuse.size] = create_alien_fuse(alien, "tag_back_ri");
}

create_alien_fuse(alien, alien_part) {
  angles = (0, 0, 0);
...
remove_alien_fuse(var_0, fuse) {
  fuse.owner.available_fuse = scripts\engine\utility::array_remove(fuse.owner.available_fuse, fuse);
...
drop_soul_key() {
  key = spawn("script_model", (646, 774, 105));
  key setModel("zmb_soul_key_base");
  key_glow = spawnfx(level._effect["soul_key_glow"], key.origin);
  triggerfx(key_glow);
  key thread item_keep_rotating(key);
  key thread soul_key_pick_up_monitor(key, key_glow);
}

soul_key_pick_up_monitor(key, key_glow) {
  key endon("death");
  cutscene_id = 137;
  key makeusable();
  ...
  for(;;) {
    ...
    if(isPlayer(player)) {
      ...
      foreach(player in level.players) {
        check_willard_pick_up_soul_key(player);
        player setplayerdata("cp", "haveSoulKeys", "any_soul_key", 1);
        player setplayerdata("cp", "haveSoulKeys", "soul_key_1", 1);
        player scripts\cp\zombies\achievement::update_achievement("SOUL_KEY", 1);
      }
      ...
      if(any_player_is_willard()) {
        ...
        stop_gameplay_audio();
        scripts\cp\utility::play_bink_video("sysload_o4", cutscene_id);
        wait(cutscene_id);
        resume_spawn_wave();
      ...
      level thread soulkey_quest_vo(player);
      break;
    }
  }

  level thread scripts\cp\zombies\directors_cut::try_drop_talisman(key.origin, vectortoangles((0, 1, 0)));
  key_glow delete();
  key delete();
}
...
stop_gameplay_audio() {
  ...
  foreach(player in level.players) {
    scripts\cp\maps\cp_zmb\cp_zmb_vo::clear_up_all_vo(player);
    player setclienttriggeraudiozone("bink_fadeout_amb", 0.66);
  }
}
...
spaceland_powernode_damage_monitor() {
  ...
  for(;;) {
    ...
    if(scripts\engine\utility::is_true(var_11.is_activated)) {
      continue;
    }
...
should_play_arc_vfx(var_0) {
  if(var_0.targetname == "main_gate_powernode_5") {
    return 0;
  }

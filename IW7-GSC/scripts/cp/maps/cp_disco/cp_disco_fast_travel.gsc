...
trigger_when_player_close_by() {
  ...
  for(;;) {
    ...
    if(isPlayer(var_6) && scripts\engine\utility::is_true(var_6.kung_fu_mode)) {
      break;
    }
  ...
  for(;;) {
    ...
    if(scripts\engine\utility::is_true(var_16.isrewinding)) {
      scripts\engine\utility::waitframe();
      var_16 thread scripts\cp\maps\cp_disco\cp_disco::update_special_mode_for_player(var_16);
      continue;
    }

    if(scripts\engine\utility::is_true(var_16.inlaststand)) {
      scripts\engine\utility::waitframe();
      continue;
    }
...
script_add_teleport_spots() {
  ...
  foreach(var_3 in var_0) {
    ...
    var_4.origin = var_3;
    var_4.angles = var_1;
    var_4.targetname = self.teleport_spots[0].targetname;

...
start_rk_fight() {
  ...
  foreach(var_4, var_2 in level.players) {
    if(scripts\engine\utility::is_true(var_2.start_breaking_clock)) {
      var_2.start_breaking_clock = 0;
      var_2 notify("rat_king_fight_started");
    }

    if(scripts\engine\utility::is_true(var_2.inlaststand) || scripts\engine\utility::is_true(var_2.in_afterlife_arcade)) {
...
runrkstage1() {
  ...
  var_0 = scripts\cp\utility::isplayingsolo() || scripts\engine\utility::is_true(level.only_one_player);
...
runheartrelicquest(var_0) {
  ...
  var_1 = scripts\cp\utility::isplayingsolo() || scripts\engine\utility::is_true(level.only_one_player);
...
clearsewageonzombiedeath(var_0, var_1) {
  ...
  for(;;) {
    ...
    if(!scripts\engine\utility::is_true(var_7.active)) {
      continue;
    }
    var_8 = var_7.pool;
...
watchforplayerinvolume(var_0, var_1) {
  ...
  for(;;) {
    ...
    if(!isalive(var_2) || scripts\engine\utility::is_true(var_2.inlaststand)) {
      continue;
    }
    var_3 = gettime();
    ...
    if(!isDefined(var_4) || !scripts\engine\utility::is_true(var_4.active)) {
      continue;
    }
...
cleanupsewage(var_0, var_1, var_2) {
  ...
  var_2.active = undefined;
  wait 0.25;
  var_1.count++;
  level notify("1_pool_clean");

  if(var_1.count >= var_1.goal) {
...
waitforbraindestroyed(var_0) {
  ...
  while(var_2 < var_3) {
    ...
    if(!var_4) {
      if(distance(level.rat_king.origin, var_1) <= 100) {
        if(var_2 > 0) {
          var_2 = scripts\engine\utility::ter_op(var_2 > 2, var_2 - 2, 0);
        }
      }
    }
...
getvalidalivezombies(var_0, var_1, var_2) {
  ...
  foreach(var_6 in var_4) {
    ...
    if(scripts\engine\utility::is_true(var_6.entered_playspace)) {
...
getvalidbrainpos(var_0) {
  ...
  foreach(var_4 in var_2) {
    if(scripts\engine\utility::is_true(var_4.claimed)) {
      continue;
    }
    var_4.claimed = 1;
    return var_4;
...
runeyerelicquest(var_0) {
  ...
  var_0.goal = getmaxactivetargets();
  var_0.count = 0;
...
checkraceresults(var_0, var_1, var_2) {
  ...
  for(;;) {
    ...
    if(level.active_eye_targets.size < var_1.goal) {
...
userkdebug(var_0, var_1) {
  ...
  open_sesame();
  setupplayerloadouts();
  level thread start_rk_fight();
}

open_sesame(var_0) {
  if(scripts\engine\utility::is_true(var_0)) {
    level.open_sesame = undefined;
  } else if(scripts\engine\utility::is_true(level.open_sesame)) {
    level.open_sesame = undefined;
    return;
  } else
    level.open_sesame = 1;
  ...
  foreach(var_9 in var_11) {
    var_9.scriptable notify("damage", undefined, "open_sesame");
    wait 0.1;
  }
  ...
  var_17 = scripts\engine\utility::getStructArray("interaction", "targetname");

  foreach(var_19 in var_17) {
    var_20 = scripts\engine\utility::getStructArray(var_19.script_noteworthy, "script_noteworthy");

    foreach(var_22 in var_20) {
      if(isDefined(var_22.target) && isDefined(var_19.target)) {
        if(var_22.target == var_19.target && var_22 != var_19) {
    ...
    if(scripts\cp\cp_interaction::interaction_is_door_buy(var_19)) {
      if(!isDefined(var_19.script_noteworthy)) {
        continue;
      }
      if(var_19.script_noteworthy == "team_door_switch") {
        scripts\cp\zombies\interaction_openareas::use_team_door_switch(var_19, level.players[0]);
      }
    }
  }
}
...
setupplayerloadouts() {
  ...
  foreach(var_5 in level.players) {
    ...
    if(!scripts\engine\utility::is_true(var_5.style_chosen)) {

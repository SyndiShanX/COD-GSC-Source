...
mosh_trap_trigger(var_0, var_1) {
  ...
  for(;;) {
    ...
    if(scripts\engine\utility::is_true(var_2.is_turned) || scripts\engine\utility::is_true(var_2.mosh_trap) || scripts\engine\utility::is_true(var_2.is_traversing)) {
      continue;
    }
    if(scripts\engine\utility::is_true(var_2.is_skeleton)) {
      continue;
    }
...
rockmode(var_0, var_1) {
  ...
  self endon("death");
  self.mosh_trap = 1;
  self.og_movemode = self.movemode;
  self.movemode = "sprint";
  self.goalradius_old = self.goalradius;
  self.is_rocking = 1;

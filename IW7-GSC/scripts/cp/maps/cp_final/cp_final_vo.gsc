...
final_starting_vo() {
  ...
  var_13 = scripts\engine\utility::random(["spawn_intro", "spawn_solo_first"]);
  level.players[0] thread scripts\cp\cp_vo::try_to_play_vo(var_13, "final_comment_vo", "high", 20, 0, 0, 1);
  var_3 = scripts\cp\cp_vo::get_sound_length(level.players[0].vo_prefix + var_13);
  if(isDefined(var_3)) {
    wait(var_3);
  }

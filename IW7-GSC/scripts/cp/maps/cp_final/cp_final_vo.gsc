    ...
    foreach(var_12 in var_9) {
      ...
      foreach(var_10 in level.players) {
        if(isDefined(var_14) && var_10.vo_prefix == var_14 || var_13 || getdvarint("scr_solo_dialogue", 0) == 1) {
          var_17 = scripts\cp\cp_vo::create_vo_data(var_15, var_3, var_5, var_6);
...
willard_intro_vo() {
  ...
  if(scripts\engine\utility::is_true(level.directors_cut_is_activated)) {
...
purchase_area_vo(var_0, var_1) {
  ...
  if(scripts\engine\utility::is_true(var_1.played_vo)) {
    var_1.played_vo = 0;
    return;
  }
  ...
  if(randomint(100) < 50) {
    level thread scripts\cp\cp_vo::try_to_play_vo("ww_access_area", "final_announcer_vo", "highest", 70, 0, 0, 1);
  } else {
    var_1 thread scripts\cp\cp_vo::try_to_play_vo("purchase_area_misc", "final_comment_vo", "low", 10, 0, 2, 1, 40);
  }
...
final_starting_vo() {
  ...
  if(scripts\cp\cp_music_and_dialog::can_play_dialogue_system()) {
    ...
    foreach(var_11 in level.players) {
      var_3 = scripts\cp\cp_vo::get_sound_length(var_11.vo_prefix + "spawn_team_first");
      wait(var_3);
    }

    level thread willard_intro_vo();
  } else {
    level.players[0] thread scripts\cp\cp_vo::try_to_play_vo("spawn_solo_first", "final_comment_vo", "high", 20, 0, 0, 1);
    var_3 = scripts\cp\cp_vo::get_sound_length(level.players[0].vo_prefix + "spawn_solo_first");
    wait(var_3);
    level thread willard_intro_vo();
  }
}
...
can_play_backstory_vo() {
  if(scripts\engine\utility::is_true(level.started_backstory_dialogue)) {
    return 0;
  }

  if(scripts\engine\utility::is_true(level.dialogues_playing)) {
    return 0;
  }

...
register_interactions() {
  ...
  scripts\cp\cp_interaction::register_interaction("atm_deposit", "atm", undefined, undefined, ::atm_deposit, 1000);
...
use_computer(var_0, var_1) {
  if(scripts\engine\utility::is_true(var_1.has_zis_soul_key)) {
    return;
  } else if(scripts\engine\utility::is_true(level.has_picked_up_fuses)) {
    return;
  } else {
...
interaction_pickup_harpoon_weapon(var_0, var_1) {
  ...
  if(!scripts\engine\utility::is_true(var_0.quest_complete)) {
...
register_crafting_interactions() {
  ...
  scripts\cp\cp_interaction::register_interaction("crafted_ims", "craftable", 1, undefined, scripts\cp\crafted_trap_fireworks::give_crafted_fireworks_trap, 0);
  scripts\cp\cp_interaction::register_interaction("crafted_medusa", "craftable", undefined, undefined, scripts\cp\cp_weapon_medusa::give_crafted_medusa, 0);
  scripts\cp\cp_interaction::register_interaction("crafted_electric_trap", "craftable", undefined, undefined, scripts\cp\crafted_trap_electric::give_crafted_trap, 0);
  scripts\cp\cp_interaction::register_interaction("crafted_boombox", "craftable", undefined, undefined, scripts\cp\crafted_trap_boombox::give_crafted_boombox, 0);
  scripts\cp\cp_interaction::register_interaction("crafted_revocator", "craftable", undefined, undefined, scripts\cp\crafted_trap_revocator::give_crafted_revocator, 0);
  scripts\cp\cp_interaction::register_interaction("crafted_gascan", "craftable", undefined, undefined, scripts\cp\crafted_trap_gascan::give_crafted_gascan, 0);
...
survivor_use_func(var_0, var_1) {
  ...
  if(!scripts\engine\utility::is_true(level.metks)) {
...
play_meet_ks_vo(var_0, var_1) {
  ...
  } else if(scripts\engine\utility::flag("photo_2_kev_given") && !scripts\engine\utility::flag("photo_2_kev_vo_done")) {
    var_0 scripts\cp\maps\cp_rave\cp_rave_j_mem_quest::play_jay_memory_to_kev(var_1);
...
update_rave_mode_for_player(var_0) {
  ...
  while(scripts\engine\utility::is_true(var_0.rave_mode_updating)) {
    scripts\engine\utility::waitframe();
  }
...
unsetravetriggeraftertime(var_0, var_1, var_2, var_3, var_4) {
  level endon("game_ended");
  var_0 scripts\engine\utility::waittill_any_timeout(scripts\engine\utility::ter_op(scripts\engine\utility::is_true(level.only_one_player) || scripts\cp\utility::isplayingsolo(), 60, 2), "picked_up");
...
rave_ritual_stone_hint(var_0, var_1) {
  if(!scripts\engine\utility::is_true(var_1.has_rave_dust)) {
...
trigger_rave_mode_ritual(var_0, var_1) {
  ...
  while(gettime() < var_2) {
    foreach(var_7 in level.players) {
      ...
      if(scripts\engine\utility::is_true(var_7.rave_mode)) {
        continue;
      }
...
cp_rave_interaction_monitor() {
  ...
  for(;;) {
    ...
    if(!scripts\cp\cp_interaction::can_use_interaction(var_4)) {
      ...
      if(isDefined(self.last_ticket_item_outlined)) {
        self.last_ticket_item_outlined hudoutlinedisableforclient(self);
        self.last_ticket_item_outlined = undefined;
      }
  ...
  for(;;) {
    ...
    if(isDefined(level.interactions[var_0.script_noteworthy].can_use_override_func)) {
      ...
      if(scripts\engine\utility::is_true(level.has_picked_up_fuses) && !isDefined(level.placed_alien_fuses)) {
        var_2 = 0;
      } else if(scripts\engine\utility::is_true(var_1.has_zis_soul_key) || scripts\engine\utility::is_true(level.placed_alien_fuses)) {
        if(var_5 == 3) {

...
enter_afterlife_arcade(var_0) {
  ...
  if(isDefined(var_0)) {
    if(check_self_revive_attempts(var_0)) {
      var_0 scripts\cp\utility::setlowermessage("welcome_to_afterlife", &"CP_ZOMBIE_AFTERLIFE_ARCADE_WELCOME", 6);
    } else {
      var_0 scripts\cp\utility::setlowermessage("welcome_to_afterlife_no_self_revives", &"CP_ZOMBIE_AFTERLIFE_ARCADE_NO_REVIVES", 6);
    }
  ...
  var_0 visionsetnakedforplayer("cp_zmb_afterlife", 1);
...
play_willard_afterlife_vo(var_0) {
  ...
  while(scripts\engine\utility::istrue(var_0.vo_system_playing_vo)) {
    wait(0.1);
  }
  ...
  while(var_0.array_of_memoir_vos.size > 0) {
    ...
    if(isDefined(var_0.array_of_memoir_vos[0])) {
      if(soundexists(var_0.array_of_memoir_vos[0])) {
        wait(randomfloatrange(80, 110) + scripts\cp\cp_vo::get_sound_length(var_0.array_of_memoir_vos[0]));
      }
...
black_screen_fades_in(var_0) {
  ...
  var_1.alpha = 1;
  var_1.foreground = 1;
  var_1 fadeovertime(15);
  var_1.alpha = 0;
  var_1 scripts\cp\utility::waittill_any_ents_or_timeout_return(15, var_0, "player_exit_afterlife", level, "game_ended");
  var_1 destroy();
}
...
is_in_afterlife_arcade(var_0) {
  return scripts\engine\utility::istrue(var_0.in_afterlife_arcade);
}
...
use_selfrevive_door(var_0, var_1) {
  ...
  if(var_1 scripts\cp\utility::isplayingsolo() || level.only_one_player) {
    ...
    var_2.times_self_revived = var_2.times_self_revived + 1;
    var_2 = int(level.wave_num / 10) + 1 - var_2.times_self_revived;
  }
...
get_self_revive_door_hint(var_0, var_1) {
  if(check_self_revive_attempts(var_1)) {
    if(has_self_revive_token(var_1) || scripts\cp\utility::isplayingsolo() || scripts\engine\utility::istrue(level.only_one_player)) {
...
close_afterlife_door_for_player(var_0) {
  ...
  stopfxontagforclients(level._effect["vfx_zmb_portal_centhub"], level.ala_portal_org, "tag_origin", var_0);
}
...
has_self_revive_token(var_0) {
  return scripts\engine\utility::istrue(var_0.has_self_revive_token);
}
...
check_self_revive_attempts(var_0) {
  ...
  if(scripts\engine\utility::istrue(var_0.have_gns_perk)) {
    return 1;
  }
  ...
  if(int(level.wave_num / 10) + 1 - var_0.times_self_revived >= 1) {
    return 1;
  }
...
player_enter_transition_monitor(var_0) {
  ...
  var_0 visionsetnakedforplayer("", 2);
}

...
revive_power_on_func(var_0) {
  ...
  if(isDefined(self.target)) {
    self.light = getEnt(self.target, "targetname");
...
play_perk_vo_additional(var_0) {
  wait(scripts\cp\cp_vo::get_sound_length(self.vo_prefix + var_0) + 5.0);
...
give_revive_perk() {
  if((scripts\cp\utility::isplayingsolo() || level.only_one_player) && self.self_revives_purchased < self.max_self_revive_machine_use) {
    ...
    self.perk_data["medic"].revive_time_scalar = 2.0;
}
...
update_revive_perks() {
  foreach(var_1 in level.players) {
    if(var_1 scripts\cp\utility::has_zombie_perk("perk_machine_revive")) {
      ...
      var_1.perk_data["medic"].revive_time_scalar = 2.0;
    }
  }
}
...
run_deadeye_charge_watcher() {
  ...
  for(;;) {
    ...
    if(self adsButtonPressed() && !scripts\engine\utility::is_true(self.no_deadeye) && !var_3) {
      var_4 = gettime();
      ...
      if(!isDefined(var_0)) {
        ...
        if(!scripts\engine\utility::is_true(self.deadeye_charge)) {
...
give_rat_a_tat_perk() {
  ...
  self.perk_data["damagemod"].bullet_damage_scalar = 2.0;
...
burn_loop(var_0) {
  ...
  for(;;) {
    ...
    if(isalive(var_1) && !scripts\engine\utility::is_true(var_1.marked_for_death)) {
...
remove_perks_from_player() {
  ...
  if(scripts\engine\utility::is_true(self.dontremoveperks)) {
    return;
  }

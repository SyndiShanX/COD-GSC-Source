...
playflyoveraudioline() {
  level endon("host_migration_begin");
  self endon("disconnect");
  var_0 = scripts\mp\utility::gettimepassed() / 1000 + 6;
  ...
  var_1 = self getplayerdata("mp", "activeMissionTeam");
...
dointro() {
  ...
  var_0 = !scripts\mp\utility::gameflag("prematch_done") && !scripts\mp\utility::isroundbased() || scripts\mp\utility::func_9DF6();
  if(var_0) {
...
func_C54B(var_0) {
  ...
  if(scripts\mp\utility::istrue(var_0.playedlastmanstandingvo)) {
    return;
  }

  var_0.playedlastmanstandingvo = 1;
  if(level.gametype == "siege") {
    var_0 scripts\mp\utility::leaderdialogonplayer("lastalive_zones");
    return;
  }

  if(level.gametype == "sr" && !level.nofriendlytags) {
    var_0 scripts\mp\utility::leaderdialogonplayer("lastalive_revive");
    return;
  }
...
post_match_music() {
  ...
  foreach(var_2 in level.players) {
    ...
    if(level.finalkillcam_winner != "none") {
      var_2 setplayermusicstate("mus_mp_killcam");
    }
  }
}
  ...
  if(level.teambased) {
    ...
    if(var_7 == "allies") {
      foreach(var_7 in level.players) {
        if(isDefined(var_7.team) && var_7.team == "allies") {
          var_7 setplayermusicstate(game["music"]["victory_allies"]);
          continue;
        }

        var_7 setplayermusicstate(game["music"]["defeat_axis"]);
      }
    ...
    if(var_7 == "axis") {
      foreach(var_7 in level.players) {
        if(isDefined(var_7.team) && var_7.team == "axis") {
          var_7 setplayermusicstate(game["music"]["victory_axis"]);
          continue;
        }

        var_7 setplayermusicstate(game["music"]["defeat_allies"]);
      }
    ...
    foreach(var_7 in level.players) {
      if(isDefined(var_7.team) && var_7.team == "allies") {
        var_7 setplayermusicstate(game["music"]["draw_allies"]);
        continue;
      }

      var_7 setplayermusicstate(game["music"]["draw_axis"]);
    }
...
func_7690() {
  ...
  if(isplayer(var_0) && !level.teambased) {
    for(var_2 = 0; var_2 < level.placement["all"].size; var_2++) {
      ...
      if(var_2 < 3) {
        var_3 scripts\mp\utility::leaderdialogonplayer("mission_success");
        continue;
      }

      var_3 scripts\mp\utility::leaderdialogonplayer("mission_failure");
    }
    ...
    if(!level.splitscreen) {
      if(var_0 == "time") {
        ...
        if(level.teambased) {
          ...
          var_8 scripts\mp\utility::leaderdialogonplayer("winning_score");
          scripts\mp\utility::leaderdialogonplayers("losing_score", var_9);
        }
      }
  ...
  for(;;) {
    ...
    foreach(var_4 in level.players) {
      ...
      if(var_4.team == "allies") {
        var_4 setplayermusicstate(game["music"]["allies_suspense"][level.cursuspsensetrack["allies"]]);
        continue;
      }

      var_4 setplayermusicstate(game["music"]["axis_suspense"][level.cursuspsensetrack["axis"]]);
    }
  }
}

...
dosummonspawn() {
  ...
  foreach(var_2 in self.spawnpoints) {
    var_3 = scripts\cp\zombies\zombies_spawning::_id_13F53(var_0.summon_agent_type, var_2, self.angles, "axis");

    if(!isDefined(var_3)) {
      break;
    }

    var_3 thread scripts\cp\zombies\zombies_spawning::_id_64E7(var_0.summon_agent_type);
  }
}
...
aimatenemy(var_0, var_1) {
  ...
  while(isDefined(var_1) && isalive(var_1)) {
    self.lookposition = var_1 getshootatpos();
...
gibthyself() {
  self.nocorpse = 1;
  self.full_gib = 1;
  self dodamage(self.health + 50000, self.origin);
}
...
lostorfoundstaff(var_0, var_1, var_2, var_3) {
  if(scripts\engine\utility::is_true(self.bstaffchanged)) {

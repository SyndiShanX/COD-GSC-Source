...
soul_to_door(var_0) {
  ...
  var_2 = self.scriptable.origin + (0, 0, 50);
...
open_team_killdoor(var_0) {
  ...
  self.scriptable setscriptablepartstate("fx", "normal");
  self.scriptable setscriptablepartstate("default", "hide");
  scripts\cp\zombies\zombie_analytics::_id_AF7E(1, var_0, self.name, self.kill_goal, level.wave_num);
...
open_team_killdoor_sfx(var_0) {
  wait 0.5;
  playsoundatpos(self.scriptable.origin, "zmb_clear_barricade");
  wait 0.5;
}
...
chi_door_think() {
  ...
  for(;;) {
    ...
    if(!isDefined(level.open_sesame) || !level.open_sesame) {
      ...
      if(issubstr(var_17, "fists_zm_")) {} else if(!issubstr(var_17, "shuriken")) {
        var_18 = ["door_chi_none_1", "door_chi_none", "door_chi_notenough"];
        var_9 thread scripts\cp\cp_vo::try_to_play_vo(scripts\engine\utility::random(var_18), "disco_comment_vo");
        continue;
      }
    ...
    if(isPlayer(var_9) && var_9 scripts\cp\utility::is_valid_player()) {
      foreach(var_21 in var_3) {
        if(!isDefined(level.spawn_volume_array[var_21.script_area])) {
          continue;
        }
    ...
    foreach(var_21 in var_3) {
      scripts\cp\zombies\zombies_spawning::set_adjacent_volume_from_door_struct(var_21);
      scripts\cp\zombies\zombies_spawning::activate_volume_by_name(var_21.script_area);
      wait 0.05;
    }

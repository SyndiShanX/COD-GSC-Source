...
_id_8715(var_0) {
  ...
  if(!scripts\sp\mgturret::_id_13030(var_0)) {
...
_id_491C() {
  ...
  waittillframeend;
  var_0 = spawnStruct();
  scripts\engine\utility::array_thread(level._id_B6B2, ::_id_B6B1, var_0);
...
_id_DDEB() {
  ...
  self._id_A8BB = self.enemy getEye();
  self notify("saw_enemy");

  if(!isalive(self._id_4B6D) || self._id_4B6D != self.enemy) {
    self._id_4B6D = self.enemy;

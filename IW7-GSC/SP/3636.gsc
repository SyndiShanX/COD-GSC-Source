...
_id_2A2D() {
  _id_2A30();
  level.player._id_2A23 = 100;
  level.player._id_2A2E = 0;
  level.player._id_4BDD = 0;
  var_0 = getspawnerteamarray("axis");
  scripts\sp\utility::_id_22C7(var_0, ::_id_2A32);
  level.player thread _id_2A35();
}
...
_id_2A2B() {
  ...
  var_0 = [];
  var_1 = scripts\sp\hud_util::_id_499B("alien_icon_craft_battery", 15, 15);
...
_id_2A33(var_0) {
  ...
  self._id_2A29 = 6000;
  self._id_2A2F = 0;
  thread _id_2A27(var_0);

  for(;;) {
    self allowfire(0);

    if(!self attackButtonPressed()) {} else if(self issprinting()) {} else if(self issprintsliding()) {} else if(self ismeleeing()) {} else if(self isthrowinggrenade()) {} else if(self._id_9F47) {} else if(self._id_2A2F) {} else {
      ...
      if(!self._id_2A2E) {
        thread _id_FC1E();
        self _meth_80D8(0.18, 0.18);
        thread scripts\sp\utility::_id_D2CD(25, 0.05);
      } else
        wait 0.3;
      ...
      self._id_2A2A = 0;
      self _meth_80A6();
      thread scripts\sp\utility::_id_D2CA(0.05);
...
_id_28C6(var_0, var_1, var_2, var_3, var_4) {
  ...
  for(var_17 = 0; var_17 < var_15; var_17++) {
    ...
    if(distance2d(var_16, var_11["position"]) < var_13) {
      break;
    }

    var_10 = var_11["position"];
    var_18 = vectorNormalize(var_10 - var_9);
    var_16 = var_16 + var_18 * var_13;

    if(level.player._id_4BDD >= 500) {
      continue;
    }
    thread _id_2A26();
    playFX(scripts\engine\utility::getfx("beam_rifle_beam"), var_16);
  }
  ...
  if(isDefined(var_11["surfacetype"]) && level.player._id_4BDD < 500) {
    var_20 = vectortoangles(var_12["normal"]);
...
_id_2A27(var_0) {
  ...
  for(;;) {
    ...
    while(self._id_2A2A) {
      var_1 = self._id_2A23 - level._id_EFFE;
      _id_2A34(var_1);
      wait 0.05;
    }

    thread _id_2A31(var_0);
  }
}
...
_id_2A25(var_0) {
  ...
  for(;;) {
    ...
    if(self._id_2A23 >= 100) {
      continue;
    }
    break;
  }

  var_2 = self._id_2A23 + 15;
  _id_2A34(var_2);
...
_id_2A34(var_0) {
  ...
  if(isDefined(self._id_2A2B)) {
    self._id_2A2B["energyBar"] scripts\sp\hud_util::updatebar(var_1);
    self._id_2A2B["ammo"] settext(int(self._id_2A23));
  }
}

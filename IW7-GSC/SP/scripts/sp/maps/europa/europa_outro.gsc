...
_id_94FB() {
  ...
  foreach(var_2 in var_0) {
    var_2._id_99E5 = var_2 _meth_8134();
...
_id_6222() {
  ...
  foreach(var_2 in var_0) {
    if(isDefined(var_2._id_99E5)) {
      var_2 setlightintensity(var_2._id_99E5);
      continue;
    }
...
_id_C7B4() {
  ...
  scripts\engine\utility::delaythread(0.6, ::_id_6222);
...
_id_ABE1() {
  scripts\sp\utility::_id_1264E("europa_fatty_tr");
  wait 0.05;
...
_id_EBEA() {
  ...
  var_0 = scripts\sp\utility::_id_22CD("outro_enemies", 1);
  var_1 = [];

  foreach(var_3 in var_0) {
    if(var_3._id_1FBB == "sdf1") {
      var_3 scripts\sp\utility::_id_72EC("iw7_m4", "primary");
      var_3.a.nodeath = 1;
      var_1[var_1.size] = var_3;
      level._id_C7D2 = var_3;
      continue;
    } else if(var_3._id_1FBB == "sdf2") {
      var_3 scripts\sp\utility::_id_72EC("iw7_m4", "primary");
      var_1[var_1.size] = var_3;
      var_3 scripts\sp\utility::_id_F6FE("vignette");
      var_3 _id_0A1E::_id_2318();
      continue;
    } else {
      if(var_3._id_1FBB == "kotch") {
        var_3 scripts\sp\utility::_id_86E4();
        level._id_A70E = var_3;
        level._id_C7BD[level._id_C7BD.size] = var_3;
        continue;
      }

      if(var_3.unittype == "c6" || var_3._id_1FBB == "sdf4") {
        level._id_C7BD[level._id_C7BD.size] = var_3;
        var_3 scripts\sp\utility::_id_72EC("iw7_m4", "primary");
        continue;
      }
  ...
  level._id_A70E attach("oxygen_bottle_air_boss", "tag_accessory_right");
  thread _id_C7C9();
  ...
  var_1[var_1.size] = level.player._id_E505;
  var_1[var_1.size] = level._id_EBBB;
  var_1[var_1.size] = level._id_EBBC;
  level.player._id_E505 setModel("viewmodel_un_jackal_pilots_frost");
  level.player._id_8632 = spawn("script_origin", level.player.origin);
  level.player._id_8632 linkTo(level.player._id_E505, "tag_player", (0, 0, 0), (0, 0, 0));
  level.player takeallweapons();
  level.player disableweapons();
  level.player _meth_80D8(0.2, 0.2);
  level.player setviewangleresistance(5, 5, 5, 5, 1);
  ...
  thread scripts\sp\hud_util::_id_6A99(8);
  thread _id_C7BC();
  setmusicstate("mx_173_cine_europaoutro");
  level.player._id_8632 = spawn("script_origin", level.player.origin);
  level.player._id_8632 linkTo(level.player._id_E505, "tag_player", (0, 0, 0), (0, 0, 0));
  level.player._id_E505 thread _id_D20A(var_9);
  thread _id_C7D7();
  level.player _meth_8392(1, 1, 1);
  level.player lerpviewangleclamp(3, 1.5, 1.5, 20, 20, 20, 20);
  var_9 thread scripts\sp\anim::_id_1F2C(var_1, "outro");
...
_id_912F() {
  thread scripts\sp\hud::_id_8DFD(2, 1);
  thread scripts\sp\hud::_id_8DFF(-280, 1);
  level.player waittill("o2_in");
  thread scripts\sp\hud::_id_8DFD(7, 10);
  level.player waittill("o2_out");
  thread scripts\sp\hud::_id_8DFD(0, 1);
}
...
_id_BF03() {
  _id_0B0A::_id_583F(0, 60, 1, 0, 0, 0, 0.5);
  level.player waittill("kotch_intro");
  level waittill("kotch_kneel");
  _id_0B0A::_id_583F(0, 0, 0, 53, 88, 1.28, 1);
  level waittill("look_at_friendlies");
  _id_0B0A::_id_583D(1.5);
  level waittill("kotch_stands");
  level waittill("kotch_kneel2");
  _id_0B0A::_id_583F(0, 0, 0, 53, 88, 1.5, 2);
  level.player waittill("o2_out");
  _id_0B0A::_id_583D(0.5);
  level.player waittill("connor");
  _id_0B0A::_id_583F(0, 35, 3, 0, 0, 0, 2);
}

_id_584C() {
  level endon("stop_dof_target_thread");
  var_0 = spawn("script_origin", level.player getEye());
  level._id_584B = level._id_EBBB;
  var_1 = vectorNormalize(level._id_584B gettagorigin("j_neck") - level.player getEye());
  var_0.origin = level.player getEye() + var_1 * 2;
  level._id_5840 = var_0;
  wait 5;
  var_0 thread _id_5841();
  var_2 = scripts\engine\utility::array_combine(level._id_C7BD, level._id_EBCA);

  for(;;) {
    var_3 = level.player getplayerangles();
    var_4 = level.player getEye();
    var_5 = _id_77D9(level._id_584B, var_4, var_3);
    var_6 = scripts\engine\utility::array_remove(var_2, level._id_584B);
    var_7 = level._id_584B;

    foreach(var_9 in var_2) {
      if(!isalive(var_9)) {
        continue;
      }
      var_10 = _id_77D9(var_9, var_4, var_3);
    ...
    level._id_584B = var_7;
    wait 0.05;
  }
}

_id_77D9(var_0, var_1, var_2) {
...
_id_5841() {
  ...
  for(;;) {
    ...
    var_3 = level._id_584B gettagorigin("j_neck");
...
_id_F4B0(var_0, var_1, var_2) {
  ...
  var_10 = var_8 * var_5;
  var_11 = var_9 + 150;
  _id_0B0A::_id_583F(var_10, var_8, var_3, var_9, var_11, var_4, var_1);
}
...
_id_C7BD(var_0) {
  ...
  level._id_A70E scripts\anim\shared::placeweaponon("iw7_nrg", "left");
  var_1 = scripts\sp\utility::_id_10639("kotch_gun", level._id_A70E.origin);
  level._id_A70E._id_1FB6 = var_1;
  var_2 = level._id_C7BD;
  var_2[var_2.size] = var_1;
  scripts\engine\utility::delaythread(8, ::_id_C06D);
  scripts\engine\utility::delaythread(11, ::_id_CFA5);
  var_3 = scripts\sp\maps\europa\europa_util::_id_5F32(var_0);
  wait 0.05;
  var_3 thread scripts\sp\anim::_id_1F2C(var_2, "outro");
}

_id_C7C0(var_0) {
  var_1 = scripts\sp\maps\europa\europa_util::_id_5F32(var_0);
  level._id_C7C1 scripts\sp\utility::_id_86E4();
  var_2 = scripts\sp\utility::_id_10639("flag");
  var_3 = [level._id_C7C1, var_2];
  var_1 thread scripts\sp\anim::_id_1F2C(var_3, "outro");
}
...
_id_C7BC() {
  ...
  var_0 show();
  var_0 thread _id_5E0B();
  wait 22;
  var_1 = scripts\sp\vehicle::_id_1080C("outro_dropship");
  var_1._id_55A4 = 1;
  var_1 notsolid();
...
_id_5E0C() {
  ...
  _id_5E0D(var_1, var_0);
  wait 0.1;
  _id_5E0D(var_2, var_0);
  wait 0.1;
  _id_5E0D(var_3, var_0);
  wait 0.1;
  _id_5E0D(var_4, var_0);
}
...
_id_EBEF() {
  var_0 = 3;
  wait(level._id_C7D5 - var_0);
  thread scripts\sp\hud_util::_id_6AA3(var_0);
  waittillframeend;
  var_1 = scripts\sp\hud_util::_id_7B4F();
  var_1.foreground = 0;
  wait(var_0);
}
...
_id_8E0B() {
  ...
  for(;;) {
    if(var_2 != level._id_8E0F) {
      var_2 = level._id_8E0F;
      var_1 = gettime() + 500;
      var_3 = gettime() + var_4[level._id_8E0F];

      if(level._id_8E0F == "good") {
        var_3 = 999999;
        var_1 = 999999;
      }
    }
    ...
    if(gettime() > var_1) {
      var_1 = gettime() + 6000;
      thread scripts\sp\utility::_id_10350(var_0[level._id_8E0F]);
    }
...
_id_FB33() {
  scripts\sp\utility::_id_10461("plr_helmet_air_leak_lp", 1, 4, 1);
  level.player waittill("o2_in");
  self stoploopsound();
  level.player waittill("o2_out");
  scripts\engine\utility::delaythread(11, scripts\sp\utility::_id_10461, "plr_helmet_air_leak_lp", 1, 2, 1);
  level.player waittill("sfx_beep_fade");
  scripts\sp\utility::_id_10460(1);
}

_id_FB47() {
  scripts\sp\utility::_id_10461("plr_helmet_o2_level_ok_lp", 1, 4, 1);
  wait 6;
  level.player thread scripts\sp\utility::_id_10350("europa_cmp_oxygendepleted");
  level.player waittill("o2_lvl_2");
  self stoploopsound();
  self playLoopSound("plr_helmet_o2_level_low_lp");
  thread scripts\sp\utility::_id_10350("europa_cmp_oxygendepleted");
  level.player waittill("o2_lvl_3");
  thread scripts\sp\utility::_id_10350("europa_cmp_oxygenlevelcrit");
  wait 1.0;
  self stoploopsound();
  ...
  level.player waittill("o2_out");
  wait 0.6;
  scripts\sp\utility::_id_10461("plr_helmet_o2_level_low_lp", 1, 2, 1);
  wait 2;
  thread scripts\sp\utility::_id_10350("europa_cmp_oxygendepleted");

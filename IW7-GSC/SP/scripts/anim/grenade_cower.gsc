/******************************************
 * Decompiled and Edited by SyndiShanX
 * Script: scripts\anim\grenade_cower.gsc
******************************************/

#using_animtree("generic_human");

main() {
  if(getdvarint("ai_iw7", 0) == 1) {
    scripts\asm\asm::asm_fireephemeralevent("grenade response", "avoid");
    self endon("killanimscript");
    self endon("death");
    self waittill("killanimscript");
  }

  self endon("killanimscript");
  scripts\anim\utility::_id_9832("grenadecower");

  if(isDefined(self._id_85B6)) {
    self[[self._id_85B6]]();
    return;
  }

  if(self.a.pose == "prone") {
    scripts\anim\stop::main();
    return;
  }

  self animmode("zonly_physics");
  self orientmode("face angle", self.angles[1]);
  var_0 = 0;

  if(isDefined(self.grenade))
    var_0 = angleclamp180(vectortoangles(self.grenade.origin - self.origin)[1] - self.angles[1]);
  else
    var_0 = self.angles[1];

  if(self.a.pose == "stand") {
    if(isDefined(self.grenade) && _id_12895(var_0)) {
      return;
    }
    self _meth_82E4("cowerstart", scripts\anim\utility::_id_B027("grenade", "cower_squat"), %body, 1, 0.2);
    scripts\anim\shared::donotetracks("cowerstart");
  }

  self.a.pose = "crouch";
  self.a.movement = "stop";
  self _meth_82E4("cower", scripts\anim\utility::_id_B027("grenade", "cower_squat_idle"), %body, 1, 0.2);
  scripts\anim\shared::donotetracks("cower");
  self waittill("never");
}

end_script() {
  self.safetochangescript = 1;
}

_id_12895(var_0) {
  if(randomint(2) == 0)
    return 0;

  if(self.stairsstate != "none")
    return 0;

  var_1 = undefined;

  if(abs(var_0) > 90)
    var_1 = scripts\anim\utility::_id_B027("grenade", "cower_dive_back");
  else
    var_1 = scripts\anim\utility::_id_B027("grenade", "cower_dive_front");

  var_2 = getmovedelta(var_1, 0, 0.5);
  var_3 = self localtoworldcoords(var_2);

  if(!self maymovetopoint(var_3))
    return 0;

  self.safetochangescript = 0;
  self _meth_82E4("cowerstart", var_1, %body, 1, 0.2);
  scripts\anim\shared::donotetracks("cowerstart");
  self.safetochangescript = 1;
  return 1;
}
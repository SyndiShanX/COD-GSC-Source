...
hint_manager_common_hints() {
  _id_0BDC::jackal_hintmanager_hint_add("roll", &"ZEROG_ROTATE_TUTORIAL_HINT", ::hint_roll_conditions, ::hint_roll_terminator, ["+actionslot 3", "+actionslot 4"]);
  _id_0BDC::jackal_hintmanager_hint_add("missiles", &"JACKAL_MISSILE", ::hint_missile_conditions, ::hint_missile_terminator, ["+frag"]);
  _id_0BDC::jackal_hintmanager_hint_add("weaponDrone", &"JACKAL_MARK_DROPZONE", ::hint_weapdrone_conditions, ::hint_weapdrone_terminator);
  _id_0BDC::jackal_hintmanager_hint_add("dodfight", &"JACKAL_ADS", ::hint_dogfight_conditions, ::hint_dogfight_terminator, ["+speed_throw", "+toggleads_throw", "+ads_akimbo_accessible"]);
  _id_0BDC::jackal_hintmanager_hint_add("ascend", &"ZEROG_ASCEND_TUTORIAL_HINT", ::hint_ascend_conditions, ::hint_ascend_terminator, ["+gostand"]);
  _id_0BDC::jackal_hintmanager_hint_add("descend", &"ZEROG_DESCEND_TUTORIAL_HINT", ::hint_descend_conditions, ::hint_descend_terminator, ["+stance"]);
}
...
hint_manager() {
  ...
  if(!scripts\sp\gameskill::jackal_arena_is_early_in_the_game() && level._id_7683 == 3) {
    return;
  }
  _id_0BDC::_id_137D6();
  wait 1;
  jackal_hintmanager_run_action_watchers();
  _id_0BDC::jackal_hintmanager_refresh_all_timers();
  do_progression_specific_tutorials();

  for(;;) {
    foreach(var_1 in level._id_A056.hintmanager.hints) {
      if(level._id_A056.hintmanager jackal_hint_common_conditions(var_1) && [[var_1.hint_conditions]](var_1) || var_1._id_7258) {
        level._id_A056.hintmanager jackal_hint(var_1);
...
jackal_hint(var_0) {
  ...
  self.hinting = 1;
  self._id_00C8 = var_0.hint_name;
  var_0.hinting = 1;
  scripts\sp\utility::_id_56BA(var_0.hint_name);
  ...
  for(;;) {
    ...
    if(tutorial_roll_hint_conditions()) {
      if(!scripts\engine\utility::flag("flag_jackal_hintmanaged_hint")) {
        scripts\engine\utility::flag_set("flag_jackal_hintmanaged_hint");
        scripts\sp\utility::_id_56BA("roll");
      }
...
tutorial_roll_pass_conditions() {
  ...
  if(level._id_241D && level._id_D127.spaceship_mode == "hover") {
    return 0;
  }

  if(level._id_A056.hintmanager._id_54F1) {
    return 0;
  }
...
hint_roll_conditions(var_0) {
  ...
  if(abs(level._id_D127.angles[2]) > 110) {
  ...
  if(_id_0BD1::_id_D30D()) {
    return 0;
  }

  if(level._id_D127 _id_0BDC::_id_8B87()) {
    return 0;
  }
  ...
  if(isDefined(var_0.inverted_time) && var_1 - var_0.inverted_time > 1000 && var_1 - var_0.last_action_time > 45000) {
...
hint_weapdrone_conditions(var_0) {
  ...
  if(gettime() - level._id_D127.missiles._id_A8E8 < 90000) {
    return 0;
  }

  if(level._id_A056._id_933B.size > 0) {
    return 0;
  }
...
hint_weapdrone_terminator(var_0) {
  ...
  if(level._id_A056._id_933B.size > 3) {
    return 0;
  }
...
hint_dogfight_conditions(var_0) {
  if(!isDefined(level.player._id_4BE7)) {
    return 0;
  }

  var_1 = vectordot(anglesToForward(level._id_D127.angles), vectorNormalize(level.player._id_4BE7.origin - level._id_D127.origin));
  var_2 = distance(level.player._id_4BE7.origin, level._id_D127.origin);

  if(var_2 > 18000 || var_1 < 0.85) {
    return 0;
  }

  if(gettime() - var_0.last_time < 45000) {
    return 0;
  }

  if(gettime() - level._id_A056._id_A976 < 140000) {
    return 0;
  }
...
common_ascend_descend_conditions() {
  ...
  if(!isDefined(level._id_A056.target_capitalship)) {
    return 0;
  }

  var_0 = vectordot(anglesToForward(level._id_D127.angles), vectorNormalize(level._id_A056.target_capitalship.origin - level._id_D127.origin));
  var_1 = distance(level._id_A056.target_capitalship.origin, level._id_D127.origin);
...
get_target_capship_local_z() {
  var_0 = rotatevectorinverted(level._id_D127.origin - level._id_A056.target_capitalship.origin, level._id_A056.target_capitalship.angles);
  return var_0[2];
}
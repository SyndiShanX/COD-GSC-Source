...
can_regen_health(var_0) {
  if(!scripts\engine\utility::istrue(var_0.can_do_health_regen)) {
    return 0;
  }
...
process_stop_regen_health_action(var_0) {
  ...
  if(var_0.num_of_times_stop_regen_health >= 2) {
...
mobile_shield_visibility_monitor(var_0, var_1) {
  ...
  for(;;) {
    ...
    if(var_2 && scripts\engine\utility::istrue(var_0.is_shooting)) {
      var_1 hide();
    } else {
      var_3 = gettime();
...
delay_update_next_hide_time(var_0) {
  var_0 endon("death");
  if(scripts\engine\utility::istrue(var_0.in_delay_update_next_hide_time)) {
    return;
  }

 ...
 * Script: aitype\enemy_cp_rus_desert_sniper_nvg.gsc
*****************************************************/
...
function main() {
  ...
  self.accuracy = 0.2;
  self.weapon = scripts\cp\cp_weapon::buildweapon("iw8_sn_delta_mp", ["laserir_bar_always_on"], "none", "none");
...
function setup_model(var_0) {
  scripts\cp\calloutmarkerping_cp::main_mp();
}

function precache() {
  var_0 = "actor_enemy_cp_rus_desert_sniper_nvg";
  ...
  if(!isDefined(level.agent_definition[var_0])) {
    ...
    scripts\cp\calloutmarkerping_cp::precache_mp(var_0);
  }

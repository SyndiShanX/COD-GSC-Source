 ...
 * Script: scripts\cp\gametypes\cp_survival.gsc
************************************************/
...
function onstartgametype() {
  ...
  if(level.gametype == "cp_survival") {
    for(var0 = 0; var0 < level.players.size; var0++) {
      level.players[var0] setclientomnvar("ui_session_state", "spectator");
    }
  ...
  for(;;) {
    ...
    if(!isai(var0)) {
      ...
      if(isDefined(level.playerclearstreamhintorigin)) {
        var0 thread[[level.playerclearstreamhintorigin]]();
      }
      ...
      if(scripts\engine\utility::flag("introscreen_over")) {
        ...
        if(isDefined(level.setobjectivelocations)) {
          var0 thread[[level.setobjectivelocations]]();
        }
      }
...
function default_strike_player_connect_black_screen(var0) {
  ...
  if(istrue(var0.unset_relic_oneclip)) {
...
function kick_for_inactivity(var0) {
  ...
  var1 = 0;
  var2 = gettime();
  var3 = level.onlinegame && !getdvarint("LSTLQTSSRM");
...
function add_to_kick_queue(var0) {
  ...
  if(istrue(var0.start_anim_train_scene)) {
    return;
  }
...
function enable_dogtag_revive(var0) {
  ...
  level notify("laststand_dogtag_spawned", var1);
}
...
function revivetriggerthink(var0) {
  ...
  for(;;) {
    ...
    var3 = scripts\cp\utility::player_lua_progressbar(var2, var1, 9216, 5);
...
function lbravo_infil_spawn_blackscreen_func(var0) {
  level endon("game_ended");
  var0 endon("disconnect");
  var0 endon("stop_intro");
  var1 = 6;
  ...
  wait 5;
  var0 scripts\cp\survival\survival_loadout::givedefaultloadout(0, undefined, 0);
  var0 disableweapons();
  var0 scripts\cp\utility::freezecontrolswrapper(1);
  var0 setclientomnvar("ui_hide_hud", 0);
  show_introscreen_text(var0);
  ...
  if(!scripts\cp\utility::gameflag("infil_started")) {
    var0.introscreen_overlay destroy();
    level notify("trying_to_join_infil", var0);
    return;
  }
...
function canspawnitemname(var0, var1) {
  self endon("disconnect");
  scripts\cp\utility::hint_prompt(var0, 1);
  wait var1;
  scripts\cp\utility::hint_prompt(var0, 0);
}
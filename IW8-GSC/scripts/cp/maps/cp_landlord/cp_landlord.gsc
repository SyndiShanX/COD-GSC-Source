 ...
 * Script: scripts\cp\maps\cp_landlord\cp_landlord.gsc
*******************************************************/

function main() {
  scripts\cp\cp_compass::setupminimap("compass_map_cp_landlord");
  scripts\cp\maps\cp_landlord\cp_landlord_precache::main();
  scripts\cp\maps\cp_landlord\gen\cp_landlord_art::main();
  scripts\cp\maps\cp_landlord\cp_landlord_fx::main();
  ...
  if(isDefined(var0) && var0 != "") {
    ...
    if(ref_132d6()) {
      ...
      var1 = getDvar("cp_landlord_start_obj", "player_infil_landlord");
    } else {
      var1 = getDvar("cp_landlord_start_obj", "safehouse_landlord");
    }

    if(var1 == "") {
      var1 = "safehouse_landlord";
    }
...
function create_level_funcs_tables_and_vars() {
  ...
  level.map_interaction_func = &scripts\cp\maps\cp_landlord\cp_landlord_interactions::register_interactions;
  level.player_interaction_monitor = &scripts\cp\maps\cp_landlord\cp_landlord_interactions::level_specific_player_interaction_monitor;
  level.wait_for_interaction_func = &scripts\cp\maps\cp_landlord\cp_landlord_interactions::level_specific_wait_for_interaction_triggered;
...
function levelobjectives_init() {
  level.objectives_table = "cp/cp_landlord_objectives.csv";
...
function init_global_systems() {
  ...
  level thread scripts\cp\intel\cp_intel::init_intel_pieces("landlord");
...
function ref_124ba(var0) {
  if(scripts\cp\gametypes\cp_survival::ref_124b9()) {
    scripts\cp\cp_objectives::overridenextstep(var0, "safehouse_landlord");
    return;
  }
...
function setup_create_script() {
  ...
  register_cs_inits();
  register_create_script_arrays("cp_donetsk_landlord_helicopters", "cp_donetsk_landlord_helicopters", level.scripted_spawner_func.size, &scripts\cp\maps\cp_landlord\cp_donetsk_landlord_helicopters::main);
...
function register_cs_inits() {
  ...
  scripts\cp\cp_create_script_utility::set_cs_file_dvar("cp_donetsk_landlord_helicopters");

 ...
 * Script: scripts\cp\maps\cp_smuggler\cp_smuggler.gsc
*******************************************************/

function main() {
  level.default_player_spawns = "default_spawn_" + level.script;
  scripts\cp\cp_compass::setupminimap("compass_map_cp_smuggler");
  scripts\cp\maps\cp_smuggler\cp_smuggler_precache::main();
  scripts\cp\maps\cp_smuggler\gen\cp_smuggler_art::main();
  scripts\cp\maps\cp_smuggler\cp_smuggler_fx::main();
  scripts\cp\maps\cp_smuggler\cp_smuggler_lighting::main();
  setDvar("PKKMTTRQO", 8);
  ...
  if(isDefined(var0) && var0 != "") {
    ...
    var1 = getDvar("cp_smuggler_start_obj", "safehouse_suburbs11_mnu");

    if(var1 == "") {
      var1 = "safehouse_suburbs11_mnu";
    }

    setDvar("cp_smuggler_start_obj", var1);
...
function incrementalrespawnpunishmax() {
  ...
  var0 settransientsoundbank("cp_op_smuggler.all", 1);
}

function create_level_funcs_tables_and_vars() {
  level.objectivesfunc = &levelobjectives_init;
  level.map_interaction_func = &scripts\cp\maps\cp_smuggler\cp_smuggler_interactions::register_interactions;
  level.player_interaction_monitor = &scripts\cp\maps\cp_smuggler\cp_smuggler_interactions::level_specific_player_interaction_monitor;
  level.wait_for_interaction_func = &scripts\cp\maps\cp_smuggler\cp_smuggler_interactions::level_specific_wait_for_interaction_triggered;
  level.custom_onspawnplayer_func = &onplayerspawned;
  level.custom_onplayerconnect_func = &onplayerconnect;
  level.weapon_rank_event_table = "scripts/cp/maps/cp_smuggler/cp_smuggler_weaponrank_event.csv";
...
function levelobjectives_init() {
  level.objectives_table = "cp/cp_smuggler_objectives.csv";
...
function init_global_systems() {
  ...
  scripts\vehicle\mkilo23_ai_infil::main("veh8_mil_lnd_mkilo23_physics_mp", "mkilo_physics_cp", "script_veh8_mil_lnd_mkilo23_physics_ai_infil");
  ...
  level thread scripts\cp\intel\cp_intel::init_intel_pieces("smuggler");
...
function onplayerconnect(var0) {
  ...
  var0 thread scripts\cp\coop_personal_ents::player_run_pent_updates(var0);
...
function player_exfil_struct() {
  var0 = getEntArray("clip256x256x256", "targetname")[0];
  var1 = spawn("script_model", (16790, 21198.5, 293));
  var1.angles = (0, 330, 90);
  var1 clonebrushmodeltoscriptmodel(var0);
}

function ref_13528() {
  var0 = spawn("script_model", (2165.46, 29700.2, 184));
  var0.angles = (0, 62.6779, 0);
  var0 setModel("debris_override_deadbody_safehouse_male_04_cn");
}
...
function objective_convoy_start(var_0) {
  ...
  foreach(var_4 in level.globalobjectives) {
    if(var_4.objname == "objective_heli_down_start") {
      var_2 = var_4;
      break;
    }
  }
  ...
  var_6 = "small";
  var_7 = level scripts\cp\cp_convoy_manager::spawn_convoy_from_type("convoy1", var_6, "helidown2", var_2, var_1, "objective_convoy_2");
  var_7 endon("event_convoy_delete");
...
function set_proto_values(var_0, var_1) {
  if(getdvarint("scr_convoy_roam", 0) == 0) {
    return;
  }
  ...
  var_5 = level scripts\cp\cp_convoy_manager::spawn_convoy_from_type("convoy1", var_3, var_2, var_0, var_1, var_4);
  ...
  if(var_3 == "small-roaming") {
    var_5 thread scripts\cp\cp_convoy_manager::attach_smuggler_loot("barrel", 4, 5);
    var_5 thread scripts\cp\cp_convoy_manager::keep_smuggler_loot_on_death("barrel", 1, 1);
  } else if(var_3 == "medium-roaming") {
    var_5 thread scripts\cp\cp_convoy_manager::attach_smuggler_loot("barrel", 7, 7);
    var_5 thread scripts\cp\cp_convoy_manager::keep_smuggler_loot_on_death("barrel", 2, 3);
  }
...
function convoy_end_this_event(var_0, var_1) {
  var_0 waittill("fully_removed");
  level scripts\cp\cp_objectives_events::stop_event(var_1);
}

function get_far_away_startpath() {
  var_0 = scripts\engine\utility::getStruct("convoy_start_helidown2", "targetname");
  var_1 = scripts\engine\utility::getStruct("convoy_start_airport1", "targetname");
  var_2 = scripts\engine\utility::getStruct("convoy_start_dam1", "targetname");
  var_3 = scripts\engine\utility::getStruct("convoy_start_shipping1", "targetname");
  var_4 = [];
  GscBinSkip0(0x2e, var_4.size, var_0);
}
...
function convoy_go_to_helidown_location() {
  ...
  wait 0.05;
  level.all_convoys["convoy1"] thread scripts\cp\cp_convoy_manager::set_convoy_target(var_5, var_3);
...
function convoy_attempt_pickup() {
  ...
  var_0 = level.all_convoys["convoy1"] scripts\cp\cp_convoy_manager::get_convoy_targeted_hvt();
...
function set_convoy_path_from_helidown() {
  ...
  if(isDefined(level.last_helidown_loc)) {
    ...
    var_3 = getdvarint("scr_helidown_loc");
...
function convoy_set_helidown_pickup_hvt(var_0) {
  if(var_0 == "harness") {
    if(!isDefined(self.carried)) {
      level waittill("player_picked_up_hostage");
    }
  } else if(var_0 == "escort") {

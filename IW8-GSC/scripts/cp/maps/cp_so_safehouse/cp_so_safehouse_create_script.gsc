 ...
 * Script: scripts\cp\maps\cp_so_safehouse\cp_so_safehouse_create_script.gsc
*****************************************************************************/

function main(var_0, var_1) {
  level endon("game_ended");
  var_2 = spawnStruct();
  scripts\engine\utility::flag_init("cp_so_safehouse_create_script");
...
function cs_return_and_wait_for_flag(var_0, var_1, var_2, var_3) {
  ...
  if(istrue(var_0)) {
    level thread scripts\cp\cp_create_script_utility::wait_for_flags(var_2, "cp_so_safehouse_create_script");
    return;
  }

  scripts\cp\cp_create_script_utility::wait_for_flags(var_2, "cp_so_safehouse_create_script");
}

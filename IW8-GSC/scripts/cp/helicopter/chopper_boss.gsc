 ...
 * Script: scripts\cp\helicopter\chopper_boss.gsc
**************************************************/
...
function init() {
  ...
  level.challengesallowed = level.matchmakingmatch && !getdvarint("LSTLQTSSRM") || getdvarint("OSPNSPSKL") || getdvarint("debug_challenges");
...
function clean_up_structs() {
  if(isDefined(level.struct_filter_kvps)) {
    var0 = getarraykeys(level.struct_filter_kvps);
  ...
  for(var1 = 0; var1 < var2.size; var1++) {
    if(isDefined(var2[var1])) {
      level.struct[level.struct.size] = var2[var1];
    }
  }
}
...
function setupcallbacks() {
  ...
  level.endgame = &scripts\cp\cp_endgame::ref_13445;
  level.forceendgame = &scripts\cp\cp_endgame::ref_13446;
}
...
function coopstartgametype() {
  ...
  if(isDefined(level.scripted_spawner_func)) {
    ...
    if(isarray(level.scripted_spawner_func)) {
      foreach(var1 in level.scripted_spawner_func) {
        [[var1]](1, "cs" + var2);
      }
...
function track_forward_velocity() {
  ...
  for(;;) {
    ...
    var1 = [];
    var2 = [1024 * min(length(var0) / 240, 1)];

    if(var0 != (0, 0, 0)) {
      var1 = var0;
    }

    var3 = int(min(19, self.velo_array.size));
    var4 = var2[0];

    for(var5 = 0; var5 < var3; var5++) {
      var1 = self.velo_array[var5];
      var2 = self.mag_array[var5];
      var4 += self.mag_array[var5];
    }

    self.velo_array = var1;
    self.mag_array = var2;
    self.average_mag = var4 / self.mag_array.size;
...
function spawnplayer_actual(var0) {
  ...
  if(istrue(self.bspawningviaac130)) {} else if(!scripts\cp\cp_endgame::gamealreadyended() && !istrue(var0)) {}
  ...
  if(var0) {
    self[[level.custom_giveloadout]](var0);
  }
...
function getspawnorigin(var0, var1) {
  ...
  if(!var3 && isDefined(var0.forcespawnorigin)) {
...
function player_role_monitor(var0) {

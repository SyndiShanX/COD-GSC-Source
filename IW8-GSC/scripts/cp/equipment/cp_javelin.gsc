...
function javelin_scanforvehicletarget() {
  var0 = scripts\cp\cp_weapon::lockonlaunchers_gettargetarray();
  ...
  if(var0.size != 0) {
    ...
    foreach(var2 in var0) {
      var6 = self worldpointinreticle_rect(var2.origin, 35, 120, 80);
    ...
    if(var4.size != 0) {
      var8 = sortbydistance(var4, self.origin);

      if(javelin_vehiclelocksighttest(var8[0])) {
        return var8[0];
      }
    }
  }
...
function javelin_getvehicleoffset(var0) {
  ...
  if(scripts\cp\utility\entity::ischoppergunner(var0)) {
    var1 = (0, 0, -50);
  } else if(scripts\cp\utility\entity::issupporthelo(var0)) {
    var1 = (0, 0, -100);
  } else if(scripts\cp\utility\entity::isgunship(var0)) {
    var1 = (0, 0, 50);
  } else if(scripts\cp\utility\entity::isclusterstrike(var0)) {
    var1 = (0, 0, 40);
  } else if(scripts\cp\utility\entity::isradardrone(var0)) {
    var1 = (0, 0, 10);
  } else if(scripts\cp\utility\entity::isscramblerdrone(var0)) {
    var1 = (0, 0, 10);
  } else if(scripts\cp\utility\entity::isradarhelicopter(var0)) {
    var1 = (0, 0, -30);
  }

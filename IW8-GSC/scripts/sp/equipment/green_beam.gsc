...
function laser_init_simple(var_0, var_1) {
  laser_init((0, 0, 0), 0, var_0, var_1, 1);
}

function laser_init(var_0, var_1, var_2, var_3, var_4) {
...
function player_green_beam(var_0, var_1) {
...
function laser_player_instructions(var_0, var_1) {
  if(isDefined(var_1)) {
    level endon(var_1);
  }
...
function take_green_beam(var_0) {
...
function check_target_in_hit_zone(var_0) {
...
function drone_origin_updater(var_0) {
  var_1 = level.player.origin + (0, 0, 500);
  var_1 += anglesToForward(level.player.angles) * -1500;
  var_1 += anglestoleft(level.player.angles) * 1500;
  ...
  while(var_3 < 12) {
    ...
    var_1 += anglestoright(level.player.angles) * 1000;
    var_3++;

    if(var_3 % 4 == 0) {
      var_1 += (0, 0, 1000);
      var_1 += anglestoleft(level.player.angles) * 4000;
    }
  }
}

function fire_drone_rocket(var_0) {
  scripts\engine\utility::flag_set("laser_armed");
  ...
  wait 0.5;
  take_beam_ammo(1);
  fire_drone_rocket_internal(level.drone, var_1, var_0["entity"]);
  scripts\engine\utility::flag_clear("laser_armed");
}

function fire_drone_rocket_internal(var_0, var_1) {
  var_2 = undefined;
...
function hellfire_rocket_launch_sfx(var_0, var_1) {
  var_2 = 0;
...
function rocket_ff_check(var_0) {
  var_1 = getaiarrayinradius(var_0, 500, "allies").size;
  var_2 = getaiarrayinradius(var_0, 500, "axis").size;
  level waittill("hellfire_impact");

  if(var_1 > 0) {
    level.player.participation += level.friendlyfire["friend_kill_points"] * var_1 - level.friendlyfire["friend_kill_points"] * var_2;
    scripts\sp\friendlyfire::friendly_fire_checkpoints(0);
    return;
  }
}

function track_hellfire_kills(var_0) {
  var_1 = spawnStruct();
  var_1.killcount = 0;
...
function track_hellfire_vehicle_kills(var_0) {
  for(;;) {
    ...
    if(!isDefined(var_3) || var_3.basename == "iw8_projectile_hfoxtrot" || var_3.basename == "hellfire_rocket") {
      ...
      self.vehiclekills++;
      self.killcount += var_0.riders.size;
    }
...
function check_target_is_valid(var_0) {
...
function take_beam_ammo(var_0) {
  level.hellfire_count -= var_0;
...
function give_beam_ammo(var_0, var_1, var_2) {
  if(isDefined(var_2)) {
    level endon(var_2);
  }
...
function green_beam_demeanor() {
  for(;;) {
    ...
    if(!scripts\engine\utility::is_equal(var_0, "normal")) {
      level.player scripts\engine\sp\utility::set_player_demeanor("normal");
    }
  }
}

function set_green_beam_demeanor(var_0) {

...
function init_window() {
  ...
  foreach(var_4 in var_2) {
    if(scripts\engine\utility::is_equal(var_4.script_noteworthy, "delete_on_open")) {
      var_4 delete();
      continue;
    }

    var_13 = var_4.angles;
    var_14 = var_4.origin - level.player.origin;
    ...
    var_19 = anglelerpquatfrac(var_13, var_18, randomfloatrange(1, 1.15));
    var_4 rotateTo(var_19, var_10, var_10 * 0.8, var_10 * 0.2);
    var_4 playSound(var_11);

    if(var_11 == "melee_pressed") {
      ...
      var_4 scripts\engine\utility::delaycall(var_10, &rotateto, var_18, randomfloatrange(0.75, 1.25));
      var_4 scripts\engine\utility::delaycall(var_10, &playsound, "door_open_stop");
      self notify("trigger");
    }
  }

  foreach(var_9 in var_7) {
    if(scripts\engine\utility::is_equal(var_9.targetname, "traverse")) {
      var_22 = getnode(var_9.target, "targetname");
      createnavlink("window_traversal", var_9.origin, var_22.origin, var_9);
      continue;
    }

    if(scripts\engine\utility::is_equal(var_9.script_noteworthy, "cover")) {
      var_9 connectnode();
    }
  }
}
...
function window_melee_valid(var_0) {
...
function init_door_internal(var_0) {
  if(istrue(var_0)) {}
...
function cursor_hint_thread(var_0) {
...
function has_cursor_hint(var_0) {
  if(!isDefined(var_0)) {
    return false;
  }
  ...
  if(isarray(var_0)) {
    ...
    return isDefined(var_3.cursor_hint_ent);
  }

  return false;
}

function adjust_cursor_hint_side(var_0) {
  ...
  var_1 += self.cursorhintdir * var_0.radius;
...
function yaw_collision_check(var_0, var_1, var_2) {
  ...
  var_6 = var_5 + anglesToForward(var_5) * self.length * 0.2;
  var_7 = var_6 + anglestoright(var_5) * var_4;
  var_8 = scripts\engine\trace::capsule_trace(var_6, var_7, 6, 80, var_5, [self, self.clip], var_3, 0);
  var_9 = distance2d(var_6, var_8["position"]);

  if(var_9 > 3) {
    var_6 = var_5 + anglesToForward(var_5) * self.length * 0.9;
    var_8 = scripts\engine\trace::capsule_trace(var_6, var_7, 6, 80, var_5, [self, self.clip], var_3, 0);
    var_9 = distance2d(var_6, var_8["position"]);
    return (var_9 > 5);
  }
...
function draw_max_yaw(var_0) {
  ...
  if(var_0) {
    ...
    var_2 = self.max_yaw_right * -1;
    var_2 = self.pivots["open_right"].origin;
  }

  var_3 = self.true_start_angles + (0, var_2, 0);
...
function set_pivot_point(var_0) {
  if(!can_pivot_change()) {
...
function setup_open_struct(var_0) {
...
function player_door_gesture(var_0) {
...
function player_window_gesture(var_0) {
...
function door_bashable_by_player(var_0) {
...
function bash_debug(var_0) {}

function stealth_broadcast(var_0, var_1) {
...
function combat_getinfoinradius(var_0) {
...
function bashed_locked_door(var_0) {
  level.player endon("death");
...
function monitor_door_push(var_0) {
  self endon("reset_door");
...
function push_door() {
  ...
  var_7 = var_3 * (1 - var_6);
  var_7 *= self.masterdoorratescale;
...
function push_door_player_effects() {
  ...
  for(;;) {
    ...
    var_19 = scripts\engine\math::normalize_value(0.6, 1, abs(var_8));
    ...
    if(level.player adsButtonPressed()) {
      var_28 = scripts\engine\math::normalize_value(0.8, 1, var_28);
    }
    ...
    scripts\sp\player::player_apply_local_weap_position(self.dooroffset * var_28 * 1, 0, var_2);
    scripts\sp\player::player_apply_local_weap_rotation(self.doorrot * var_28 * 1, 0, var_2);
    var_29 = 1;
    var_22 = scripts\engine\math::normalize_value(0, 1, var_13);
...
function createinitialnavmodifier(var_0) {
...
function suspicious_door_stealth_check(var_0) {
  if(!isDefined(level.stealth)) {
    return;
  }
...
function door_is_at_max_yaw(var_0) {
...
function isnavpointaccesiblefrombehinddoor(var_0, var_1) {
  ...
  var_5 = navtrace(var_4, var_1, self, 1);
  var_6 = (0, 1, 0);
...
function print3d_on_me(var_0, var_1) {
  self endon("death");
  var_1 *= 1000;
  var_2 = gettime();
...
function double_doors_hint_pos(var_0) {
  scripts\engine\utility::flag_wait("interactive_doors_ready");
  var_1 = spawnStruct();

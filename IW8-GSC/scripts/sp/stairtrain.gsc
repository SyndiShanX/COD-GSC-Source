...
function stairtrain_thread(var_0, var_1, var_2) {
  ...
  self setflaggedanim(var_0.base_anime, var_0.base_anim, 1, 0.4, 0);
  ...
  for(;;) {
    ...
    if(var_14.animratefrac > 0 && !scripts\engine\utility::flag("stairtrain_nagging")) {
      ...
      if(isDefined(var_0.settle_anim)) {
        if(var_11 == "none") {
          var_11 = "settling";
          self setflaggedanimrestart(var_0.base_anime + "_settle", var_0.settle_anim, 1, 0.2);
      ...
      if(isDefined(var_0.additive_anim)) {
        if(var_11 == "startidle") {
          ...
          var_15 = randomfloatrange(0.7, 1);
          self setflaggedanim(var_0.base_anime + "_additive", var_0.additive_anim, 1, 0.2, var_15);
...
function try_nag(var_0) {
  ...
  if(gettime() > var_0.nagtime) {
    ...
    GscBinSkip1(0x74, var_0.fnnag);
  }
}

function nag_anim(var_0) {
  var_1 = "something";
...
function stairtrain_notetracks(var_0) {
  self endon("death");
  self notify("stairtrain_stop_notetracks");
  ...
  if(isDefined(level.scr_notetrack[var_3])) {
    if(isDefined(level.scr_notetrack[var_3][var_0])) {
      var_1 = level.scr_notetrack[var_3][var_0];
    }

    if(isDefined(level.scr_notetrack[var_3]["any"])) {
      var_1 = level.scr_notetrack[var_3]["any"];
    }
  }
...
function stairtrain_player_data(var_0, var_1) {
  var_2 = spawnStruct();
  ...
  var_9 = get_closest_on_path(level.stairtrain_rearguy.origin, var_5);
  var_10 = get_dist_on_segment(var_9["origin"], var_9["segment"]);

  if(var_10 < 1) {
    var_11 = [];
    GscBinSkip0(0x2e, "segment", var_5.segments[0]);
  }

  if(var_11 < var_7.maxplayerdist) {
    var_11 = get_closest_on_path(level.player.origin, var_6);
    var_13 = distance(level.player.origin, var_10["origin"]);
    var_14 = var_11["segment"][0].radius;
  } else {
    var_11 = get_closest_on_path(level.player.origin, var_9);
    var_12 = get_dist_on_segment(var_11["origin"], var_11["segment"]);
    var_14 = var_11["segment"][0].radius;
    var_13 = abs(var_14 - var_12);
  }
  ...
  if(var_19 < 0 || var_20 < 0) {
    level.stairtrain_rearguy.stairtrain_data = var_11;
    return var_11;
  }
  ...
  if(var_11) {
    ...
    var_18 = vectorNormalize(scripts\engine\utility::flat_origin(level.player.origin) - scripts\engine\utility::flat_origin(var_13["origin"]));
    ...
    var_11.playerdistfrac = scripts\engine\math::lerp_fraction(var_10.minplayerdist, var_10.maxplayerdist, var_13);
    var_11.animratefrac = var_11.playerdistfrac;

    if(var_11.animratefrac < var_10.animfrac_min * 0.5) {
      var_11.animratefrac = 0;
    } else if(var_11.animratefrac < var_10.animfrac_min) {
      var_11.animratefrac = var_10.animfrac_min;
    }

    var_11.animratefrac = clamp(var_11.animratefrac, 0, 1.5);
  } else {
    var_11.playerdistfrac = scripts\engine\math::lerp_fraction(var_10.maxplayerdist, var_10.minplayerdist, var_13);
    var_11.animratefrac = var_11.playerdistfrac;
  ...
  var_11.playerdistfrac = clamp(var_11.playerdistfrac, 0, 1);

  if(var_9.startonpath && var_11.animratefrac < 0.5) {
    if(var_14 < 1) {
      var_11.playerdistfrac = 1;
      var_11.animratefrac = 0.5;
    } else {
      var_9.startonpath = 0;
    }
  }
  ...
  if(!var_11) {
    var_11.playerspeedfrac = scripts\engine\math::lerp_fraction(var_10.minplayerspeeddist, var_10.maxplayerspeeddist, var_13);
    var_11.playerspeedfrac = clamp(var_11.playerspeedfrac, 0, 1);
  }
...
function get_closest_on_path(var_0, var_1) {
  ...
  GscBinSkip1(0x45, "origin", var_5);
}

function get_dist_on_segment(var_0, var_1) {
  return var_1[0].dist + distance(var_0, var_1[0].origin);
}

function get_patharray(var_0) {
  var_1 = [var_0];
  var_2 = 0;
...
function get_segments(var_0) {
  var_1 = [];

  for(var_2 = 0; var_2 < var_0.path.size - 1; var_2++) {
    var_1 = [var_0.path[var_2], var_0.path[var_2 + 1]];
  }

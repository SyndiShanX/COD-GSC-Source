...
function player_update_allowed_callouts() {
  if(!isDefined(anim.player.allowedcallouts)) {
    ...
    anim.player scripts\anim\battlechatter::addallowedthreatcallout("rpg");
    anim.player scripts\anim\battlechatter::addallowedthreatcallout("exposed");
    anim.player scripts\anim\battlechatter::addallowedthreatcallout("acquired");
    anim.player scripts\anim\battlechatter::addallowedthreatcallout("sighted");
    anim.player scripts\anim\battlechatter::addallowedthreatcallout("target_compass");
    anim.player scripts\anim\battlechatter::addallowedthreatcallout("target_distance");
    anim.player scripts\anim\battlechatter::addallowedthreatcallout("target_elev");
    anim.player scripts\anim\battlechatter::addallowedthreatcallout("player_object_clock");
    anim.player scripts\anim\battlechatter::addallowedthreatcallout("player_location");
    anim.player scripts\anim\battlechatter::addallowedthreatcallout("ai_location");
    anim.player scripts\anim\battlechatter::addallowedthreatcallout("generic_location");
    anim.player scripts\anim\battlechatter::addallowedthreatcallout("concat_location");
    return;
  }
}
...
function playerthreadthreader() {
  ...
  var0 = 0.5;
  wait var0;
  thread scripts\anim\battlechatter_ai::aiinformweaponwaiter();
  wait var0;
  thread playerdogfightwaiter();
  wait var0;
  thread playerdamagewaiter();
  wait var0;
  thread scripts\anim\battlechatter_ai::aibattlechatterloop();
}

function playerdamagewaiter() {
  while(isalive(anim.player) && scripts\anim\battlechatter::bcsenabled() && isDefined(anim.player.battlechatterallowed) && anim.player.battlechatterallowed) {
    anim.player waittill("damage", var0, var1);

    if(!scripts\engine\utility::player_is_in_jackal()) {
      if(var1 scripts\anim\battlechatter::bcissniper()) {
        ...
        foreach(var4 in var2) {
          if(isalive(var4) && isai(var4) && distancesquared(anim.player.origin, var4.origin) > 10000) {
            var4 scripts\anim\battlechatter_ai::addthreatevent("infantry", var1, 0.9);
            break;
          }
        }
      }
...
function playerdogfightwaiter() {
  while(!scripts\engine\utility::player_is_in_jackal() && isalive(anim.player) && scripts\anim\battlechatter::bcsenabled()) {
    wait 1;
  }
      ...
      if(isDefined(level.player.dogfighttarget)) {
        if(isDefined(level.player.dogfighttarget._blackboard) && isDefined(level.player.dogfighttarget._blackboard.isevading)) {
          if(level.player.dogfighttarget._blackboard.isevading) {
            ...
            if(isDefined(level.player.dogfighttarget)) {
              anim.player scripts\anim\battlechatter_ai::addreactionevent("movement", "generic", level.player.dogfighttarget, 0.9);
            }
          } else {
            wait randomfloatrange(0.5, 0.75);

            if(isDefined(level.player.dogfighttarget)) {
              anim.player scripts\anim\battlechatter_ai::addthreatevent("acquired", level.player.dogfighttarget);
            }
          }
        }
      }
...
function playervehiclewaiter() {
  ...
  while(isalive(anim.player) && scripts\anim\battlechatter::bcsenabled() && isDefined(anim.player.battlechatterallowed) && anim.player.battlechatterallowed) {
    if(!scripts\engine\utility::player_is_in_jackal()) {
      var1 = scripts\engine\sp\utility::getvehiclearray();
      ...
      foreach(var3 in var1) {
        ...
        if(issubstr(var3.classname, "dropship")) {
          if(isDefined(var3.script_team) && var3.script_team != anim.player.team) {
            if(anim.player scripts\anim\battlechatter::pointinfov(var3.origin) && distancesquared(anim.player.origin, var3.origin) < 4000000) {
              ...
              foreach(var6 in var4) {
                if(isalive(var6) && isai(var6) && distancesquared(anim.player.origin, var6.origin) < 250000) {
                  var6 scripts\anim\battlechatter_ai::addinformevent("incoming", "dropship", 0.9, "vehicle");
                  var0 = var3;
                }
              }
...
function player_battlechatter_generic_event_check() {
  ...
  for(;;) {
    ...
    if(var2 != var0 && var2 != "pc_clear_last_event") {
      anim.player scripts\anim\battlechatter::playbattlechatter(var2);
      var0 = var2;

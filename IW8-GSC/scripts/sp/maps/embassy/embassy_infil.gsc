 ...
 * Script: scripts\sp\maps\embassy\embassy_infil.gsc
*****************************************************/

function embassy_infil_precache() {
...
function embassy_infil_precache_delay() {
  wait 0.3;
...
function embassy_infil_fx() {}

function embassy_infil_init() {
  ...
  level.alley_life_kids_door = scripts\sp\maps\embassy\embassy_util::setup_office_door("alley_life_kids_door", 1);
...
function temp_print_screen_type() {
  for(;;) {
    waitframe();
  }
}

function set_monitor_script_noteworthy() {
  ...
  if(scripts\engine\utility::array_contains(level.lower_floors_scriptables, self)) {
    switch (randomint(10)) {
      case 3:
      case 2:
      case 1:
...
function set_tv_script_noteworthy() {
  if(scripts\engine\utility::array_contains(level.offices_scriptables, self)) {
    switch (randomint(10)) {
      case 4:
      case 3:
...
function embassy_infil_scriptables_grab() {
...
function hide_crawl_forward_hint() {
  return level.player scripts\sp\maps\embassy\embassy_util::moving_forward() || scripts\engine\utility::flag("missionfailed");
}
...
function embassy_infil_objectives() {
...
function heli_crash_debris_setup() {
...
function heli_crash_debris_pre() {
...
function heli_crash_debris_post() {
...
function heli_crash_debris_post_roof() {
...
function helicopter_main() {
  ...
  scripts\sp\maps\embassy\embassy_util::put_player_into_rig(level.player.rig, 0, 0, 0, 0, 0);
  thread audio_crowd_angry_outside();
  scripts\sp\maps\embassy\embassy_util::make_player_and_price_story_only();
  ...
  thread scripts\sp\maps\embassy\embassy_lighting::lt_infil_helicopter_start();
  thread gate_overrun();
  thread crowd();
...
function heli_kyle_third_person() {
...
function audio_office_alarm_loop_start() {
...
function audio_post_truck_office_alarm_loop_start() {
...
function audio_bpg_entrance_alarm_loop_start() {
...
function audio_crowd_cheer_at_end_of_infil_crash() {
...
function crowd_setup_ap() {
  ...
  level.crowd_ap.angles += (0, -90, 0);
}

function temp_draw_crowd_num() {
...
function crowd_change_anim_at_offices() {
...
function crowd_individual_vehicle() {
  level.crowd_ap scripts\common\anim::anim_single_solo(self, "crowd_enter");
  self delete();
}
...
function gate_overrun() {
  spawn_gate_overrunners();
  scripts\sp\maps\embassy\embassy_util::spawn_aq_enforcer();
...
function gate_overrun_fire_rate_controller() {
  for(;;) {
    jumpiffalse(!isDefined(level.gate_overrunners_marine) || !isDefined(level.gate_overrunners_aq)) LOC_00000019;
    waitframe();
  }
...
function heli_shake_controller() {
  thread scripts\sp\maps\embassy\embassy_util::heli_shake_door_closed();
  level waittill("price_opens_door");
  thread scripts\sp\maps\embassy\embassy_util::heli_shake_door_open();
  level waittill("rocket_fire");
  wait 1.5;
...
function wait_variables_initialized() {
...
function helicopter_crawl_main() {
  ...
  thread scripts\sp\maps\embassy\embassy_util::heli_shake_spin();

  if(!level.player scripts\sp\maps\embassy\embassy_util::moving_forward()) {
    level.infil_heli thread scripts\common\anim::anim_loop_solo(level.player.rig, "infil_crash_crawl_a_idle", "stop_loop_player");
    thread display_heli_hint(level.infil_heli, "hint_crawl_forward");
    level.player scripts\sp\maps\embassy\embassy_util::wait_moving_forward();
  ...
  if(!level.player jumpbuttonPressed() && !level.player useButtonPressed()) {
    ...
    level notify("do_rope_nag");
    thread display_heli_hint(level.infil_heli, "hint_jump");
    level.player scripts\sp\maps\embassy\embassy_util::wait_player_jumping();
    level.infil_heli notify("stop_loop_player");
  }
  ...
  level.pilot unlink();
  level.price thread scripts\sp\maps\embassy\embassy_util::halligan_stow();
}

function grab_rope_nags() {
...
function helicopter_crash_main() {
  ...
  if(!istrue(level.crash_start_point)) {
    ...
    level.scr_goaltime["infil_heli"]["infil_crash_fall"] = 0;
  }
  ...
  thread helicopter_crash_fall_relative_to_world();
  thread helicopter_crash_fall_relative_to_world();
...
function helicopter_crash_fall_relative_to_world() {
  ...
  if(self == level.price) {
    thread helicopter_crash_fall_price_gun();
...
function helicopter_crash_lerp_ap() {
  wait 1.1;
...
function helicopter_player_fall(var_0) {
  ...
  thread scripts\sp\maps\embassy\embassy_util::flag_on_crouch_pressed("fall_impact_done");
  thread scripts\sp\maps\embassy\embassy_util::flag_on_sprint_pressed("fall_impact_done");
...
function helicopter_crash_vfx() {
  thread scripts\sp\maps\embassy\embassy_util::heli_shake_rope();
...
function helicopter_crash_rack_focus() {
  ...
  level.player setphysicaldepthoffield(1.75, 1800);
  level waittill("rack_to_price");
  level.player setphysicaldepthoffield(3.45, 42.5, 0.25, 0.5);
  level waittill("rack_to_zero");
  level.player setphysicaldepthoffield(22, 70, 0.5, 1);
...
function heli_player_damage_radius() {
  level endon("stop_damage_radius");

  for(;;) {
    physicsexplosionsphere(level.player.origin + anglestoleft(level.player.angles) * 1, 50, 50, 200);
...
function heli_wreckage_break() {
  self endon("entitydeleted");
  level.ap_infil_crash scripts\common\anim::anim_single_solo(self, "infil_crash_fall");
  self delete();
}

function helicopter_crash_catchup() {
  heli_crash_debris_post();
  heli_crash_debris_post_roof();
  scripts\engine\utility::exploder("heli_fire");
  scripts\engine\utility::exploder("heli_palm_fire");
...
function notetrack_nag(var_0, var_1) {
  ...
  for(;;) {
    ...
    if(scripts\sp\maps\embassy\embassy_util::player_moving_toward()) {
      continue;
    }
...
function offices_main() {
  ...
  thread scripts\sp\maps\embassy\embassy_util::swap_card_reader("office_card_reader");
  thread offices_door();
...
function stop_office_walla() {
  level.office_walla_1 scripts\engine\sp\utility::sound_fade_and_delete(2, 1);
  level.office_walla_2 scripts\engine\sp\utility::sound_fade_and_delete(2, 1);
}
...
function offices_individual_prop() {
  self endon("entitydeleted");
  self endon("death");
  ...
  if(self.animname == "offices_prop_18") {
    ...
    self setanimrate(scripts\engine\utility::getanim("offices_chaos_idle")[0], 0);
    level waittill("key_guy_second_loop");
    self setanimrate(scripts\engine\utility::getanim("offices_chaos_idle")[0], 1);
...
function offices_idle_only_civ() {
...
function offices_setup_door() {
  level.offices_door = scripts\sp\maps\embassy\embassy_util::setup_office_door("office_entrance_door");
...
function offices_door() {
  level.ap_offices_chaos scripts\common\anim::anim_single_solo(level.offices_door, "offices_chaos");
  level.offices_door scripts\sp\maps\embassy\embassy_util::connect_office_door_paths();
}

function offices_chaos_civs() {
  self.fakeactor_face_anim = 1;
...
function stairwell_main() {
  thread scripts\sp\maps\embassy\embassy_util::swap_card_reader("stairway_card_reader");
...
function mus_beadvised() {
  wait 0.5;
}

function stairwell_molotov() {
  scripts\engine\utility::flag_wait("stair_b");
...
function audio_start_stairwell_car_scene() {
  level.stairwell_crowd_car_sound = spawn("script_origin", (-7939, -2080, 163));
  level.stairwell_crowd_car_sound thread scripts\engine\sp\utility::sound_fade_in("embassy_stairwell2_auto_scene_lp", 1, 2, 1);
...
function bp_glass_metal_detectors_start() {
  scripts\sp\maps\embassy\embassy_util::spawn_price();
...
function bpg_md_cowering_civ() {
  self.fakeactor_face_anim = 1;
  self.animname = "civ";
  thread scripts\sp\maps\embassy\embassy_util::civ_friendly_fire_think();
  thread scripts\common\anim::anim_loop_solo(self, scripts\sp\maps\embassy\embassy_util::get_civ_cower_anim());
  scripts\engine\utility::flag_wait("bpg_scene_clear_exit_door");
  self delete();
}
...
function bpg_md_aq() {
  ...
  scripts\sp\maps\embassy\embassy_util::spawn_aq_enforcer_entourage();
...
function bpg_md_aq_stack_up_at_door() {
...
function bpg_md_price() {
  ...
  level.ap_bpg_scene scripts\sp\maps\embassy\embassy_util::anim_reach_solo_skip_offscreen(level.price, "bpg_scene_entrance", undefined, undefined, 0);
...
function bp_glass_metal_detectors_catchup() {
...
function bp_glass_scene_main() {
  ...
  level.ap_bpg_scene.door_1 = scripts\sp\maps\embassy\embassy_util::setup_office_door("bpg_scene_sliding_door_l");
  ...
  level.ap_bpg_scene.door_2 = scripts\sp\maps\embassy\embassy_util::setup_office_door("bpg_scene_sliding_door_r");
...
function bpg_scene_player_interact() {
  level.ap_bpg_scene scripts\common\anim::anim_first_frame_solo(level.player.rig, "bpg_scene_door_enter");
  thread scripts\sp\maps\embassy\embassy_util::put_player_into_rig(level.player.rig, 0.3, 0, 0, 0, 0);
  ...
  if(scripts\engine\utility::flag("bpg_scene_interact_success")) {
    ...
    level.price setanimrate(level.price scripts\engine\utility::getanim("bpg_scene"), 0);
    level.price hide();
  ...
  thread scripts\sp\maps\embassy\embassy_util::pull_player_out_of_rig_hide_rig(level.player.rig);
}

function bpg_scene_player_interact_aq_enforcer_entourage() {
...
function bpg_scene_player_interact_death() {
...
function mus_enforcer_interact() {}

function bpg_scene_key_civ() {
...
function bpg_scene_civ_otherside() {
  thread scripts\sp\maps\embassy\embassy_util::hide_offscreen_shadow(800);
  self.fakeactor_face_anim = 1;
...
function bpg_scene_check_door_interact() {
  ...
  thread scripts\sp\maps\embassy\embassy_util::cursor_hint_unusable_think();
...
function pre_truck_office_cleanup() {
  scripts\sp\maps\embassy\embassy_util::make_player_and_price_non_story();
...
function truck_office_main() {
  level.player setsoundsubmix("sp_npc_steps_down", 4);
...
function audio_start_pre_impact_group_walla() {
  level.truck_crowd_walla_loop = spawn("script_origin", (-6814, -3750, 116));
  level.truck_crowd_walla_loop thread scripts\engine\sp\utility::sound_fade_in("embassy_truck_crowd_before_impact", 1, 3, 1);
  scripts\engine\utility::flag_wait("truck_wreck_start");
...
function audio_aq_run_behind_truck() {
  wait 1.5;
  ...
  level.aq_run_behind_truck thread scripts\engine\sp\utility::sound_fade_in("embassy_truck_guys_rushing_in_loop", 1, 1.5, 1);
  level.aq_run_behind_truck moveTo((-7401, -2978, 92), 12, 0.2, 0.2);
  wait 10;
  scripts\engine\utility::flag_wait_any("wall_truck_enemies_defeated", "perfect_info");
  level.aq_run_behind_truck moveTo((-7736, -2890, 92), 10, 0.2, 0.2);
  wait 10;
  audio_wait_until_player_close_enough_to_obj();
  level.aq_run_behind_truck moveTo((-7736, -2561, 92), 5, 0.2, 0.2);
  wait 3;
  audio_wait_until_player_close_enough_to_obj(485);
  level.aq_run_behind_truck moveTo((-8077, -2561, 92), 5, 0.2, 0.2);
  wait 3;
  audio_wait_until_player_close_enough_to_obj(450);
  level.aq_run_behind_truck moveTo((-8137, -2272, 92), 5, 0.2, 0.2);
  wait 2;
  level.aq_run_behind_truck stoploopsound(5);
  audio_wait_until_player_close_enough_to_obj(450);
  level.aq_run_behind_truck moveTo((-7675, -2242, 117), 5, 0.2, 0.2);
  wait 2;
  audio_wait_until_player_close_enough_to_obj(600);
  level notify("now_thru_basement_door");
}

function audio_aq_run_behind_truck_one_shots() {
...
function truck_office_price_move_to_marine() {
  ...
  level.price thread scripts\sp\maps\embassy\embassy_util::go_to_node_targetname("price_pre_truck");
...
function truck_office_setup_truck() {
...
function truck_office_setup_walls() {
...
function truck_office_hide_all() {
...
function truck_office_delete_all() {
...
function truck_office_walls_pre() {
...
function truck_office_truck() {
  truck_office_walls_pre();
  scripts\engine\utility::flag_wait("truck_wreck_start");
  thread truck_office_walls_post(1);
  thread scripts\sp\maps\embassy\embassy_lighting::lt_truck_crash_start();
...
function truck_office_wreck_vfx() {
...
function truck_office_wreck_reaction_gesture() {
  wait 0.4;
...
function truck_office_enemy_death_counter() {
  self waittill("death");
...
function truck_office_aq_driver() {
  self endon("death");
...
function truck_office_aq_rider() {
  self endon("death");
  scripts\engine\sp\utility::set_allowdeath(1);
  level.ap_truck_office scripts\common\anim::anim_single_solo(self, "to_attack_scene_ride");
  scripts\sp\spawner::go_to_node();
...
function truck_office_bpg_scene_marine_move_to_marine() {
  level endon("truck_wreck_start");
  level.bpg_scene_marine scripts\common\utility::demeanor_override("cqb");
  level.bpg_scene_marine thread scripts\sp\maps\embassy\embassy_util::go_to_struct_targetname("bpg_scene_marine_pre_truck");
...
function truck_office_bpg_scene_marine() {
  scripts\engine\utility::flag_wait("truck_wreck_start");
  level.bpg_scene_marine.health = 1;
  level.bpg_scene_marine scripts\common\utility::demeanor_override("combat");
  level.bpg_scene_marine thread scripts\sp\maps\embassy\embassy_util::go_to_node_targetname("bpg_scene_marine_post_truck");
  level waittill("marine_dialogue_done");
  wait 0.5;
...
function truck_office_crash_marine_vo() {
  self endon("death");
  wait 2.4;
...
function truck_office_crash_price_vo() {
  while(scripts\engine\utility::array_removedead_or_dying(getaiarray("axis")).size > 0) {
    level waittill("ai_killed");
  }

  wait 0.8;
  scripts\sp\maps\embassy\embassy_util::wait_combat_cooldown(0.8, 2);
  level.price scripts\sp\maps\embassy\embassy_util::say_as_chatter("dx_vom_pri_breach_vehicle_90");
  wait 1;
  level.price scripts\sp\maps\embassy\embassy_util::say_as_chatter("dx_vom_pri_breach_vehicle_100");
  wait 0.2;
  level scripts\sp\maps\embassy\embassy_util::say_as_chatter("dx_vom_alx_breach_vehicle_110");
...
function interrupt_marine_vo() {
  self waittill("death");
  self stopsounds();
}

function truck_office_civs() {
  scripts\engine\sp\utility::trigger_wait_targetname("wall_truck_civs_trigger");
  scripts\engine\utility::flag_wait("bpg_scene_doors_closed");
...
function cleanup_wall_truck_civ() {
  self endon("death");
  self endon("entitydeleted");

  if(isDefined(self)) {
    while(scripts\sp\maps\embassy\embassy_util::within_player_fov(self.origin) || distance2dsquared(level.player.origin, self.origin) < squared(200)) {
      wait 0.1;
    }
...
function bp_glass_combat_start() {
  scripts\sp\maps\embassy\embassy_util::spawn_price();
...
function bp_glass_combat_main() {
  thread scripts\sp\maps\embassy\embassy_lighting::setup_bpg_combat();
...
function bpg_combat_price() {
  ...
  if(scripts\engine\utility::flag("bpgc_price_1a")) {
    level.price thread scripts\sp\maps\embassy\embassy_util::go_to_struct_targetname("price_1_right");
  } else {
    level.price thread scripts\sp\maps\embassy\embassy_util::go_to_node_targetname("price_1_left");
  }

  scripts\engine\utility::flag_wait_any("bpgc_price_2a", "bpgc_price_2b");

  if(scripts\engine\utility::flag("bpgc_price_2a")) {
    level.price thread scripts\sp\maps\embassy\embassy_util::go_to_struct_targetname("price_2_right");
  } else if(scripts\engine\utility::flag("bpgc_price_1b")) {
    level.price thread scripts\sp\maps\embassy\embassy_util::go_to_struct_targetname("price_2_left");
  } else {
    level.price thread scripts\sp\maps\embassy\embassy_util::go_to_node_targetname("price_2_left_cover");
  }
  ...
  level.price thread scripts\sp\maps\embassy\embassy_util::go_to_node_targetname("bpgc_price_basement_hallway");
}

function bpg_combat_wait_autosave() {
  scripts\engine\utility::flag_wait("perfect_info");
  scripts\engine\sp\utility::autosave_by_name("bpg_combat_start");
}

function bp_glass_hint_flash() {
  scripts\engine\utility::flag_wait("truck_cleared");
...
function bp_glass_combat_enemies() {
...
function bpg_first_wave_enemy() {
  self endon("death");
  self endon("entitydeleted");
  thread scripts\sp\maps\embassy\embassy_util::dont_shoot_through_bp_glass();
  thread scripts\sp\maps\embassy\embassy_util::check_and_kill("bpgc_price_to_basement", "bpgc_kill_node");
...
function bpg_window_break_through() {
  self endon("death");
  self endon("entitydeleted");
  thread scripts\sp\maps\embassy\embassy_util::dont_shoot_through_bp_glass();
  thread scripts\sp\maps\embassy\embassy_util::check_and_kill("bpgc_price_to_basement", "bpgc_kill_node");
...
function bpgc_hostage_bullet_shield() {
  self endon("death");
  self endon("entitydeleted");
  self endon("stop_checking_damage");
  self.other endon("death");
...
function bpgc_hostage_check_damage() {
  self endon("stop_checking_damage");
...
function bpg_closers() {
  self endon("death");
  self endon("entitydeleted");

  for(;;) {
    thread scripts\sp\maps\embassy\embassy_util::close_in_on_player();
...
function bpg_individual_civ_flee() {
  self endon("death");
  self endon("entitydeleted");
  scripts\sp\spawner::go_to_node(scripts\engine\utility::random(level.wall_truck_civ_hiding_spots));
...
function bpgc_halligan_breakout_vo() {
  wait 0.8;
  level.price scripts\sp\maps\embassy\embassy_util::say("dx_vom_pri_basement_halligan_60", 1);
}
...
function bp_glass_combat_catchup() {
  if(!scripts\sp\starts::is_after_start("infil_saferoom")) {
    thread saferoom_screens();
    return;
  }
...
function basement_override_price_demeanor() {
  if(!scripts\sp\maps\embassy\embassy_util::within_distance(level.player.origin, level.price.origin, 250)) {
...
function basement_price() {
  level endon("basement_aq_defeated");
  scripts\engine\utility::flag_wait("bas_price_stairs");
  level.price thread scripts\sp\maps\embassy\embassy_util::go_to_node_targetname("bas_price_stairs_cover");
  level.price notify("stop_loop_price");
  ...
  level.price scripts\sp\maps\embassy\embassy_util::go_to_node_targetname("bas_price_hallway_cover");
  scripts\engine\utility::flag_wait("bas_price_boiler");
  level.price scripts\sp\maps\embassy\embassy_util::go_to_node_targetname("bas_price_boiler_cover");
}
...
function basement_sr_crowguy() {
  ...
  scripts\sp\maps\embassy\embassy_util::wait_scene_on_screen_and_visible_flag([self], "reached_saferoom");
...
function basement_sr_last_stand() {
  self endon("death");
  self endon("entitydeleted");
...
function saferoom_setup_doors_and_ap() {
  level.ap_saferoom = scripts\engine\utility::getStruct("ap_saferoom", "targetname");
  level.sr_entrance_door = scripts\sp\maps\embassy\embassy_util::setup_office_door("sr_entrance_door");
...
function saferoom_main() {
  ...
  thread scripts\sp\maps\embassy\embassy_lighting::lt_saferoom_wolf_on();
  thread mus_saferoom();
  ...
  thread saferoom_scene_then_idle(level.farah);
  thread saferoom_scene_then_idle(level.hadir);
  thread saferoom_scene_then_idle(level.wolf);
  ...
  level scripts\engine\utility::delaythread(4.5, &scripts\sp\maps\embassy\embassy_util::say_as_chatter, "dx_vom_amba_cctv_01_intro_20", 1);
  ...
  thread scripts\sp\maps\embassy\embassy_util::put_player_into_rig(level.player.rig, 0.25, 0, 0, 0, 0);
  level.ap_cctv thread scripts\common\anim::anim_single_solo(level.cctv_mic, "cctv_enter");
  thread scripts\sp\maps\embassy\embassy_lighting::lt_safehouse_mic();
  ...
  scripts\engine\utility::delaythread(3, &saferoom_escape_screens);
  scripts\sp\utility::notetrack_mission_failed_vo_enable();
}

function load_cctv_transient() {
  waitframe();
  loadtransient("embassy_building_cctv_tr");
}

function saferoom_player_uses_buzzer() {
...
function mus_saferoom() {
  wait 0.1;
  setmusicstate("");
}

function saferoom_price_reach_or_skip() {
...
function saferoom_teleport_price() {
...
function saferoom_price_behind_catchup() {
...
function saferoom_scene_then_idle(var_0) {
  ...
  if(self != level.wolf) {
    for(;;) {
      if(scripts\sp\maps\embassy\embassy_util::within_distance(level.player.origin, self.origin, 50)) {
        level.ap_saferoom notify("stop_loop_" + self.animname);
...
function saferoom_door_delay_notify_for_keypad() {
  wait 0.1;
  level notify("card_reader_swap");
}
...
function escape_main() {
  ...
  level.player thread scripts\sp\player::player_movement_state("cqb");
  level.sr_exit_door.angles += (0, 120, 0);
  ...
  thread scripts\sp\maps\embassy\embassy_lighting::lt_escape_start();
  ...
  thread scripts\sp\maps\embassy\embassy_lighting::lt_garage_keycard();
  level.sr_exit_door.angles -= (0, 120, 0);
}

function audio_cctv_mix_clear() {
...
function disable_slow_aim() {
  level.player disableslowaim();
}

function esc_embassy_fallen_vo() {
  wait 2;
  level scripts\sp\maps\embassy\embassy_util::say("dx_vom_us1_basement_traverse_02");

  if(!istrue(level.price.skip_fallen_vo)) {
    level.price scripts\sp\maps\embassy\embassy_util::say("dx_vom_pri_basement_traverse_10");
  }

  if(!istrue(level.price.skip_fallen_vo)) {
    level scripts\sp\maps\embassy\embassy_util::say("dx_vom_us1_basement_traverse_20");
    return;
  }
}

function esc_garage_vo() {
  level.player endon("death");
  level endon("reached_garage_exit");
  thread esc_embassy_fallen_vo();
  scripts\engine\utility::flag_wait("reached_rendezvous");
  wait 3;
  level.farah scripts\sp\maps\embassy\embassy_util::say("dx_vom_far_escape_garage_30");
  level.stacy scripts\sp\maps\embassy\embassy_util::say("dx_vom_stac_escape_garage_40");
  level.price.skip_fallen_vo = 1;
  wait 2;
  level.price scripts\sp\maps\embassy\embassy_util::say("dx_vom_pri_escape_garage_50");
  level.player scripts\sp\maps\embassy\embassy_util::say("dx_vom_kyle_escape_garage_60");
  level.price scripts\sp\maps\embassy\embassy_util::say("dx_vom_pri_escape_garage_70");
  level.hadir scripts\sp\maps\embassy\embassy_util::say("dx_vom_had_escape_garage_80");
  wait 0.5;
  wait 2;
  level.farah scripts\sp\maps\embassy\embassy_util::say("dx_vom_far_escape_garage_110");
  level.price scripts\sp\maps\embassy\embassy_util::say("dx_vom_pri_escape_garage_120");
  level.player scripts\sp\maps\embassy\embassy_util::say("dx_vom_kyle_escape_garage_130");
...
function escape_garage_keycard() {
...
function escape_combat_main() {
  ...
  thread scripts\sp\maps\embassy\embassy_util::swap_card_reader("es_garage_exit_reader");
...
function esc_com_vo() {
  level.player endon("death");
  wait 4.5;
  level.price scripts\sp\maps\embassy\embassy_util::say("dx_vom_pri_escape_combat_streets_30");
  level.farah scripts\sp\maps\embassy\embassy_util::say_as_chatter("dx_vom_far_escape_combat_streets_40");
  scripts\engine\utility::waittill_any_ents_array(getaiarray(), "weapon_fired");
  wait 0.2;
  level.stacy scripts\sp\maps\embassy\embassy_util::say_as_chatter("dx_vom_stac_escape_combat_streets_100");
  level.alex scripts\sp\maps\embassy\embassy_util::say_as_chatter("dx_vom_alx_escape_combat_streets_20");
  ...
  wait 0.4;
  scripts\sp\maps\embassy\embassy_util::wait_combat_cooldown(0.6, 4);
  wait 1;
  level.price scripts\sp\maps\embassy\embassy_util::say_as_chatter("dx_vom_pri_alley_moving_10");
}

function esc_com_enemy_vo() {
  level endon("esc_com_cleared");
  scripts\engine\utility::flag_wait("esc_com_pushed_to_courtyard");
  wait 3;
  scripts\sp\maps\embassy\embassy_util::wait_combat_cooldown(0.6, 4);
  level.alex scripts\sp\maps\embassy\embassy_util::say_as_chatter("dx_vom_alx_escape_combat_streets_90");
}
...
function esc_com_aq_alley_health_drop() {
  self endon("death");
  self endon("entitydeleted");
  scripts\engine\utility::flag_wait("reached_alley");
  self.health = 1;
...
function esc_com_price() {
  ...
  level.price scripts\sp\maps\embassy\embassy_util::go_to_node_targetname("esc_com_p_courtyard_start");
  scripts\engine\utility::flag_wait("esc_com_pushed_to_courtyard_exit");
  level.price scripts\sp\maps\embassy\embassy_util::go_to_node_targetname("esc_com_p_courtyard_exit");
  scripts\engine\utility::flag_wait("esc_com_pushed_to_alley_start");
  level.price scripts\sp\maps\embassy\embassy_util::go_to_node_targetname("esc_com_p_alley_start");
  scripts\engine\utility::flag_wait("reached_alley");
  level.price scripts\sp\maps\embassy\embassy_util::go_to_node_targetname("esc_com_p_alley_prep");
}

function esc_com_farah() {
  level endon("combat_done");
  ...
  level.farah scripts\sp\maps\embassy\embassy_util::go_to_node_targetname("esc_com_f_courtyard_start");
  scripts\engine\utility::flag_wait("esc_com_pushed_to_courtyard_exit");
  level.farah scripts\sp\maps\embassy\embassy_util::go_to_node_targetname("esc_com_f_courtyard_exit");
  scripts\engine\utility::flag_wait("esc_com_pushed_to_alley_start");
  level.farah scripts\sp\maps\embassy\embassy_util::go_to_node_targetname("esc_com_f_alley_start");
  scripts\engine\utility::flag_wait("reached_alley");
  level.farah scripts\sp\maps\embassy\embassy_util::go_to_node_targetname("esc_com_f_alley_prep");
}

function escape_combat_catchup() {
  if(!scripts\sp\starts::is_after_start("residence_arrival")) {
    thread periph_traffic("defend_entrance_door_closed");
  }
...
function ally_vo() {
  ...
  level.price scripts\sp\maps\embassy\embassy_util::say_as_chatter("dx_vom_pri_alley_moving_20");
  level.alex scripts\sp\maps\embassy\embassy_util::say_as_chatter("dx_vom_alx_alley_moving_30");
...
function alley_children_vo() {
  ...
  wait 1;
  level.alex thread scripts\sp\maps\embassy\embassy_util::say_as_chatter("dx_vom_alx_alley_moving_100");
  level.price thread scripts\sp\maps\embassy\embassy_util::say_as_chatter("dx_vom_pri_alley_moving_110");
  scripts\engine\utility::flag_wait("reached_alley_end");
  wait 1;
  level.farah scripts\sp\maps\embassy\embassy_util::say_as_chatter("dx_vom_far_alley_compound_10");
  level.price.dontrallynag = undefined;
}

function load_compound_transient() {
  waitframe();
  loadtransient("embassy_compound_anims_tr");
  loadtransient("embassy_streets_field_tr");
}

function alley_price() {
  ...
  level.price thread scripts\sp\maps\embassy\embassy_util::go_to_node_targetname("alley_p_start");
  scripts\engine\utility::flag_wait("reached_alley_start");
  level notify("price_alley_life_door");
  level.price thread scripts\sp\maps\embassy\embassy_util::go_to_struct_targetname("alley_p_end");
  scripts\engine\utility::flag_wait("reached_alley_end");
  level.price scripts\sp\maps\embassy\embassy_util::go_to_struct_targetname("res_arrival_p_start");
...
function alley_farah() {
  ...
  level.farah thread scripts\sp\maps\embassy\embassy_util::go_to_node_targetname("alley_f_start");
  scripts\engine\utility::flag_wait("reached_alley_start");
  ...
  level.farah thread scripts\sp\maps\embassy\embassy_util::go_to_struct_targetname("alley_f_end");
  scripts\engine\utility::flag_wait("reached_alley_end");
  ...
  level.farah scripts\sp\maps\embassy\embassy_util::go_to_struct_targetname("res_arrival_f_start");
...
function alley_life_kids_player_firing_listener() {

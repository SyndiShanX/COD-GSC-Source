 ...
 * Script: scripts\sp\maps\captive\captive_basement_stealth.gsc
****************************************************************/

function basement_stealth_flags() {
...
function basement_stealth_start() {
...
function basement_stealth_main() {
  ...
  thread basement_stealth_eventdists();
  thread basement_stealth_melee_counter_monitor();
  ...
  thread check_got_gun();
  level.basementguards = scripts\engine\sp\utility::array_spawn_targetname("basement_guards");
  thread check_in_drain();
  thread check_sprinting();
  thread check_rush_past_guards();
  ...
  level thread scripts\sp\maps\captive\captive_vo::vo_bs_guards_enter();
  thread guard_1_patrol_unaware();
  thread check_swipe_melee_dmg();
  thread check_swipe_melee_dmg();
  thread guard_2_patrol_unaware();
  ...
  if(isalive(level.guard1)) {
    ...
    level thread scripts\sp\maps\captive\captive_vo::vo_bs_climbing_stairs();
    level thread scripts\sp\maps\captive\captive_vo::mus_far_sister_infil();
  }
...
function audio_player_spawn_mud_loop() {
...
function audio_player_start_mud_loop() {
  if(isDefined(level.mud_sfx)) {
    level.mud_sfx scalevolume(1, 0.5);
    return;
  }
}

function audio_player_stop_mud_loop() {
  if(isDefined(level.mud_sfx)) {
    level.mud_sfx scalevolume(0, 0.75);
    return;
  }
}

function audio_player_delete_mud_loop() {
  if(isDefined(level.mud_sfx)) {
...
function basement_stealth_catchup() {
...
function guard_1_patrol_unaware() {
  self endon("death");
  self.ignoreall = 1;
  ...
  level thread scripts\sp\maps\captive\captive_vo::vo_bs_guards_sounds_alert();
  self.ignoreall = 0;
  ...
  wait 1;
  thread scripts\sp\maps\captive\captive_vo::vo_bs_callout_to_guard_2();
}
...
function check_interrupt_search() {
...
function guard_2_patrol_unaware() {
...
function basement_stealth_stairwell_cleanup() {
  level.player endon("death");
...
function basement_guard_spawn_func() {
  ...
  thread scripts\sp\maps\captive\captive_vo::vo_bs_stab_guard();
  ...
  scripts\sp\maps\captive\captive_util::pistol_weapon_user();
}

function basement_guard_death_func() {
  if(self.script_noteworthy == "basement_guard_2") {
    ...
    level thread scripts\sp\maps\captive\captive_vo::vo_bs_radio_convo();
  }
...
function stairwell_guard_should_hunt() {
  return false;
}

function check_got_gun() {
...
function basement_guard_combat_think() {
  scripts\sp\nvg\nvg_ai::flashlight_off();
}

function stairwell_guard_combat_think() {
...
function check_rush_past_guards() {

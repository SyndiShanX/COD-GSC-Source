 ...
 * Script: scripts\sp\maps\captive\captive_cellblock_escape.gsc
****************************************************************/

function cellblock_escape_flags() {
...
function cellblock_escape_start() {
...
function cellblock_escape_main() {
  ...
  thread scripts\sp\maps\captive\captive_vo::mus_far_sewer_crawl();
  thread check_exit_vent_look();
  thread check_view_window_scene();
  level thread scripts\sp\maps\captive\captive_vo::vo_cb_look_at_drain();
  level thread scripts\sp\maps\captive\captive_vo::vo_cb_linger_no_chair();
  level thread scripts\sp\maps\captive\captive_vo::vo_cb_looking_at_stairs();
...
function cellblock_escape_catchup() {
...
function check_exit_vent_look() {
  ...
  scripts\sp\maps\captive\captive_lighting::lights_on("fallen_grate");
  scripts\sp\maps\captive\captive_lighting::lights_off("break_final");
...
function chair_place_hint_timer() {
...
function check_view_window_scene() {
...
function create_vent_mantle_hint() {
...
function chair_idle_update() {
  if(self.showingcursor) {
    ...
    if(abs(level.player.origin[2] - self.origin[2]) > 6) {
      disable_chair_carry();
      return;
    }
...
function chair_check_show_cursor() {
  enable_chair_carry();
...
function chair_carry_enter() {
  ...
  self.carrycount++;
  level thread scripts\sp\maps\captive\captive_vo::vo_cb_chair_carry(self.carrycount);
...
function restore_default_player_control_post_carry() {

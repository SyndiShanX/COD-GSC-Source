 ...
 * Script: scripts\sp\maps\captive\captive_cell_escape.gsc
***********************************************************/

function cell_escape_flags() {
...
function cell_escape_main() {
  ...
  thread reset_prone_scale_post_first_raise();
  level thread scripts\sp\maps\captive\captive_vo::vo_ce_made_shiv();
  thread scripts\sp\maps\captive\captive_vo::mus_far_open_vent();
  scripts\engine\utility::flag_wait("reached_second_cell");
  level thread scripts\sp\maps\captive\captive_vo::vo_ce_enter_cell();
  level thread scripts\sp\maps\captive\captive_vo::vo_ce_rock_hints();
  level thread scripts\sp\maps\captive\captive_vo::vo_ce_button_hint();
  level thread scripts\sp\maps\captive\captive_vo::vo_ce_check_rock_near_button();
...
function spoon_check() {
  ...
  if(scripts\engine\utility::flag("did_failed_open_vent")) {
    level thread scripts\sp\maps\captive\captive_vo::vo_ce_pickup_spoon();
  }

  scripts\engine\utility::flag_set("got_spoon");
}

function reset_prone_scale_post_first_raise() {
  wait 4;
...
function stone_respawner_check() {
  self endon("death");
...
function stone_respawner_cursor_check() {
  self endon("death");
  level endon("stop_stone_respawner");
  scripts\engine\utility::flag_wait("got_spoon");
...
function stone_respawn_pool_check() {
  self endon("death");
  level endon("stop_stone_respawner");
...
function has_thrown_noisemaker() {
...
function check_pickup_noisemaker() {
...
function check_threw_noisemaker() {
  level endon("hit_celldoor_button");

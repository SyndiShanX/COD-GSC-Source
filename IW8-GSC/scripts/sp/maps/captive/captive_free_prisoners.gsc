 ...
 * Script: scripts\sp\maps\captive\captive_free_prisoners.gsc
**************************************************************/

function free_prisoners_flags() {
...
function free_prisoners_main() {
  ...
  if(scripts\engine\utility::flag("opened_gun_locker")) {
    ...
    thread women_get_guns_from_open_locker();
  } else {
    ...
    thread women_move_to_gun_locker();
  }
...
function free_prisoners_catchup() {
...
function upper_cell_door_button_check() {
...
function wait_upper_cell_door_button_switch() {
...
function women_move_to_gun_locker() {
  ...
  level.weaponslockerref notify("end_ayah_locker_idle");
  level.ayah thread scripts\sp\maps\captive\captive_util::play_single_anim_into_idle_anim(level.weaponslockerref, "weapon_locker_nag", "weapon_locker_idle", "end_ayah_locker_idle");
  wait 8;
  scripts\engine\utility::flag_waitopen("near_weapons_locker");
  level.weaponslockerref notify("end_darine_locker_idle");
  level.darine thread scripts\sp\maps\captive\captive_util::play_single_anim_into_idle_anim(level.weaponslockerref, "weapon_locker_nag", "weapon_locker_idle", "end_darine_locker_idle");
  wait 10;
  scripts\engine\utility::flag_waitopen("near_weapons_locker");
  level thread scripts\sp\maps\captive\captive_vo::vo_ff_weaponlocker_nag();
}

function women_get_guns_from_open_locker() {
  level.weaponslockerref notify("end_cell_aware_idle");
  level.weaponslockerref thread scripts\common\anim::anim_loop(level.allprisoners, "cell_aware_idle", "end_button_idle");
  wait 1.75;

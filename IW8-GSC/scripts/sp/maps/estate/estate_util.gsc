...
function hide_ents(var_0, var_1) {
...
function show_ents(var_0, var_1) {
  var_2 = getEntArray(var_0, "targetname");
...
function axis_stealth_filter(var_0) {
  switch (var_0.typeorig) {
    case "explode":
      ...
      if(isDefined(var_1)) {
        ...
        if(distancesquared(self.origin, var_1.origin) > var_2 * var_2) {
          return true;
        }
      ...
      break;
    case "light_killed":
      if(var_2.entity getscriptablepartstate("onoff") == "off") {
        if(scripts\engine\utility::is_equal(var_2.entity.targetname, "lights_bld_service")) {
          return true;
        }

        if(var_2.type == "combat") {
          var_2.type = "cover_blown";
        }
      }
...
function stealth_combat_music_updatefunc(var_0, var_1) {
...
function stealth_offhand_monitor() {
  for(;;) {
    ...
    thread offhand_explode_monitor(var_0);
  }
}

function offhand_explode_monitor(var_0) {
...
function fusebox_damage_think(var_0) {
  ...
  while(self.script_model.health > 0) {
    ...
    if(istrue(var_0) && var_2 != level.player) {
      self.script_model.health += var_1;
    }
  }
...
function fusebox_interact_anim_unlink_player(var_0) {
...
function fusebox_handle_down(var_0) {
...
function floodlight_death_watcher(var_0) {
...
function anyone_has_known_player_since_time(var_0, var_1) {
  if(!scripts\engine\utility::flag("stealth_enabled")) {
    return false;
  }
...
function make_price_pistol(var_0) {
  var_1 = ["rec_papa320|1", "mag_papa320|1", "slide_papa320|1"];

  if(istrue(var_0)) {
    GscBinSkip0(0x2e, var_1.size, "silencerpstl_west01");
  }
...
function get_closest_guy_by_path(var_0, var_1) {
  var_2 = undefined;
  var_3 = 1048576;
  var_4 = [];
...
function delete_at_distance_to_player(var_0, var_1, var_2) {
  var_3 = squared(var_1);
...
function waittill_player_stops_rotating_or_timeout(var_0) {
  var_1 = gettime();
...
function should_warn_nonsuppressed(var_0, var_1) {
...
function vo_warn_nonsuppressed(var_0) {
  ...
  if(isDefined(level.price) && isalive(level.price)) {
    ...
    for(;;) {
      var_2 = scripts\sp\maps\estate\estate_grounds::request_overwatch_vo("weapon", var_1, 1, 0);
...
function fight_with_player(var_0, var_1) {
  self endon("death");
  ...
  for(;;) {
    ...
    while(distancesquared(self.origin, var_2) > squared(200)) {
      ...
      var_4 += (0, randomfloatrange(-30, 30), 0);
    ...
    foreach(var_3 in level.friendlies) {
      if(isDefined(var_3.fighting_with_player)) {
        if(!istrue(var_3.move_override)) {
          stop_fighting_with_player(var_3);
          var_3 scripts\engine\sp\utility::enable_ai_color();
        }
      }
    }

    thread monitor_no_colors(var_0);
  }
}

function monitor_no_colors(var_0) {
...
function nags_til_notify(var_0, var_1, var_2, var_3) {
  if(!isarray(var_1)) {
    var_1 = [var_1];
  }
  ...
  jumpiffalse(isDefined(var_3)) LOC_00000055;
  wait var_3;
...
function delete_noteworthy_ents(var_0) {
  wait 1;
...
function abs_int(var_0) {
  return int(abs(var_0));
}

function make_alias_group(var_0, var_1) {
...
function alias_group_exists(var_0) {
  return isDefined(level.alias_groups[var_0]);
}

function clear_alias_group(var_0) {
  level.alias_groups[var_0] = undefined;
}

function get_next_alias_in_group(var_0, var_1) {
...
function increment_alias_group_index(var_0) {
  if(isstring(var_0)) {
...
function ownthenight_spawner_think(var_0) {
  jumpiffalse(isDefined(var_0)) LOC_0000002e;

...
function skip_intro(var_0, var_1) {
...
function light_meter_hint(var_0) {
...
function toggle_price_poi(var_0) {
  if(var_0) {
...
function vo_keep_up_nags(var_0, var_1) {
...
function actor_die_when_shot(var_0) {
...
function price_kill(var_0, var_1) {
...
function price_kill_logic(var_0, var_1) {
  if(!isalive(var_0)) {
    return;
  }
...
function waittill_price_aims_or_guy_dies(var_0, var_1) {
  var_0 endon("death");
  var_2 = cos(10);
  var_3 = 0;
  var_4 = 0;
  ...
  jumpiftrue(isarray(var_1)) LOC_00000047;
  var_1 = [var_1];
...
function woods_main() {
  ...
  var_0 thread scripts\sp\maps\estate\estate_util::lerp_fov_over_distance_trigger();
...
function gate_enemy_stealth_filter(var_0) {
...
function price_color_trigger_set(var_0) {
...
function vo_gate() {
  ...
  GscBinSkip4(0x35, "price_behind_building", var_0);
}
...
function price_gate_anim() {
  ...
  scripts\engine\sp\utility::flagwaitthread("rappel_start", &scripts\sp\maps\estate\estate_util::delete_at_distance_to_player, var_2.origin, 1500, [var_2, var_2.prop, level.gate_chain]);
...
function price_approach_gate(var_0) {
...
function stop_idle_on_spotted(var_0, var_1) {
  var_0 endon(var_1);
  scripts\engine\utility::flag_wait("stealth_spotted");
  var_0 notify("death");
}

function initgate(var_0, var_1, var_2, var_3) {
...
function vo_fusebox() {
  ...
  if(!scripts\engine\utility::flag("player_used_fusebox") && !scripts\engine\utility::flag("player_shot_door")) {
    ...
    if(distancesquared(level.player.origin, level.price.origin) > 90000) {
      ...
      level.price thread scripts\sp\maps\estate\estate_util::notetrack_nag(var_0, ["player_used_fusebox", "player_shot_door", "player_at_fusebox"]);
    ...
    level.price thread scripts\sp\maps\estate\estate_util::notetrack_nag(var_0, ["player_used_fusebox", "player_shot_door"]);
    return;
  }
}
...
function vo_light_enemies_convo(var_0, var_1) {
  level endon("tutorial_light_shot");
...
function set_flag_when_dead(var_0, var_1) {
  if(var_0.size) {
...
function price_kills_left_guys(var_0) {
  for(;;) {
    foreach(var_2 in var_0) {
...
function light_enemy_stealth_filter(var_0) {
...
function vo_rappel() {
  ...
  for(;;) {
    ...
    thread scripts\sp\maps\estate\estate_util::price_line(var_1[scripts\sp\maps\estate\estate_util::abs_int(var_2 % var_1.size)]);
    var_2++;
  }
}
...
function rappel_player() {
  ...
  for(;;) {
    ...
    var_17 += (var_23 - var_17) * var_18;

    if(var_22 > 0) {
      thread rappelfootstep(var_16);
...
function choose_rappel_weapon(var_0) {
  var_1 = undefined;
  var_2 = level.player getweaponslistprimaries();

  foreach(var_4 in var_2) {
    if(issubstr(var_4.basename, "_pi_") && has_ammo(var_4)) {
      var_1 = var_4;
  ...
  if(!isDefined(var_1)) {
    foreach(var_4 in var_2) {
      if(var_4 hasattachment("silenc", 1) && has_ammo(var_4)) {
        var_1 = var_4;
        break;
      }
    }
...
function player_rappel_weapon_switch(var_0) {
...
function doinjuredgesture(var_0) {
  while(nullweapon(level.player getcurrentweapon())) {
    waitframe();
  }
...
function rappel_actor_stealth_filter(var_0) {
...
function rappel_actor_dmg_func(var_0, var_1, var_2, var_3, var_4, var_5, var_6, var_7, var_8, var_9) {
...
function vo_price_take_em_out(var_0) {
...
function rappel_victim_death(var_0) {
...
function rappel_scene_vo(var_0, var_1) {
  level endon("rappel_actors_engaged");
...
function price_kills_rappel_enemies(var_0) {
  var_1 = 0;
  ...
  foreach(var_7 in var_0) {
    if(isalive(var_7)) {
      ...
      if(!scripts\engine\trace::ray_trace_passed(var_10, var_7 getEye(), [level.price, var_7])) {
        var_10 += anglesToForward(level.price gettagorigin("tag_flash")) * 50;
      }
...
function canrappelreload(var_0) {
  GscBinSkip1(0x45, "rifle", 1);
}

function tossclip(var_0) {
  if(!issubstr(var_0.basename, "_pi_")) {
    return;
  }
...
function spawn_price_infill() {
  ...
  var_0 = scripts\sp\maps\estate\estate_util::make_price_ar();
  level.price scripts\common\utility::initweapon(var_0);
  level.price scripts\anim\shared::placeweaponon(var_0, "back");
...
function stayahead_values_tallgrass(var_0) {

 ...
 * Script: scripts\sp\maps\proxywar\proxywar_forest.gsc
********************************************************/

function proxywar_forest_precache() {
  precachemodel("lighting_fixtures_security_lamp_01");
}

function proxywar_forest_flags() {
...
function proxywar_forest_hints() {
...
function proxywar_forest_spawn_funcs() {
...
function forest_trees_start() {
  ...
  thread forest_trees_ally_alpha();
}
...
function forest_trees_main() {
  ...
  level.forestmoveref = scripts\engine\utility::getStruct("forest_move_ref", "targetname");
  level thread scripts\sp\maps\proxywar\proxywar_vo::vo_fo_forest_walk();
  thread forest_trees_ally_bravo();
  thread scripts\sp\maps\proxywar\proxywar_vo::mus_forest_overlook();
  thread check_throw_grenade_during_patrol();
  ...
  thread overlook_patrol_animate(level.overlookpatrol2);
  thread check_overlook_patrol(level);
  thread check_player_visibility_to_patrol(level);
  thread check_player_flashlight_visible_to_patrol();
...
function check_player_flashlight_visible_to_patrol() {
  level endon("overlook_patrol_reached_end");
  level endon("alerted_patrol");
  wait 4;
...
function check_throw_grenade_during_patrol() {
  ...
  if(!scripts\engine\utility::flag("played_go_hot_admonishment")) {
    level thread scripts\sp\maps\proxywar\proxywar_vo::vo_fo_go_hot_admonishment();
    return;
  }
}
...
function forest_trees_ally_alpha() {
  thread check_forest_walk_anim_speed();
...
function forest_trees_alpha_to_ridge() {
...
function forest_trees_ally_bravo() {
  ...
  waitframe();
  level.forestmoveref notify("end_forest_idle_a");
  level.bravo_team scripts\sp\maps\proxywar\proxywar_util::play_group_single_anim_into_idle_anim(level.forestmoveref, "forest_move_b", "forest_move_b_idle", "end_forest_idle_b");
}

function rus_overlook_patrol_spawnfunc() {
  self.ignoreall = 1;
  self.ignoreme = 1;
...
function check_forest_walk_anim_speed() {
  ...
  level waittill("start_forest_move_a");
  level notify("end_anim_scale");
  thread scale_anim_on_player_speed(level.alpha_and_bravo_team, "forest_move_a");
...
function forest_trees_catchup() {
  scripts\engine\utility::exploder("begin_amb_fx");
}

function forest_overlook_start() {
  scripts\sp\maps\proxywar\proxywar_util::spawn_ally_teams();
...
function forest_overlook_main() {
  ...
  thread setup_laser_pointer_fsm();

  if(!scripts\engine\utility::flag("overlook_went_hot")) {
    level thread scripts\sp\maps\proxywar\proxywar_vo::vo_fo_overlook_reached_edge();
  }
  ...
  scripts\sp\maps\proxywar\proxywar_vo::vo_fo_target_calledin();
}

function forest_overlook_alpha1() {
...
function forest_overlook_alpha2() {
  level.ap_overlook_reveal scripts\common\anim::anim_single_solo(self, "overlook_reveal");
  level.ap_overlook_reveal thread scripts\common\anim::anim_loop_solo(self, "overlook_reveal_idle", "stop_loop_" + self.script_noteworthy);
  waitframe();
...
function overlook_group_spawn_func() {
  level endon("death");
  self endon("remove");
  self.ignoreall = 1;
  self.goal_radius = 0;
...
function setup_overlook_scene() {
  ...
  thread overlook_truck_behavior();
  thread overlook_suv_behavior();
  thread check_tower_guard();
}

function overlook_truck_behavior() {
  ...
  thread scripts\sp\maps\proxywar\proxywar_lighting::lights_on("truck_b_tail_lights");
...
function truck_move_on() {
  ...
  thread scripts\sp\maps\proxywar\proxywar_lighting::lights_off("truck_b_head_lights");
  thread scripts\sp\maps\proxywar\proxywar_lighting::lights_off("truck_b_tail_lights");
}

function clear_all_vehicle_lights() {
  thread scripts\sp\maps\proxywar\proxywar_lighting::lights_off("truck_b_head_lights");
  thread scripts\sp\maps\proxywar\proxywar_lighting::lights_off("truck_b_tail_lights");
...
function overlook_suv_behavior() {
  ...
  thread scripts\sp\maps\proxywar\proxywar_lighting::lights_on("suv_c_tail_lights");
...
function spotter_scope_check() {
  level endon("overlook_went_hot");
  thread spotterscope_check_lookat();
  ...
  scripts\sp\maps\proxywar\proxywar_vo::vo_fo_callin_strike();
  thread check_callin_strike();
}

function check_callin_strike() {
...
function railyard_guards_comment(var_0) {
  ...
  scripts\sp\maps\proxywar\proxywar_vo::vo_fo_railyard();
...
function clean_up_post_spotter_comment() {
  wait 1;
...
function spotterscope_equip_hint_check() {
...
function spotterscope_aim_hint_check() {
...
function spotterscope_fire_hint_check() {
...
function check_loud_weapon_fire() {
  level endon("overlook_went_hot");
...
function update_searchlight_facing() {
  for(;;) {
...
function wait_searchlight_destroyed() {
...
function forest_phosphorus_start() {
  scripts\sp\maps\proxywar\proxywar_util::spawn_ally_teams();
...
function forest_phosphorus_main() {
  ...
  thread scripts\sp\maps\proxywar\proxywar_vo::vo_fa_post_phosphorus();
...
function forest_phosphorus_freeze_offscreen() {
  scripts\engine\utility::flag_wait("start_halligan_open");
  wait 10;
  ...
  if(!scripts\sp\maps\proxywar\proxywar_util::within_player_fov(level.alpha1.origin) && !scripts\sp\maps\proxywar\proxywar_util::within_player_fov(level.alpha2.origin)) {
    ...
    while(!scripts\sp\maps\proxywar\proxywar_util::within_player_fov(level.alpha1.origin) && !scripts\sp\maps\proxywar\proxywar_util::within_player_fov(level.alpha2.origin)) {
      waitframe();
    }
...
function forest_phosphorus_alpha() {
  scripts\engine\utility::flag_wait("start_halligan_open");

  if(self.script_noteworthy == "alpha1") {
    level.alpha1 thread scripts\sp\maps\proxywar\proxywar_util::halligan_draw();
  }
  ...
  if(self.script_noteworthy == "alpha1") {
    level.alpha1 thread scripts\sp\maps\proxywar\proxywar_util::halligan_stow();
  }
...
function show_objective_tutorial() {
...
function forest_phosphorus_bravo() {
  scripts\engine\utility::flag_wait("trigger_drop_down");
...
function jets_drop_phosphorus() {
  ...
  thread scripts\sp\maps\proxywar\proxywar_lighting::lights_off("suv_c_head_lights");
  thread scripts\sp\maps\proxywar\proxywar_lighting::lights_off("suv_c_tail_lights");
  thread clear_all_vehicle_lights();
  ...
  scripts\engine\utility::delaythread(6, &scripts\sp\maps\proxywar\proxywar_util::alarm_sound);
}

function wp_explosions() {
...
function audio_wp_sizzle() {
  ...
  level.sfx_wp_02 scripts\engine\utility::delaycall(2, &playsound, "scn_wp_explo_travel_02");
  level.sfx_wp_01 moveTo((2332, 587, 397), 6, 0, 0);
  level.sfx_wp_02 scripts\engine\utility::delaycall(2, &moveto, (-182, -166, 858), 6, 0, 0);
  wait 20;
  level.sfx_wp_01 delete();
  level.sfx_wp_02 delete();
}

function guys_react_to_jets() {
  wait 1;
  ...
  thread play_jet_look();
  thread play_jet_look();
}

function play_jet_look() {
...
function forest_phosphorus_catchup() {
  thread scripts\sp\maps\proxywar\proxywar_lighting::lights_off("proxywar_lights_overlook");
...
function forest_overlook_door() {
  ...
  level.overlookdoor = scripts\sp\maps\proxywar\proxywar_util::setup_scripted_door("overlook_door");
...
function forest_patrol_main() {
  ...
  if(!scripts\engine\utility::flag("player_skipping_patrol")) {
    scripts\engine\utility::flag_wait_any_timeout(3, "player_fired_at_patrol", "player_too_close_to_patroller", "player_skipping_patrol");
  }
...
function forest_patrol_enemy_return() {
  ...
  if(scripts\engine\utility::flag("start_patrol_anim")) {
    ...
    wait randomfloatrange(1.5, 2) + level.patrol_return_offset;
  }
  ...
  if(!scripts\engine\utility::flag("player_fired_at_patrol") && !scripts\engine\utility::flag("player_too_close_to_patroller") && !scripts\engine\utility::flag("player_skipping_patrol")) {
    scripts\sp\maps\proxywar\proxywar_util::go_to_targetname("return_pmcs_end_" + self.script_noteworthy);
    return;
  }
...
function forest_patrol_enemy_approach_flashlight() {
...
function forest_patrol_enemy_light_sweep() {
  self endon("death");
  self endon("damage");
...
function forest_patrol_player_fire_checker() {
...
function forest_exit_main() {
  ...
  thread scripts\sp\maps\proxywar\proxywar_vo::vo_fe_burning_woods();
...
function forest_exit_ally() {
  ...
  scripts\sp\maps\proxywar\proxywar_util::go_to_targetname("patrol_exit_" + self.script_noteworthy);
    ...
    if(level.idlespottertime > 6 && !scripts\engine\utility::flag("spotter_actively_tracking") && !scripts\engine\utility::flag("done_facility_scan")) {
      ...
      if(var_0.size > 0) {
        ...
        switch (var_7.script_noteworthy) {
          ...
          case "overlook_railyard_enemy2":
          case "overlook_railyard_enemy1":
            level thread scripts\sp\maps\proxywar\proxywar_vo::vo_fo_callout_railway();
            ...
            break;
          case "overlook_truck":
            level thread scripts\sp\maps\proxywar\proxywar_vo::vo_fo_callout_truck();
          ...
          case "front_gate_guard_right":
          case "front_gate_guard_left":
            level thread scripts\sp\maps\proxywar\proxywar_vo::vo_fo_callout_checkpoint();
            self.endactionflag = "done_checkpoint_spot";
...
function laser_idle_enter() {
  scripts\sp\statemachine::set_notify_handlers([["track", &laser_track_handler], ["move_to", &laser_move_to_handler], ["complete", &laser_complete_handler]]);
  self.idlestarttime = gettime();
}

function laser_idle_update() {
  check_show_laser();
...
function laser_complete_handler() {

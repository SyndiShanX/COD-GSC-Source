...
function create_state_machine(var_0) {
  if(!isDefined(level.fa_state_machines)) {
...
function get_state_machine(var_0) {
  return level.fa_state_machines[var_0];
}

function add_state(var_0, var_1, var_2, var_3, var_4) {
  if(!isDefined(level.fa_state_machines)) {
    level.fa_state_machines = [];
  }
...
function remove_state(var_0, var_1) {
  if(!isDefined(level.fa_state_machines[var_0])) {
    return;
  }
...
function do_traverse_anim(var_0) {
  var_1 = get_traverse_anim(var_0);
  play_scripted_anim(var_1, undefined, &scripts\anim\traverse\shared::handletraversenotetracks, "traverseAnim", self.current_node);
}
...
function move_think() {
  ...
  var_16 = undefined;
  var_17 = scripts\engine\utility::random(var_8["node"] scripts\sp\fakeactor_node::fakeactor_node_get_cover_list());

  if(should_do_arrivals() && !var_8["node"] scripts\sp\fakeactor_node::fakeactor_node_is_traverse() && !var_8["node"] scripts\sp\fakeactor_node::fakeactor_node_is_turn() && var_8["node"] scripts\sp\fakeactor_node::fakeactor_node_allow_arrivals()) {
    var_18 = self;
    ...
    var_16 = get_arrival_anim(var_8["node"], var_18, var_17);

    if(isDefined(var_16)) {
      var_19 = getmovedelta(var_16, 0, 1);
      var_20 = getangledelta3d(var_16, 0, 1);
      var_21 = invertangles(var_20);
      var_22 = combineangles(var_8["angles"], var_21);
      var_23 = var_8["origin"] - rotatevector(var_19, var_22);
      var_8 = scripts\engine\utility::spawn_script_origin(var_23, var_22);
      add_state_ent(var_8["anim_node"]);
      var_8 = var_23;
      var_8 = var_22;
    }
  }
  ...
  if(var_9 == self.node_path.size) {} else {
    var_28 = var_27 + self.look_ahead_value;

    while(var_28 > self.node_path[var_9]["dist"]) {
      var_28 -= self.node_path[var_9]["dist"];
      var_9++;

      if(var_9 == self.node_path.size) {
        if(self.current_node != var_8["node"]) {
          self.current_node = var_8["node"];
        }
        ...
        var_30 = (0, 0, 0);
        var_31 = (0, 0, 0);
        var_32 = var_8["origin"] - self.origin;
        var_29 = vectortoangles(var_32);
        var_33 = length(var_32);
        var_8 = var_33 / var_6 * var_24;

        if(var_8 > 0) {
          if(var_7) {
            self moveTo(var_8["origin"], var_8);
            ...
            add_state_ent(var_7);
            self linkTo(var_7);
            var_7 moveTo(var_8["origin"], var_8);
        ...
        if(isDefined(var_16)) {
          self notify("stop_running_anim");
          play_scripted_anim(var_16, undefined, undefined, undefined, var_8["anim_node"], 0);
          var_8["anim_node"] delete();
          set_animsets([var_17]);
        } else {
      ...
      if(self.current_node != self.node_path[var_26]["node"]) {
        self.current_node = self.node_path[var_26]["node"];
        set_animsets(self.current_node scripts\sp\fakeactor_node::fakeactor_node_get_cover_list());
        fakeactor_check_node(self.current_node);
      }
    }

    var_31 = self.node_path[var_26]["to_next_node"] * var_30;
    var_31 += self.node_path[var_26]["origin"];
    var_32 = var_31;

    if(!var_24) {
      self.look_ahead_point = var_32;
    }

    var_33 = vectortoangles(var_32 - self.origin);
    GscBinSkip4(0x35, var_33, self.loop_time);
  }
...
function fakeactor_rotate_to(var_0, var_1) {
...
function fakeactor_check_node(var_0) {
...
function set_current_node(var_0) {
  if(isDefined(self.current_node)) {
    self.current_node scripts\sp\fakeactor_node::fakeactor_node_remove_claimed(self);
  }
...
function teleport_to_node(var_0) {
  set_current_node(var_0);
  self.current_node scripts\sp\fakeactor_node::fakeactor_node_set_claimed(self);
  fakeactor_check_node(self.current_node);
...
function fire_weapon(var_0) {
  self endon("death");
  GscBinSkip4(0x35);
}

function get_accuracy(var_0) {
...
function fake_bullet(var_0, var_1, var_2, var_3) {
...
function get_target_point(var_0) {
  if(isPlayer(var_0)) {
    if(is_human()) {
      var_1 = 50;
    } else {
    ...
    var_2 = var_1 getplayerangles();
    var_3 = var_1 getorigin() + anglestoup(var_2) * var_1;
    return var_3;
  }
...
function get_animation_from_alias(var_0, var_1, var_2, var_3) {
  ...
  if(isDefined(var_4)) {
    if(isarray(var_4.anims)) {
      ...
      var_5 = randomint(var_7.anims.size);
      return var_7.anims[var_5];
    }
...
function get_animation(var_0, var_1) {
...
function get_turn_anim(var_0, var_1, var_2) {
...
function get_arrival_anim(var_0, var_1, var_2) {
  if(!isDefined(var_2)) {
    var_2 = self.animset;
  }
...
function get_exit_anim(var_0, var_1, var_2, var_3) {
  if(!isDefined(var_1)) {
    var_1 = self.origin;
  }
...
function get_traverse_anim(var_0) {
...
function limit_dropped_weapons(var_0) {
...
function play_running_anim_internal(var_0, var_1) {
  if(!isDefined(var_1)) {
    var_1 = 1;
  }
...
function play_scripted_anim(var_0, var_1, var_2, var_3, var_4, var_5) {
  ...
  if(isDefined(var_2)) {
    thread scripts\anim\notetracks::donotetracks(var_3, var_2);
  }
...
function set_aim_target(var_0, var_1) {
...
function aim_weights(var_0, var_1, var_2, var_3, var_4, var_5) {
  var_6 = 0.1;
  var_7 = 1;
...
function set_run_anim_override(var_0) {
...
function set_idle_anim_override(var_0) {
...
function release_control(var_0) {
  set_controlled(0);
  var_1 = undefined;
...
function set_do_arrivals(var_0) {
  if(var_0) {
    self.flags |= 8;
    return;
  }
...
function set_wants_to_move(var_0) {
  if(var_0) {
    self.flags |= 2;
    return;
  }
...
function set_target_in_view(var_0) {
  if(var_0) {
    self.flags |= 1;
    return;
  }
...
function set_ignore_claimed(var_0) {
  if(var_0) {
    self.flags |= 64;
    return;
  }
...
function obstacle_in_way(var_0) {
  if(var_0) {
    self.flags |= 128;
    return;
  }
...
function set_recent_pain(var_0) {
  if(var_0) {
    self.flags |= 2048;
    return;
  }
...
function trigger_fakeactor_move(var_0) {
  ...
  for(;;) {
    ...
    set_wants_to_move(var_1, 1);
  }
}

function trigger_fakeactor_node_disable(var_0) {
  if(!isDefined(var_0.targetname)) {
    return;
  }

  var_1 = scripts\engine\utility::getStructArray(var_0.targetname, "target");
  jumpiffalse(var_1.size == 0) LOC_0000002a;
  return;
}

function trigger_fakeactor_node_enable(var_0) {
  if(!isDefined(var_0.targetname)) {
    return;
  }
...
function trigger_fakeactor_node_enablegroup(var_0) {
  jumpiftrue(isDefined(var_0.script_parameters)) LOC_0000000f;
  return;
}

function trigger_fakeactor_node_disablegroup(var_0) {
  jumpiftrue(isDefined(var_0.script_parameters)) LOC_0000000f;
  return;
}

function trigger_fakeactor_node_passthrough(var_0) {
  if(!isDefined(var_0.targetname)) {
    return;
  }
...
function trigger_fakeactor_node_lock(var_0) {
  if(!isDefined(var_0.targetname)) {
    return;
  }
...
function is_higher_priority(var_0, var_1) {

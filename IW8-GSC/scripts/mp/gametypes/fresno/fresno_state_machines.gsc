...
function vehicle_occupancy_showcashbag(var_0) {
  if(level.ref_11e18.ref_13bef[var_0.agent_type] >= level.delayedeventtypes[16].secondwindthink) {
    ...
    var_0 thread scripts\mp\gametypes\br_publicevent_fresno::kk_onkilled();
    var_0 scripts\mp\gametypes\br_alt_mode_mxp::tplayinterruptableanim("s4_mp_kenosha_idle_staggered_01");
    return 0;
  }
...
function wait_for_tanks_almost_gone(var_0) {
  ...
  if(isPlayer(var_2)) {
    ...
    var_0.ref_13f03 = var_2.as_player;
  }

  scripts\mp\gametypes\br_alt_mode_mxp::wait_in_spectate_for_time(var_0);
  var_3 = scripts\mp\gametypes\br_alt_mode_mxp::randomizeattacklocation(var_2.origin, level.ref_11e18.waitandstartplunderpolling);
  var_4 = scripts\mp\gametypes\br_public::semtex_used();
  var_3 = scripts\mp\gametypes\br_public::modifyplayer_damage(var_3, abs(var_3[2]) + var_4);
  ref_11e19(var_0, var_3, &scripts\mp\gametypes\br_alt_mode_mxp::vehicleturretshootthread);
  var_5 = vectortoangles(var_3 - var_0.origin);
  _getrandomlocations::vehicle_spawn_cp_gamemodesupportsabandonedtimeout(var_3, var_5, level.ref_11e18.waitandstartplunderpolling);
  waitframe();
  scripts\mp\gametypes\br_alt_mode_mxp::waitfornukecarriernearlz(var_0);
...
function gmoveoffpathtofrenzypoint(var_0, var_1) {
  ...
  if(isDefined(level.ref_11e18.gz_monolith)) {
    var_5 = level.ref_11e18.gz_monolith.origin;
...
function gisclosetofrenzypoint(var_0, var_1) {
  ...
  if(var_3 <= level.ref_11e18.ggoaldist) {
    return true;
  }
...
function post_blockade_breadcrumb_struct(var_0) {
  if(level.ref_11e18.ref_13bef[var_0.agent_type] >= level.delayedeventtypes[16].secondwindthink) {
    ...
    var_0 thread scripts\mp\gametypes\br_publicevent_fresno::gz_onkilled();
    var_0 scripts\mp\gametypes\br_alt_mode_mxp::ref_13c1d("s4_mp_greenbay_idle_staggered_01");
    return 0;
  }
...
function send_munition_used_notify(var_0) {
  ...
  scripts\mp\gametypes\br_alt_mode_mxp::set_distances_for_groups(var_0);
  var_3 = scripts\mp\gametypes\br_alt_mode_mxp::randomizeattacklocation(var_2.origin, level.ref_11e18.gsmallkillstreakradius);
  var_4 = scripts\mp\gametypes\br_public::semtex_used();
  var_5 = scripts\mp\gametypes\br_public::modifyplayer_damage(var_3, abs(var_3[2]) + var_4);
  var_6 = level.ref_11e18.gtargetairborneheight < var_3[2] - var_5[2];

  if(var_6) {
    var_7 = vectortoangles(var_3 - var_0.origin);
    _getrandomlocations::greenbaystrikeatpoint(var_3, var_7, level.ref_11e18.gsmallkillstreakradius);
  } else {
    var_7 = vectortoangles(var_6 - var_1.origin);
    _getrandomlocations::serverroomdogtagrevive(var_6, var_7, level.ref_11e18.gsmallkillstreakradius);
  }

  waitframe();
  scripts\mp\gametypes\br_alt_mode_mxp::set_relic_grounded(var_1, 0, 0);
  scripts\mp\gametypes\br_alt_mode_mxp::set_relic_aggressive_melee_params(var_1);
  var_1.ref_13f03 = undefined;
}
...
function is_safe_point(var_0) {
  ...
  if(isDefined(level.ref_11e18.pe_endtime) && isDefined(level.ref_11e18.safe_circle_end_time)) {
    return (level.ref_11e18.safe_circle_end_time > level.ref_11e18.pe_endtime);
  }
...
function light_kk_monolith(var_0) {
  ...
  level.ref_11e18.kk_monolith = var_2;
...
function light_gz_monolith(var_0) {
  ...
  level.ref_11e18.gz_monolith = var_2;
...
function disable_kk_monolith() {
  if(isDefined(level.ref_11e18.kk_monolith)) {
    level.ref_11e18.kk_monolith setscriptablepartstate("fx", "stop");
  }

  level.ref_11e18.kk_monolith = undefined;
}

function disable_gz_monolith() {
  if(isDefined(level.ref_11e18.gz_monolith)) {
    level.ref_11e18.gz_monolith setscriptablepartstate("fx", "stop");
  }

  level.ref_11e18.gz_monolith = undefined;
}
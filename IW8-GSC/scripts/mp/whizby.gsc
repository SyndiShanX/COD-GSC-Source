...
function dowhizby(var0, var1) {
  if(gettime() - var1 > 190 && !scripts\mp\utility\player::isusingremote() && getDvar("scr_whizby_off") == "") {
  ...
  thread scripts\mp\battlechatter_mp::addrecentattacker(var0);
  return var1;
}
...
function whizbyblurshoweffect(var0) {
  if(getdvarint("scr_suppression", 1) == 1) {
    ...
    if(var1 hasattachment("linearbrake", 1) && !scripts\mp\utility\weapon::iskillstreakweapon(var1) && !scripts\mp\utility\perk::_hasperk("specialty_blastshield")) {
      ...
      scripts\mp\gamescore::trackdebuffassist(var0, self, var1.basename);
...
function whizbyblurrampdown(var0, var1) {
  ...
  self.suppressionmagnitude = 0;
  scripts\mp\gamescore::untrackdebuffassist(var0, self, var1);
}

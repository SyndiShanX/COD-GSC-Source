...
function sr_pick_up_tag(var_0) {
  ...
  thread clear_bot_on_bot_death(var_0);
  self.suspend_sd_role = 1;
  GscBinSkip4(0x35, var_0, "tag_picked_up", self, var_0);
}

function watch_tag_destination(var_0) {
  self endon("stop_watch_tag_destination");

  for(;;) {
    if(!var_0 scripts\mp\bots\bots_gametype_conf::custom_death_func(self.team)) {
      wait 0.05;
    }
...
function sr_ally_near_tag(var_0) {
...
function clear_bot_on_bot_death(var_0) {
  self endon("reset");
  var_1 = var_0.team;
...
function notify_when_tag_picked_up_or_unavailable(var_0, var_1) {
  self endon("stop_tag_watcher");

  while(var_0 scripts\mp\bots\bots_gametype_conf::custom_death_func(self.team) && !scripts\mp\bots\bots_gametype_conf::bot_check_tag_above_head(var_0)) {
    wait 0.05;
  }
...
function sr_camp_tag(var_0) {
  ...
  thread clear_bot_camping_on_bot_death(var_0);
  self.suspend_sd_role = 1;
  scripts\mp\bots\bots_personality::clear_camper_data();
  var_1 = self.role;

  while(var_0 scripts\mp\bots\bots_gametype_conf::custom_death_func(self.team) && self.role == var_1) {
...
function clear_bot_camping_on_bot_death(var_0) {
  self endon("reset");
  var_1 = var_0.team;
  var_0 waittill("death_or_disconnect");

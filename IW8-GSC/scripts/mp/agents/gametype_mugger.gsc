...
function agent_player_mugger_think() {
  thread scripts\mp\bots\bots_gametype_mugger::bot_mugger_think();
}

function agent_squadmember_mugger_think() {
  ...
  if(gettime() > self.next_time_check_tags) {
    ...
    if(isbot(self.owner)) {
      var0 = self botgetfovdot();
    }

    var1 = self.owner getnearestnode();

    if(isDefined(var1)) {
      var2 = self.owner scripts\mp\bots\bots_gametype_mugger::bot_find_visible_tags_mugger(var1, var0);

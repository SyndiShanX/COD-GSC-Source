...
function player_update_unresolved_collision(var0, var1) {
  ...
  if(self.unresolved_collision_count >= var2) {
    var3 = "[Unresolved Collision] p: #" + self getentitynumber() + " @" + self.origin;

    if(isDefined(var0)) {
      var3 += " m: #" + var0 getentitynumber() + " @" + var0.origin;
      ...
      if(isDefined(var0.unresolved_collision_func)) {
        ...
        var4 = isDefined(self.waittill_player_opens_scavenger_cache) && gettime() - self.waittill_player_opens_scavenger_cache <= 250;

        if(var4 || var0 scripts\cp_mp\vehicles\vehicle::isvehicle() && !scripts\cp_mp\vehicles\vehicle::ref_1418b(var0, self)) {
          var3 += " nearest_node false";
          unresolved_collision_nearest_node(var0, self, 0, var0);
        } else {
          var5 = player_should_be_killed_by_mover(var0);
          var3 += " nearest_node " + var5;
          unresolved_collision_nearest_node(var0, self, var5, var0);
        }
      }
    } else {
      var3 += " m: <undefined>";
      var5 = player_should_be_killed_by_mover(var0);
      var3 += " nearest_node " + var5;
      unresolved_collision_nearest_node(self, var5);
    }
...
function player_should_be_killed_by_mover(var0) {
  if(isDefined(var0) && istrue(var0.moverdoesnotkill)) {
    return false;
  }
...
function player_unresolved_collision_watch() {
  ...
  for(;;) {
    ...
    player_update_unresolved_collision(var0, var1);
  }
}

function ref_124b5(var0) {
  var1 = 0;

  if(isDefined(var0.streakinfo) && var0.streakinfo.streakname == "cruise_predator") {
    var1 = 1;
  }

  if(istrue(var0.play_incoming_rpg_vo)) {
    var1 = 1;
  }

  if(isDefined(var0.get_teaminquiry_alias)) {
    var1 = 1;
  }
...
function unresolved_collision_nearest_node(var0, var1, var2) {
  ...
  if(isDefined(level.hostage_callout_saveme_time)) {
    var3 = self[[level.hostage_callout_saveme_time]](var0, var1, var2);
  ...
  if(isDefined(var2)) {
    if(scripts\cp_mp\utility\game_utility::islargemap()) {
      ...
      if(var2 scripts\cp_mp\vehicles\vehicle::isvehicle()) {
        var4 = scripts\cp_mp\vehicles\vehicle_occupancy::vehicle_occupancy_getdriverseat(var2, 1);

        if(isDefined(var4)) {
          var5 = scripts\cp_mp\vehicles\vehicle_occupancy::vehicle_occupancy_getexitpositionandangles(var2, var0, var4, 1);

          if(isDefined(var5)) {
            var0 setOrigin(var5[0]);
            return;
          }
        }
      } else if(scripts\mp\weapons::isplantedequipment(var2)) {
        var2 scripts\mp\weapons::deleteexplosive();
        return;
      } else if(isDefined(var2.cover)) {
        var2.cover scripts\mp\equipment\tactical_cover::tac_cover_destroy(undefined, 0);
        return;
      } else if(isDefined(level.ref_11c87) && [[level.ref_11c87]](var0, var2)) {
        return;
      }
    } else {
      var6 = self.unresolved_collision_nodes;
      var7 = undefined;

      if(isDefined(var6)) {
        var6 = sortbydistance(var6, var0.origin);
      } else {
        var6 = getnodesinradiussorted(var0.origin, 500, 0, 128, "path");
      }

      var8 = (0, 0, -100);
      var0 cancelmantle();
      var0 dontinterpolate();
      var9 = get_mover_ents(var2);
      var10 = scripts\engine\utility::array_combine(var9, level.characters);
      ...
      for(var11 = 0; var11 < var6.size; var11++) {
        ...
        var12 = var6[var11];
        var13 = var12.origin + (0, 0, 10);
        var14 = scripts\engine\trace::create_contents(0, 1, 1, 1, 1, 1);
        var15 = scripts\engine\trace::ray_trace(var0 gettagorigin("tag_eye"), var13, var10, var14);

        if(isDefined(var15["hittype"]) && var15["hittype"] != "hittype_none") {
          continue;
        }
        ...
        if(positionwouldtelefrag(var13)) {
          continue;
        }
...
function handle_moving_platform_touch(var0) {
  ...
  for(;;) {
    ...
    if(isDefined(var0.ref_123b4) && var0.ref_123b4) {
      if(_calloutmarkerping_handleluinotify_enemyrepinged::tugofwar_tank(var1)) {
        waitframe();
        continue;
      }
    }

    if(!istrue(var0.ref_133ca)) {
      thread process_moving_platform_death(var0, var1);
    }

    self.ref_13bff = 1;
    self.wam_number_of_failures = var1;
    thread ref_123b9();
    break;
  }
}
...
function handle_moving_platforms(var0) {
  ...
  if(isDefined(var0.linkparent)) {
    ...
    if(!isDefined(var1) || var1 != var0.linkparent) {
      if(isDefined(var0.x1givelaststandoverride)) {
        self linkTo(var0.linkparent, "", var0.x1givelaststandoverride, var0.angleoffset);
      } else {

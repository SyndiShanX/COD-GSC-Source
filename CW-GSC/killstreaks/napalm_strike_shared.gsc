    ...
    if(index == 0) {
      ...
      if(isDefined(killstreak_id)) {
        plane killstreakrules::function_2e6ff61a(bundlename, killstreak_id, {
          #origin: position, #team: team
        });
      }

      plane killstreak_dialog::play_pilot_dialog_on_owner("arrive", bundlename, killstreak_id);
...
function private function_6f537b95(killstreak_id) {
  bundlename = sessionmodeiszombiesgame() ? "napalm_strike_zm" : "napalm_strike";
  wait 9;
  self killstreak_dialog::play_pilot_dialog_on_owner("waveStartFinal", bundlename, killstreak_id);
}
...
function private spawnplane(startnode, owner, killstreak_id, killstreakbundle, killstreaktype) {
  ...
  if(getdvarint(#"scr_napalm_strike_capture_model", 0)) {
    plane setModel(#"veh_t9_mil_air_flogger_fly_napalm");
  }
...
function private function_c248485(plane, targetpoint, team, owner, dosound = 0, location) {
  ...
  for(i = 0; i < killstreakbundle.var_2df25e4a; i++) {
    var_5496c504 = util::spawn_model(#"veh_t9_mil_us_air_napalm_bomb_projectile", self.origin);
    var_5496c504 ghost();
...
function function_985141f2(owner, startpos, normal, direction, killcament, team, killstreakbundle) {
  ...
  foreach(lockey in var_b1dd2ca0) {
    ...
    var_acf59456 = {
      #damageendtime: damageendtime, #damageposition: damageposition, #killcament: killcament, #owner: owner, #team: team
    };

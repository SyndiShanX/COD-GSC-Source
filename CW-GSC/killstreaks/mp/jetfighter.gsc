 ...
 * Script: killstreaks\mp\jetfighter.gsc
***********************************************/
...
function function_fee93694(killstreakbundle) {
  if(isDefined(killstreakbundle.kstype) && isinarray(level.var_a78d8a55, killstreakbundle.kstype)) {
    if(!isDefined(level.var_500867a0[killstreakbundle.kstype])) {
      level.var_500867a0[killstreakbundle.kstype] = [];
    } else if(!isarray(level.var_500867a0[killstreakbundle.kstype])) {
      level.var_500867a0[killstreakbundle.kstype] = array(level.var_500867a0[killstreakbundle.kstype]);
    }

    if(!isinarray(level.var_500867a0[killstreakbundle.kstype], self)) {
      level.var_500867a0[killstreakbundle.kstype][level.var_500867a0[killstreakbundle.kstype].size] = self;
    }
...
function private function_4b3b25af(killstreak_id) {
  ...
  self thread killstreak_dialog::play_killstreak_start_dialog("jetfighter", self.team, killstreak_id);
  team = self.team;
  ...
  var_ce2c18d3.pilotindex = killstreak_dialog::get_random_pilot_index("jetfighter");
  var_ce2c18d3 killstreakrules::function_2e6ff61a("jetfighter", killstreak_id, {
    #origin: var_ce2c18d3.origin, #team: team
  });
...
function private function_6ff76fc6() {
  ...
  return {
    #var_d44b8c3e: var_d44b8c3e, #angles: angles
  };
}
...
function private function_8f23fd43() {
  if(!is_true(self.scoreeventprocessed) && isDefined(self.owner)) {
    ...
    self.owner stats::function_b04e7184("jetfighter", #"best_destructions");
  }
}
...
function function_c578ef61(targets, killstreak_id) {
  ...
  self killstreak_dialog::play_pilot_dialog_on_owner(dialogkey, "jetfighter", killstreak_id);
}
...
function function_1e2a93a2(var_6709bbd6) {
  self killstreak_dialog::play_pilot_dialog_on_owner("activateCounter", "jetfighter", self.killstreak_id);
  var_6709bbd6 killstreak_dialog::play_destroyed_dialog_on_owner("jetfighter", self.killstreak_id);
  self killstreak_dialog::play_pilot_dialog_on_owner("killAircraft", "jetfighter", self.killstreak_id);
  self function_6fe870ea();
}

function function_6fe870ea() {
  self killstreak_dialog::play_taacom_dialog_response_on_owner("timeoutConfirmed", "jetfighter", self.killstreak_id);
}

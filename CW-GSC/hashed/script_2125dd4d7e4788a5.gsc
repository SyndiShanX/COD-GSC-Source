 ...
 * Script: hashed\script_2125dd4d7e4788a5.gsc
***********************************************/
...
function init(instance) {
  ...
  foreach(s_instance in instance.contentgroups[#"retrieval"]) {
    ...
    for(i = 0; i < 2; i++) {
      ...
      mdl_canister = util::spawn_model(#"p9_sur_console_control_01_canister_full", var_9ed85498.origin + (0, 0, 32.1), var_9ed85498.angles + (0, -90, 0));
      mdl_canister.targetname = "retrieval_component";
      mdl_canister.n_id = i;
      mdl_canister.n_index = var_9ed85498.script_int;
      instance.var_53ba65b[i] = mdl_canister;
      instance.a_models[instance.a_models.size] = mdl_canister;
      mdl_canister.var_e16d0db5 = content_manager::spawn_script_model(var_9ed85498, #"p9_fxanim_sv_canister_mod", 1);
      mdl_canister.var_e16d0db5.targetname = "retrieval_case";
      mdl_canister thread function_6acbc957(instance);
    }
  }
...
function function_62801b2f() {
  ...
  for(i = 0; i < a_zombies.size; i++) {
    if(isalive(a_zombies[i])) {
      if(a_zombies[i].zm_ai_category === # "boss" || a_zombies[i] === level.var_c39a4b8f) {
        continue;
      }
  ...
  if(isalive(zombie)) {
    ...
    if(self flag::get(#"hash_35735a804a34da04")) {
      level scoreevents::doscoreeventcallback("scoreEventSR", {
        #scoreevent: "outbreak_mq_aether_bunny_kill_zm", #attacker: self, #enemy: zombie
      });
    } else {
  ...
  if(isDefined(self.e_attacker) && isalive(self) && isDefined(n_dmg)) {
    if(self.e_attacker flag::get(#"hash_35735a804a34da04")) {
      if(self.zm_ai_category === # "elite") {
        level scoreevents::doscoreeventcallback("scoreEventSR", {
          #scoreevent: "outbreak_mq_aether_bunny_elite_kill_zm", #attacker: self.e_attacker, #enemy: self
        });
      } else if(self.zm_ai_category === # "special") {
        level scoreevents::doscoreeventcallback("scoreEventSR", {
          #scoreevent: "outbreak_mq_aether_bunny_special_kill_zm", #attacker: self.e_attacker, #enemy: self
        });
      }
    }
...
function function_f5087df2() {
  ...
  if(!is_true(self.success)) {
    foreach(mdl_canister in getEntArray("retrieval_canister", "targetname")) {
...
function function_d3122380(instance) {
  ...
  instance waittill(#"canister_collected");
  ...
  if(b_result) {
    if(isalive(e_player)) {
      if(!is_true(e_player.var_67aa3392.var_470d03b1)) {
        level scoreevents::doscoreeventcallback("scoreEventSR", {
          #scoreevent: "retrieve_perfect_delivery_zm", #attacker: e_player
        });
      }
    ...
    if(isDefined(e_player)) {
      ...
      self.e_object.var_5ea363c notify(#"installed", {
        #player: e_player
      });
...
function function_63544551(instance) {
  ...
  while(true) {
    ...
    level scoreevents::doscoreeventcallback("scoreEventSR", {
      #scoreevent: "retrieve_battery_installed_zm", #attacker: s_waitresult.player
    });
    self.var_c1c0db3c++;
...
function function_5238fe78(instance) {
  ...
  self endon(#"death", #"attackers_done");
  ...
  while(true) {
    ...
    if(isDefined(s_pt)) {
      ...
      if(isDefined(s_spawnpt)) {
        ai_spawned = namespace_85745671::function_9d3ad056(str_aitype, s_spawnpt.origin, s_spawnpt.angles, "retrieval_zombie");
        wait 0.1;
...
function function_9d2e5887(instance) {
  ...
  while(true) {
    ...
    if(isDefined(s_pt)) {
      ...
      wait randomfloatrange(0.1, 0.3);
      ai_spawned = namespace_85745671::function_9d3ad056(str_aitype, s_pt.origin, s_pt.angles, "retrieval_zombie");
      wait 0.1;
  ...
  while(true) {
    for(i = 0; i < n_spawns; i++) {
      ...
      if(isDefined(var_559503f1[i])) {
        ...
        ai_spawned = namespace_85745671::function_9d3ad056(str_aitype, var_559503f1[i].origin, var_559503f1[i].angles, "retrieval_zombie");
        wait 0.1;
...
function function_dd9b1007(v_pt) {
  ...
  ai_spawned = namespace_85745671::function_9d3ad056(str_aitype, v_pt, self.angles, "retrieval_zombie");
  wait 0.1;
...
function function_20ab9060(instance) {
  ...
  while(true) {
    ...
    if(isDefined(e_activator) && isPlayer(e_activator) && !is_true(e_activator.var_28107825) && !is_true(self.b_pickedup)) {
      ...
      foreach(mdl_canister in var_53ba65b) {
        if(mdl_canister != self && isDefined(mdl_canister.var_1474d2ee)) {
          objective_setinvisibletoplayer(mdl_canister.var_1474d2ee, player);
        }
      }
...
function retrieval_start(instance, activator) {
  if(isPlayer(activator)) {
    instance notify(#"retrieval_go");
    ...
    foreach(mdl_canister in instance.var_53ba65b) {
      mdl_canister thread function_5238fe78(instance);
      mdl_canister thread function_6d863da0(instance);
      mdl_canister thread function_6cbbd741(instance);
    }

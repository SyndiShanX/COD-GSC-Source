 ...
 * Script: hashed\script_72d96920f15049b8.gsc
***********************************************/
...
function function_5160bb1e(killstreaktype) {
  ...
  level.chopper_gunner.fx_flare = bundle.fxflares;
  ...
  self thread killstreak_dialog::play_killstreak_start_dialog("chopper_gunner", self.team, killstreak_id);
  ...
  level.chopper_gunner killstreakrules::function_2e6ff61a("chopper_gunner", killstreak_id, {
    #origin: var_e31d6cb1, #team: level.chopper_gunner.team
  });
...
function function_696b3380() {
  ...
  while(true) {
    ...
    offsetright = randomintrange(0, 30);
    offsetup = randomintrange(0, 50);
    self pathvariableoffset((0, offsetright, offsetup), offsettime);
    var_c6ac5940 = 0;
    wait offsettime * 2;
  }
}
...
function ontimecheck() {
  self killstreak_dialog::play_pilot_dialog_on_owner("timecheck", "chopper_gunner", self.killstreak_id);
}
  ...
  while(true) {
    ...
    while(player useButtonPressed() && player killstreaks::function_59e2c378()) {
      ...
      if(timeused > var_f6263fe2) {
        self killstreak_dialog::play_pilot_dialog_on_owner("remoteOperatorRemoved", "chopper_gunner", self.killstreak_id);
...
function function_bfb33872(choppergunner, eattacker, weapon) {
  ...
  if(isDefined(eattacker) && (!isDefined(choppergunner.owner) || choppergunner.owner util::isenemyplayer(eattacker))) {
    ...
    choppergunner killstreak_dialog::play_destroyed_dialog_on_owner("chopper_gunner", choppergunner.killstreak_id);
...
function function_77784598(einflictor, eattacker, idamage, idflags, smeansofdeath, weapon, vpoint, vdir, shitloc, vdamageorigin, psoffsettime, damagefromunderneath, modelindex, partname, vsurfacenormal) {
  ...
  if(isDefined(var_5aca61fb)) {
    self killstreak_dialog::play_pilot_dialog_on_owner(var_5aca61fb, "chopper_gunner", self.killstreak_id);
  }
  ...
  if(self.health > 0 && var_902cbab5 <= 0 && !self.shuttingdown) {
    ...
    if(!sessionmodeiszombiesgame() && !isDefined(self.destroyscoreeventgiven) && isDefined(idflags) && (!isDefined(self.owner) || self.owner util::isenemyplayer(idflags))) {
      ...
      self killstreak_dialog::play_destroyed_dialog_on_owner("chopper_gunner", self.killstreak_id);
    ...
    if(isDefined(self.owner)) {
      ...
      self.owner killstreaks::function_e9873ef7("chopper_gunner", self.killstreak_id, #"streak_destroyed");
    }
...
function function_568f6426() {
  ...
  if(isDefined(dialogkey) && self.owner.killstreakdialogqueue.size === 0) {
    self killstreak_dialog::play_pilot_dialog_on_owner(dialogkey, "chopper_gunner", self.killstreak_id);
  }
}
 ...
 * Script: hashed\script_6b4984dd2d3d3eea.gsc
***********************************************/
...
function init() {
  ...
  level.var_9d295460 = getweapon(#"listening_device_spy");
  ...
  level.suspect_gender_type = undefined;
  level.var_92a0168 = 0;
  ...
  level.var_c5c6c8d = 1;
  level.encodedradiousebar = encodedradio_usebar::register();
...
function function_3a93afbe(attacker) {
  ...
  if(self.state == 1 || self.state == 0) {
    ...
    if(isDefined(attacker.spyRole) && attacker.spyRole != 1) {
...
function private function_13f62d64(triggerstruct, angle_offset) {
  ...
  ang_offset = angle_offset;
  deaddrop function_92d262c6(ang_offset);
  waitframe(1);

  if(deaddrop.state == 0) {
    deaddrop.prev_state = deaddrop.state;
    deaddrop.state = 1;
    level.var_28f7b75b++;

    if(activator.spyRole == 1) {
  ...
  if(level.var_28f7b75b >= level.var_1424a0f7 && isDefined(deaddrop.prev_state)) {
...
function function_8bdff396(watcher, player) {
  ...
  self.weapon = getweapon(#"listening_device_spy");
  self.var_48d842c3 = 1;
  self.var_515d6dda = 1;
...
function function_50e09e10() {
  ...
  foreach(listeningdevice in level.var_cd6cf4fb) {
    if(!isDefined(level.var_f1f9d4a8[listeningdevice.script_noteworthy])) {
      level.var_f1f9d4a8[listeningdevice.script_noteworthy] = listeningdevice;
    }
...
function function_3fef0b35() {
  ...
  if(isDefined(player)) {
    foreach(listeningdevice in level.var_f1f9d4a8) {
      listeningdevice thread function_8f295af0(player);
    }
  }
}
...
function private watchshutdown(listeningdevice) {
  level endon(#"game_ended");
  ...
  arrayremovevalue(level.var_f1f9d4a8, listeningdevice);

  foreach(var_dfb87540 in listeningdevice.var_dfb87540) {
    arrayremovevalue(listeningdevice.var_dfb87540, var_dfb87540);
  ...
  listeningdevice function_f3da38fe();

  if(listeningdevice.e_device.state == 0) {
    level.var_50ee8e07++;
  }
...
function private function_152526e9() {
  ...
  if(isDefined(waitresult.attacker.spyRole) && waitresult.attacker.spyRole != 0 && waitresult.attacker.spyRole != 1) {
...
function private function_4d118352(attacker, callback_data) {
  ...
  if(isDefined(callback_data) && isDefined(callback_data.spyRole) && callback_data.spyRole == 1) {
...
function function_7ddae2fa(listeningdevice) {
  level endon(#"game_ended");
  self endon(#"death");
  listeningdevice endon(#"death");
  listeningdevice.e_device waittill(#"hash_666be90931a4b0e8");
...
function private function_f37d284() {
  level endon(#"game_ended");
  self.model = util::spawn_model(#"p9_wz_dirty_bomb_01", self.origin, self.angles);
...
function function_704fdca0(state, player) {
  ...
  if(state == 4) {
    ...
    player function_3bf6db8b(self.trigger, #"detonatingspymode");
  }

  if(state == 5) {
    self.var_1afb163d.using = 1;
    player flag::set("dirtybomb_stopCountDown");
    player function_3bf6db8b(self.var_1afb163d, #"stopcountdown");
  }
...
function function_5c0f763b(player) {
  ...
  player.var_9dc65a85 = 0;
  player function_5b2918e4();
  player spy_skill::function_53a81144(level.var_7e569431.var_133ba5a7);
  ...
  while(true) {
    ...
    if(a_players.size) {
      foreach(player in a_players) {
        foreach(spy_plane in level.var_4bb6bc83) {
          var_c6fecd88 = spy_plane has_line_of_sight(player);
...
function function_fabc8042() {
  foreach(encodedradio in level.var_e27df64e) {
    encodedradio.ent = util::spawn_model(#"hash_1a3554b5405e3cbf", encodedradio.origin, encodedradio.angles);
    offsets = (0, 0, 0);
    encodedradio.state = 0;
    encodedradio.objectiveid = gameobjects::get_next_obj_id();
    encodedradio thread function_94dd997d(encodedradio.objectiveid, encodedradio.ent.origin - offsets, #"hash_3c511372ef417c1e");
    ...
    if(!isinarray(level.var_a08e14c4, encodedradio.objectiveid)) {
      level.var_a08e14c4[level.var_a08e14c4.size] = encodedradio.objectiveid;
    }

    encodedradio thread function_945ccf34();
    encodedradio thread function_9d70f23a(offsets, 80, 50);
  }
...
function function_1a976b61() {
  ...
  if(var_ca806887 == "male") {
    level.suspect_gender_type = 1;
  } else if(var_ca806887 == "female") {
    level.suspect_gender_type = 2;
  } else {
    level.suspect_gender_type = 0;
  }

  function_b455be75(level.suspect_gender_type);
...
function private function_ce60e248(triggerstruct) {
  ...
  activator = triggerstruct.activator;
  encodedradio = self.encodedradio;

  if(encodedradio.state != 0) {
    return;
  }

  if(isDefined(encodedradio.trigger.activator)) {
    return;
  }

  activator function_e440f785(encodedradio);
}
...
function private function_20fdcbf2(var_fbdcd95b, state) {
  if(!level.encodedradiousebar encodedradio_usebar::is_open(self)) {
    level.encodedradiousebar encodedradio_usebar::open(self, 4);
  }

  level.encodedradiousebar encodedradio_usebar::set_state(self, state);
  level.encodedradiousebar encodedradio_usebar::function_f0df5702(self, 0);
}
...
function function_3d8b0693() {
  ...
  if(level.encodedradiousebar encodedradio_usebar::is_open(self)) {
    level.encodedradiousebar encodedradio_usebar::close(self);
  }
...
function private function_36d5c855(frac) {
  if(level.encodedradiousebar encodedradio_usebar::is_open(self)) {
    level.encodedradiousebar encodedradio_usebar::function_f0df5702(self, frac);
  }
}
...
function function_b1cbacc1() {
  if(isDefined(level.var_e27df64e)) {
    foreach(encodedradio in level.var_e27df64e) {
      arrayremovevalue(level.var_e27df64e, encodedradio);
      encodedradio function_f3da38fe();
      encodedradio.state = 0;

      if(isDefined(encodedradio.trigger)) {
        encodedradio.trigger delete();
      }

      if(isDefined(encodedradio.disabletrigger)) {
        encodedradio.disabletrigger delete();
      }

      if(isDefined(encodedradio.ent)) {
        encodedradio.ent delete();
      }
    }
...
function function_702cbb0d(player, notificationid, var_2df0c486, countdowntimer) {
  ...
  level.spyhud hud_spy::function_91a8637f(player, 0);
  ...
  level.spyhud hud_spy::function_91a8637f(player, notificationid);

  if(isDefined(countdowntimer)) {
    level.spyhud hud_spy::function_65a4dc3c(player, countdowntimer);
  }

  if(isDefined(var_2df0c486)) {
    level.spyhud hud_spy::function_ceb51392(player, var_2df0c486);
  }
}
...
function function_fa207882(notification_id) {
  player = self;
  ...
  if(isDefined(player)) {
    player clientfield::set_to_player("to_player_notification", notification_id);
  }
...
function function_1d3f6978() {
  ...
  assert(isDefined(var_882ee981), "<dev string:x52>");
  ...
  assert(isDefined(target1), "<dev string:x52>");
...
function init_devgui() {
  ...
  adddebugcommand("<dev string:x137>");

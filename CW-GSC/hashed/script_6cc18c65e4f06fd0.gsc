 ...
 * Script: hashed\script_6cc18c65e4f06fd0.gsc
***********************************************/
  ...
  if(isDefined(level.var_7f41d246)) {
    foreach(player_struct in level.var_7f41d246) {
      if(zm_utility::is_player_valid(player_struct.player, 0, 1)) {
        ...
        player_struct.player zm_challenges::function_d6b32ad3(player_struct.weapon, #"eliteeliminations", 3);
...
function function_c9cec280() {
  ...
  while(true) {
    self waittill(#"launch_hellhound");
  ...
  while(true) {
    ...
    if(isDefined(waitresult.amount) && isPlayer(waitresult.attacker)) {
      ...
      if(zm_utility::is_player_valid(attacker, 0, 0)) {
        ...
        struct_params = {
          #player: attacker, #weapon: weapon
        };
        level.var_7f41d246[attacker getentitynumber()] = struct_params;
      }
...
function function_c658f846() {
  ...
  for(var_7e46a1cf = undefined; !isDefined(var_7e46a1cf); var_7e46a1cf = zm_ai_raz::spawn_single(1, undefined, n_round)) {
...
function function_37597f29() {
  ...
  util::add_debug_command("<dev string:x97>");
  util::add_debug_command("<dev string:xfe>");
  util::add_debug_command("<dev string:x16f>");
  util::add_debug_command("<dev string:x1d0>");
  util::add_debug_command("<dev string:x229>");
  util::add_debug_command("<dev string:x282>");

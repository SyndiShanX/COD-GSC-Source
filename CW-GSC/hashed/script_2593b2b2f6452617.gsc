 ...
 * Script: hashed\script_2593b2b2f6452617.gsc
***********************************************/
...
function init(instance) {
  ...
  instance.mdl_console setdestructibledef(#"hash_46b1078a533a5a9f");
  instance.mdl_platform = s_instance.mdl_platform;
...
function function_c64d2826() {
  self endon(#"head_drop", #"disconnect");
  ...
  self val::set(#"carry_head", "disable_weapon_cycling", 1);
...
function function_9c294ad5(e_player) {
  ...
  s_result = e_player waittill(#"head_drop", #"fake_death", #"inserted");
...
function function_f4b436d5() {
  ...
  self waittill(#"head_drop", #"inserted");
  self.is_drinking = 0;
...
function function_a56e1736() {
  self endon(#"objective_ended");
  self waittill(#"underattack");
  self flag::wait_till(#"start_vo_done");
...
function function_5839db26() {
  ...
  for(i = 0; i < n_spawns; i++) {
    ...
    if(isDefined(a_s_pts[i])) {
      ...
      ai_spawned = namespace_85745671::function_9d3ad056(str_aitype, a_s_pts[i].origin, self.var_43123efe.angles, "defend_zombie");
...
function function_34ac205(instance, n_active, var_559503f1, n_wave) {
  ...
  while(true) {
    ...
    if(instance.n_active < n_active) {
      ...
      str_aitype = get_aitype(instance, level.var_b48509f9);
      ...
      ai_spawned = namespace_85745671::function_9d3ad056(str_aitype, s_loc.origin, s_loc.angles, "defend_zombie");
...
function function_22281e97(instance) {
  ...
  for(j = 0; j < n_total; j++) {
    ...
    ai_spawned = namespace_85745671::function_9d3ad056(str_aitype, a_s_pts[i].origin, a_s_pts[i].angles, "defend_zombie");
...
function function_763876af(instance) {
  instance endon(#"objective_ended");
  self endon(#"death");
  self thread cleanup_attackable(instance);
  ...
  if(isDefined(self.health) && isDefined(self.max_health) && self.health / self.max_health >= 0.8) {
    level scoreevents::doscoreeventcallback("scoreEventSR", {
      #scoreevent: "defend_pristine_zm", #allplayers: 1
    });
  }
...
function clear_vehicles(instance) {
  ...
  for(i = 0; i < var_453e0a89.size; i++) {
    if(distance2dsquared(self.origin, var_453e0a89[i].origin) <= sqr(n_radius)) {
      ...
      var_453e0a89[i] thread teleport_vehicle(self);
      wait 0.1;
    }
  }
    ...
    foreach(vehicle in var_453e0a89) {
      if(isDefined(vehicle) && distance2dsquared(self.origin, vehicle.origin) <= sqr(n_radius)) {
        if(!is_true(vehicle.abnormal_status.emped)) {
          ...
          vehicle thread teleport_vehicle(self);
        }
      }
    }

 ...
 * Script: hashed\script_5ab5be2e8aaf56f8.gsc
***********************************************/
...
function event_handler[level_init] main(eventstruct) {
  ...
  clientfield::register("scriptmover", "" + # "zombie_marked", 1, 1, "int");
  clientfield::register("scriptmover", "" + # "rocket_damage", 1, 1, "counter");
...
function init(instance) {
  ...
  for(i = 0; i < instance.var_42dcbe3d.size; i++) {
    ...
    v_start = v_org;
    var_954ec1b6 = util::spawn_model(#"p9_sur_machine_rocket_01", v_start, instance.var_42dcbe3d[i].angles);
    ...
    var_954ec1b6 thread cleanup_attackable();
...
function function_79417d24(einflictor, eattacker, idamage, idflags, smeansofdeath, weapon, var_fd90b0bb, vpoint, vdir, shitloc, psoffsettime, iboneindex, imodelindex) {
  if(isDefined(iboneindex) && isPlayer(iboneindex)) {
    ...
    self clientfield::increment("" + # "rocket_damage");
  }
...
function function_dc79011b(instance) {
  ...
  while(true) {
    foreach(zombie in function_a38db454(self.origin, self.var_75833abc)) {
      if((zombie.aitype === # "spawner_bo5_avogadro_sr" || zombie.archetype === # "zombie" || zombie.aitype === # "spawner_bo5_soa") && !isDefined(zombie.is_marked)) {
...
function function_f919d221(instance) {
  instance endon(#"objective_ended", #"players_dead");
  self endon(#"death");
  self setdestructibledef(#"hash_4de038bb5090f659");
...
function function_55ea0c81(n_time) {
  ...
  self endon(#"objective_ended", #"next_attack", #"start_attack", #"players_dead");
  wait n_time;
  self notify(#"next_attack");
}
...
function function_ddb668e6() {
  ...
  for(i = 0; i < n_spawns; i++) {
    ...
    if(isDefined(v_pt)) {
      ...
      ai_spawned = namespace_85745671::function_9d3ad056(str_aitype, v_pt, self.angles, "droppod_zombie");
    }
...
function function_5efbdd83(instance) {
  ...
  level scoreevents::doscoreeventcallback("scoreEventSR", {
    #scoreevent: "secure_transmitter_secured_zm", #allplayers: 1
  });

  if(isDefined(self.health) && isDefined(self.max_health) && self.health / self.max_health >= 0.8) {
    level scoreevents::doscoreeventcallback("scoreEventSR", {
      #scoreevent: "secure_pristine_zm", #nearbyplayers: 1, #location: self.origin
    });
  }
  ...
  dropstruct = {
    #origin: self.origin + (0, 0, 40), #angles: self.angles + (0, 90, 0), #var_738dfc81: 1
  };
...
function on_dog_killed(params) {
  ...
  if(str_aitype === # "hash_7a8b592728eec95d" && isDefined(var_954ec1b6)) {
